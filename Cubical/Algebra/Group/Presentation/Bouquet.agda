...
module _ {ℓ} {A : Type ℓ} (_≟_ : Discrete A) where
 ...
 noEmpNorm : ∀ f → NonEmpty (NF f) 
 noEmpNorm = ElimProp.f w
  where
  w : ElimProp (λ z → NonEmpty (NF z))
  ElimProp.isPropB w _ = isPropΠ λ _ → isProp⊥
  ElimProp.εB w x = x ([] , refl , tt*)
  ElimProp.ηB w a x = x ([ (true , a) ] , (sym (idr _)) , (λ ()) , _)
  ElimProp.invB w a = _∘ _∘ {!!}
  ElimProp.·B w a b x x₁ x₂ = {!!}
...
--  record ElimSet≡/₃ (X : typ _≡/₃∙_ → Type ℓ'') : Type (ℓ-max (ℓ-max ℓ ℓ') ℓ'') where
--   field
--    isSetX : ∀ x → isSet (X x) 
--    a→x : ∀ a  → X [ a ]≡/₃
