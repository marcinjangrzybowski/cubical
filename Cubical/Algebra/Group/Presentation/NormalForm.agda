...
module Cubical.Algebra.Group.Presentation.NormalForm where
...
-- open import Cubical.HITs.FreeGroupoid as FG renaming (assoc to ·assoc)
...
fromList' : ⟨ ΩFG A ⟩ → List (𝟚 × A) → ⟨ ΩFG A ⟩
fromList' = foldr (_∙_ ∘ loop*) 

fromList : List (𝟚 × A) → ⟨ ΩFG A ⟩
fromList = fromList' refl
...
module _ {ℓ} {A : Type ℓ} (_≟_ : Discrete A) where
 ...
 -- NFSnd :  ⟨ ΩFG A ⟩ → List (𝟚 × A) →  hProp _
 -- NFSnd g l = ((fromList l ≡ₚ g) ) ⊓ Normalised l 
