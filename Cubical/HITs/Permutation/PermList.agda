{-# OPTIONS --safe #-}
module Cubical.HITs.Permutation.PermList where
...
module _ (A : Type ℓ) where
 ...
 ×^→List : ∀ n → (A ×^ n) → List A
 ×^→List zero x = []
 ×^→List (suc n) (x , xs) = x ∷ ×^→List n xs

 IsoΣX×^List : Iso (Σ _ (A ×^_)) (List A) 
 Iso.fun IsoΣX×^List = ×^→List _ ∘ snd 
 Iso.inv IsoΣX×^List = (_ ,_) ∘ List→×^
 Iso.rightInv IsoΣX×^List = w
  where
   w : section (Iso.fun IsoΣX×^List) (Iso.inv IsoΣX×^List)
   w [] = refl
   w (x ∷ b) i = x ∷ (w b i)
 Iso.leftInv IsoΣX×^List = w
  where
  w : retract (Iso.fun IsoΣX×^List) (Iso.inv IsoΣX×^List)
  w (zero , a) = refl
  w (suc n , x , xs) i = 
    let (n' , xs') = w (n , xs) i
    in suc n' , x , xs' 

 ΣL* : ∀ n → (ℙrm {true} n) → ∀ n' → (Dec (n ≤ n')) → Type ℓ
 ΣL* n 𝕡 n' q? = (True q? × 𝕍₃ A n 𝕡)
           ⊎ (False q? × List A)


 ΣL : ∀ n → (ℙrm {true} n) → ℕ → Type ℓ
 ΣL n 𝕡 n' = ΣL* n 𝕡 n'  (n ≤? n')


 ΣL≃* : ∀ n → (l : List A) → ∀ q? → (ΣL* n 𝕡base (length l) q?)
 ΣL≃* n l (yes p) = inl (_ , {!!})
 ΣL≃* n l (no ¬p) = {!!}


 ΣL→ : ∀ n → List A → Σ ℕ (ΣL n 𝕡base)
 ΣL→ n l = length l , ΣL≃* n l (n ≤? length l)

 ΣL← : ∀ n → Σ ℕ (ΣL n 𝕡base) → List A
 ΣL← n x = {!!}
 ...
 𝕃 n = R𝕡elimSet.f (w n)
  where
  w : ∀ n → R𝕡elimSet (λ z → Σ (Type ℓ) (λ T → T ≃ Σ ℕ (ΣL n z)))
  R𝕡elimSet.isSetA (w n) = {!!}
  R𝕡elimSet.abase (w n) = List A , {!!}
  R𝕡elimSet.aloop (w n) = {!!}
  R𝕡elimSet.alooop (w n) = {!!}
