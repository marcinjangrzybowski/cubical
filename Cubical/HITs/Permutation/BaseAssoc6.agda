{-# OPTIONS --safe #-}
module Cubical.HITs.Permutation.BaseAssoc6 where
...
open import Cubical.Data.NatPlusOne renaming (one to one*)
...
-- -- record Cast (A : Type ℓ) : Type (ℓ-suc ℓ) where
-- --  constructor cast
-- --  field
-- --    {B} : Type ℓ
-- --    B≃ : B ≃ A
...
-- -- CastIso : Iso (Cast A) A
-- -- Iso.fun CastIso = Cast.uncast
-- -- Iso.inv CastIso = cast (idEquiv _)
-- -- Iso.rightInv CastIso b = refl
-- -- Iso.leftInv (CastIso {A = A}) (cast B≃ b) =
-- --  let z = snd (EquivContr A) (_ , B≃) 
-- --  in λ i → cast (z i .snd)
-- --      (glue (λ { (i = i0) → fst B≃ b ; (i = i1) → b }) (fst B≃ b))



-- +₁-comm : ∀ m n  → m +₁ n ≡ n +₁ m
-- +₁-comm one* n = cong 2+_ (sym (ℕprops.+-zero _ ))
-- +₁-comm (1+ (ℕ.suc n')) n =
--   cong (one* +₁_) (+₁-comm (1+ n') n) ∙
--     cong 2+_ (sym (ℕprops.+-suc _ _))
...
--   fromℕ₊₁ : ℕ₊₁ →  ℕₐ⁺¹
--   fromℕ₊₁ one* = one
--   fromℕ₊₁ (1+ (ℕ.suc n)) = one + fromℕ₊₁ (1+ n)


--   toℕ₊₁ : ℕₐ⁺¹ → ℕ₊₁
--   toℕ₊₁ = ℕₐ⁺¹rec.f w {true} {subst isSet (1+Path) ℕprops.isSetℕ }
--    where
--    w : ℕₐ⁺¹rec ℕ₊₁
--    ℕₐ⁺¹rec.aOne w = one*
--    ℕₐ⁺¹rec._a+_ w = _+₁_
--    ℕₐ⁺¹rec.a-assoc w = +₁-assoc
   
--   toFromℕ₊₁ : section toℕ₊₁ fromℕ₊₁
--   toFromℕ₊₁ one* = refl
--   toFromℕ₊₁ (1+ (ℕ.suc n)) =
--     cong (one* +₁_) (toFromℕ₊₁ (1+ n))


--   fromToℕ₊₁ : retract toℕ₊₁ fromℕ₊₁
--   fromToℕ₊₁ = ℕₐ⁺¹elim.f w
--    where
--     w : ℕₐ⁺¹elim (λ z → fromℕ₊₁ (toℕ₊₁ z) ≡ z)
--     ℕₐ⁺¹elim.asquash w _ = isProp→isSet (isSetℕₐ⁺¹ _ _ )
--     ℕₐ⁺¹elim.aOne w = refl
--     (w ℕₐ⁺¹elim.a+ p) q = {!!} ∙ cong₂ _+_ p q
--     ℕₐ⁺¹elim.a-assoc w = {!!}
...
--  isGroupoidℕₐ⁺¹ : isGroupoid ℕₐ⁺¹ 

-- record Elimℕₐ⁺¹ {ℓ} (A : ℕₐ⁺¹ → Type ℓ) : Type ℓ where
--  no-eta-equality
--  field
...
--  f (+-pentagon-□ x x₁ x₂ x₃ i i₁) = 
--      +a-pentagon-□ (f x) (f x₁) (f x₂) (f x₃) i i₁
--  f (isGroupoidℕₐ⁺¹ x x₁ x₂ y x₃ y₁ i i₁ x₄) =
--      isOfHLevel→isOfHLevelDep 3 (asquash)
--       _ _ _ _
--      (λ i j → f (x₃ i j)) (λ i j → f (y₁ i j))
--      (isGroupoidℕₐ⁺¹ x x₁ x₂ y x₃ y₁) i i₁ x₄

-- record ElimSetℕₐ⁺¹ {ℓ} (A : ℕₐ⁺¹ → Type ℓ) : Type ℓ where
--  no-eta-equality
--  field
--   asquash : ∀ x → isSet (A x)
--   onea : A oneₐ
...
--   where
--   w : Elimℕₐ⁺¹ (λ z → A z)
--   Elimℕₐ⁺¹.asquash w = isSet→isGroupoid ∘ asquash
--   Elimℕₐ⁺¹.onea w = onea
--   Elimℕₐ⁺¹.+a w = +a
--   Elimℕₐ⁺¹.+a-assoc w = +a-assoc
--   Elimℕₐ⁺¹.+a-pentagon-diag w {a'} {b'} {c'} {d'} a b c d i =
--     comp (λ j → A (+-pentagon-□ a' b' c' d' (~ j) i))
--       (λ j → λ {(i = i0) →  +a (+a-assoc a b c j) d
--                ;(i = i1) → +a a (+a-assoc b c d (~ j))
--                }) (+a-assoc a (+a b c ) d (~ i)) 
--   Elimℕₐ⁺¹.+a-pentagon-△ w _ _ _ _ =
...
-- fromℕ₊₁ : ℕ₊₁ →  ℕₐ⁺¹
-- fromℕ₊₁ one* = oneₐ
-- fromℕ₊₁ (1+ (ℕ.suc n)) = oneₐ + fromℕ₊₁ (1+ n)
...
-- -- toℕ₊₁ (x + x₁) = toℕ₊₁ x +₁ toℕ₊₁ x₁
-- -- toℕ₊₁ (+-assoc x x₁ x₂ i) =
-- --   +₁-assoc (toℕ₊₁ x) (toℕ₊₁ x₁) (toℕ₊₁ x₂) i
-- -- toℕ₊₁ (+-pentagon-diag x x₁ x₂ x₃ i) =
-- --   (cong (_+₁ (toℕ₊₁ x₃)) (sym (+₁-assoc (toℕ₊₁ x) (toℕ₊₁ x₁) (toℕ₊₁ x₂)))
-- --     ∙∙ sym (+₁-assoc (toℕ₊₁ x) (toℕ₊₁ x₁ +₁ toℕ₊₁ x₂ ) (toℕ₊₁ x₃)) ∙∙
-- --     cong ((toℕ₊₁ x) +₁_) (sym (+₁-assoc (toℕ₊₁ x₁) (toℕ₊₁ x₂) (toℕ₊₁ x₃)))) i
-- -- toℕ₊₁ (+-pentagon-△ x x₁ x₂ x₃ i i₁) = {!!}
-- -- toℕ₊₁ (+-pentagon-□ x x₁ x₂ x₃ i i₁) = {!!}
-- -- toℕ₊₁ (isGroupoidℕₐ⁺¹ x x₁ x₂ y x₃ y₁ i i₁ x₄) = {!!}

-- -- sectionToℕ₊₁fromℕ₊₁ : section toℕ₊₁ fromℕ₊₁
-- -- sectionToℕ₊₁fromℕ₊₁ one = refl
-- -- sectionToℕ₊₁fromℕ₊₁ (2+ n) = cong (one +₁_) (sectionToℕ₊₁fromℕ₊₁ (1+ n))

-- -- +-lem : ∀ n m → fromℕ₊₁ (n +₁ m) ≡
-- --         fromℕ₊₁ (n) + fromℕ₊₁ (m)
-- -- +-lem one m = refl
-- -- +-lem (1+ (ℕ.suc n)) m = cong (oneₐ +_) (+-lem (1+ n) m) ∙ +-assoc _ _ _

-- -- retractToℕ₊₁fromℕ₊₁ : retract toℕ₊₁ fromℕ₊₁
-- -- retractToℕ₊₁fromℕ₊₁ = ElimSetℕₐ⁺¹.f w
-- --  where
-- --  w : ElimSetℕₐ⁺¹ (λ z → fromℕ₊₁ (toℕ₊₁ z) ≡ z)
-- --  ElimSetℕₐ⁺¹.asquash w _ = isGroupoidℕₐ⁺¹ _ _
-- --  ElimSetℕₐ⁺¹.onea w = refl
-- --  ElimSetℕₐ⁺¹.+a w {a} {a'} p q = +-lem (toℕ₊₁ a) (toℕ₊₁ a') ∙ cong₂ _+_ p q
-- --  ElimSetℕₐ⁺¹.+a-assoc w a b c =
-- --    {!!}
...
-- -- --  w : retract toℕ₊₁ fromℕ₊₁
-- -- --  w oneₐ = refl
-- -- --  w (a + a₁) = +-lem (toℕ₊₁ a) (toℕ₊₁ a₁) ∙ cong₂ _+_ (w a) (w a₁)
-- -- --  w (+-assoc a a₁ a₂ i) = {!!}
-- -- --  w (+-pentagon-diag a a₁ a₂ a₃ i) = {!!}
-- -- --  w (+-pentagon-△ a a₁ a₂ a₃ i i₁) = {!!}
-- -- --  w (+-pentagon-□ a a₁ a₂ a₃ i i₁) = {!!}
-- -- --  w (isGroupoidℕₐ⁺¹ a a₁ x y x₁ y₁ i i₁ x₂) = {!!}

-- -- -- Isoℕₐ⁺¹ℕ₊₁ : Iso  ℕₐ⁺¹ ℕ₊₁
-- -- -- Iso.fun Isoℕₐ⁺¹ℕ₊₁ = toℕ₊₁
-- -- -- Iso.inv Isoℕₐ⁺¹ℕ₊₁ = fromℕ₊₁
-- -- -- Iso.rightInv Isoℕₐ⁺¹ℕ₊₁ = sectionToℕ₊₁fromℕ₊₁
-- -- -- Iso.leftInv Isoℕₐ⁺¹ℕ₊₁ = {!!}

-- -- -- assocLI : ∀ x y z → {!cong′ (fromℕ₊₁ ∘ toℕ₊₁) (+-assoc x y z)!} ≡ {!!}
-- -- -- assocLI = {!!}
