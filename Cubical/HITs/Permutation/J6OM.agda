{-# OPTIONS --safe #-}
module Cubical.HITs.Permutation.J6OM where
...
-- tied-comm : (q : base ≡ base) → Square {A = Path J₃S¹ base base}
--                 (λ i → tied i ∙' q)
--                 (λ i → q ∙ tied i)
--                 refl
--                 refl
...
-- -- --            --  })
-- -- --            --  (compPath'-filler ((λ j' → loop₂ (j' ∨ ~ j) (j' ∨ j)))
-- -- --            --    (CoLoop (ung↑ j x)) (~ l) k)
...
-- -- --              ;(l = i1) → CoLoopSqM.sq'-fill l' i k x
-- -- --             })
-- -- --             (compPath'-filler loop (CoLoop (ung↑ i x)) (~ l) k)
-- -- --       ;(i = i1) → ((λ l → compPath'-filler loop (CoLoop (ung↑ j x)) (~ l) k)
-- -- --                     ∙∙ refl ∙∙ λ l → CoLoopSqM.sq'-fill l j k x) l
          

-- -- --       ;(j = i1) → ((λ l → compPath'-filler loop (CoLoop (ung↑ i x)) (~ l) k)
-- -- --                     ∙∙ refl ∙∙ λ l → CoLoopSqM.sq'-fill l i k x) l

-- -- --       ;(k = i0) → {!!}
-- -- --       ;(k = i1) → {!!} 
      
-- -- --          })
-- -- --          ((CoLoopCu'-hlp ◁ CoLoopCu') i j x k)
-- -- --          -- ((CoLoop (ung♯ i j x)) k)
-- -- --      -- (CoLoop (ung♯ i j x) k)
-- -- --      -- 

-- -- -- -- l' = i0 ⊢ hcomp (doubleComp-faces loop (λ _ → base) k) (CoLoop x k)
-- -- -- -- l' = i1 ⊢ ?10 (x = x) (k = k) (l = l) (l' = i1)
-- -- -- -- l = i0 ⊢ compPath'-filler loop (CoLoop (↑ x)) l' k
-- -- -- -- l = i1 ⊢ hcomp
-- -- -- --          (λ { j₁ ((k ∨ ~ k) = i1)
-- -- -- --                 → doubleComp-faces loop (λ _ → base) k (~ l' ∧ j₁) _
-- -- -- --             ; j₁ ((~ l') = i0) → outS (inS (CoLoop x k))
-- -- -- --             })
-- -- -- --          (CoLoop x k)
-- -- -- -- k = i0 ⊢ ?20 (i = i0) (x = x) (l = l) (l' = l')
-- -- -- -- k = i1 ⊢ base
