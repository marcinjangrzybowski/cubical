...
module PentaJ1Cong {ℓ} {A' : Type ℓ} {x : ℕ}
 (p q r s : x ≡ x)  where
 ...
 co base = x
 co (loop zero i) = p i
 co (loop (suc zero) i) = q i
 co (loop (suc (suc zero)) i) = r i
 co (loop (suc (suc (suc zero))) i) = s i
 co (loop (suc (suc (suc (suc _)))) i) = x

 p' q' r' s' : Path (Bouquet ℕ) base base
 p' = loop zero 
 q' = loop (suc zero)
 r' = loop (suc (suc zero))
 s' = loop (suc (suc (suc zero)))

 pLHS' = congS (congS co) (assoc p' q' (r' ∙ s') ∙ assoc (p' ∙ q') r' s')
 rLHS' = congS (congS co) (cong (p' ∙_) (assoc q' r' s') ∙∙ assoc p' (q' ∙ r') s' ∙∙ cong (_∙ s') (assoc p' q' r'))


 penta' : pLHS' ≡ rLHS'
 penta' = congS (congS (congS co)) (isSetΩBouquet discreteℕ _ _
   ((assoc p' q' (r' ∙ s') ∙ assoc (p' ∙ q') r' s'))
   ((cong (p' ∙_) (assoc q' r' s') ∙∙ assoc p' (q' ∙ r') s' ∙∙ cong (_∙ s') (assoc p' q' r'))))

 penta : pLHS ≡ rLHS
 penta i j k = {!penta' i j k!}
