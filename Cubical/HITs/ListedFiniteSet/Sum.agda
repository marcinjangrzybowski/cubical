{-# OPTIONS --safe  #-} 
module Cubical.HITs.ListedFiniteSet.Sum where
...
module _ {ℓ'} {A' : Type ℓ'} {A : Type ℓ} (setA : isSet A) where
  ...
  fold⊎H : RRec {A = A'} (isGroupoidHSet {ℓ = ℓ})
  RRec.[]* fold⊎H = ⊥.⊥* , isProp→isSet isProp⊥*
  RRec._∷*_ fold⊎H = λ x b → (A ⊎ fst b) , ⊎.isSet⊎ setA (snd b)
  RRec.comm* fold⊎H = λ x y b → hSet≡ (ua (swap0and1⊎ {A = A} {B = A} {C = fst (b)}))
  RRec.comm-inv* fold⊎H = (λ x y b → ΣSquareSet (λ _ → isProp→isSet (isPropIsSet))
    (swap0and1⊎fInvolSq A A (fst b)))
  RRec.hexDiag* fold⊎H = (λ x y z b → hSet≡
     (ua (swap0and2⊎ {A = A} {B = A} {C = A} {D = fst (b)})))
  RRec.hexU* fold⊎H = (λ x y z b → ΣSquareSet (λ _ → isProp→isSet (isPropIsSet))
         (∙≡∙→square _ _ _ _ (isInjectiveTransport
              ((funExt (⊎.elim (λ _ → refl) (⊎.elim (λ _ → refl)
             (⊎.elim (λ _ → refl) λ _ → refl))))))))
  RRec.hexL* fold⊎H = (λ x y z b → ΣSquareSet (λ _ → isProp→isSet (isPropIsSet))
         (∙≡∙→square _ _ _ _ (isInjectiveTransport
              ((funExt (⊎.elim (λ _ → refl) (⊎.elim (λ _ → refl)
             (⊎.elim (λ _ → refl) λ _ → refl))))))))

  fold⊎ : FMSet2 A' → Type ℓ
  fold⊎ = fst ∘ RRec.f fold⊎H
