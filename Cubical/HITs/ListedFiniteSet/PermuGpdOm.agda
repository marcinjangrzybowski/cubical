...
module Cubical.HITs.ListedFiniteSet.PermuGpdOm where
...
isSetFin {n} = isSetΣ isSetℕ λ k → isProp→isSet (OR.isProp≤ {suc k} {n})
...
module Permu0 where
  PermuHR : RElim {A = Unit} (λ _ → hSet ℓ-zero)
  RElim.[]* PermuHR = ? 
  (PermuHR RElim.∷* x) {xs} x₁ = ?
  RElim.comm* PermuHR x y b = ?
     -- TypeOfHLevel≡ 2 (ua (P.permute≃ _ zero tt))
  RElim.comm-inv* PermuHR x y {xs} b = ?
    -- ΣSquareSet (λ _ → isProp→isSet (isPropIsSet))
    --    ((involPathSym (P.permuteInvo _ zero tt))) 
  RElim.hexDiag* PermuHR = {!!}
  RElim.hexU* PermuHR = {!!}
  RElim.hexL* PermuHR = {!!}
  RElim.trunc* PermuHR _ = isGroupoidHSet



-- RremoveTT : RElim (λ z → Fin₂ z → Perm₂)
-- RElim.[]* RremoveTT () 
-- (RremoveTT RElim.∷* x) {xs} x₁ nothing = xs
-- (RremoveTT RElim.∷* x) {xs} x₁ (just x₂) = tt ∷2 x₁ x₂
-- RElim.comm* RremoveTT x y {xs} b = ua→ ww
--   where
--     ww : {!!}
--     ww = {!!}
-- RElim.comm-inv* RremoveTT = {!!}
-- RElim.hexDiag* RremoveTT = {!!}
-- RElim.hexU* RremoveTT = {!!}
-- RElim.hexL* RremoveTT = {!!}
-- RElim.trunc* RremoveTT = {!!}

-- removeTT : (xs : Perm₂) → Fin₂ xs → Perm₂
-- removeTT = RElim.f RremoveTT

-- nothingE : ((nPerm₂ (suc (len2 xs))) ≡ tt ∷2 xs
-- nothingE = ?

-- inj-tt∷ : (xs : Perm₂) → (nPerm₂ (suc (len2 xs))) ≡ tt ∷2 xs →
--         nPerm₂ (len2 xs) ≡ xs
-- inj-tt∷ xs p = {!!}
 -- where
 --   zz : Path Perm₂ (nPerm₂ (len2 xs))
 --         {!removeTT (p i1) (coe0→i (λ i → Fin₂ (p i)) i1 nothing)!}
 --   zz i = removeTT (p i) (coe0→i (λ i → Fin₂ (p i)) i nothing)

-- RElimSet'.f w
--   where
--     w : RElimSet'
--           (λ z → nPerm₂ (suc (len2 z)) ≡ tt ∷2 z → nPerm₂ (len2 z) ≡ z)
--     RElimSet'.[]* w = λ _ → refl
--     (w RElimSet'.∷* x) {xs} X p =
--       cong (_ ∷2_) (X {!!})
--     RElimSet'.comm* w = {!!}
...
-- module Permu0 where
--   PermuHR : RElim {A = Unit} (λ _ → hSet ℓ-zero)
--   RElim.[]* PermuHR = P.Perm zero , P.trunc 
--   (PermuHR RElim.∷* x) {xs} x₁ = P.Perm (suc (len2 xs)) , P.trunc
--   RElim.comm* PermuHR x y b = TypeOfHLevel≡ 2 (ua (P.permute≃ _ zero tt))
--   RElim.comm-inv* PermuHR x y {xs} b =
--     ΣSquareSet (λ _ → isProp→isSet (isPropIsSet))
--        ((involPathSym (P.permuteInvo _ zero tt))) 
--   RElim.hexDiag* PermuHR = {!!}
...
--   PermuH : Perm₂ → hSet ℓ-zero
--   PermuH = RElim.f PermuHR
...
--   --  where
--   --    w : RElimSet' (λ z → Permu z)
--   --    RElimSet'.[]* w = P.ε
--   --    (w RElimSet'.∷* x) x₁ = P.ε
--   --    RElimSet'.comm* w x y {xs} b =
--   --      RElimProp'.f ww xs
--   --      where
--   --        ww : RElimProp' λ xs → PathP (λ i → Permu (comm x y xs i))
--   --           P.ε
--   --           P.ε
--   --        RElimProp'.[]* ww = ua-gluePath _ (P.invo _ _)
--   --        (ww RElimProp'.∷* x) x₁ = ua-gluePath _ (P.invo _ _)
--   --        RElimProp'.trunc* ww _ = {!!}
--   --    RElimSet'.trunc* w z = snd (PermuH z)
...
--   nPermu : ∀ n → Permu (nPerm₂ n)
--   nPermu zero = P.ε
--   nPermu (suc n) = P.ε



--   module PermPath where

--     encode : ∀ n x → nPerm₂ n ≡ x → Permu x
--     encode n x p = subst Permu p (nPermu n)



--     decodeR : RElimSet' λ z → Permu z → nPerm₂ (len2 z) ≡ z
--     RElimSet'.[]* decodeR _ = refl
--     (decodeR RElimSet'.∷* x) {xs} p =
--        P.Rrec.f ww
--      where
--        ww : P.Rrec (nPerm₂ (len2 (x ∷2 xs)) ≡ x ∷2 xs)
--        P.Rrec.isSetA ww = trunc _ _
--        P.Rrec.εA ww = cong (_ ∷2_) (p (Permuε xs)) --
--          --
--        P.Rrec.∷A ww x x₁ = commK (suc (len2 xs)) x ∙ x₁
--        P.Rrec.invoA ww k x = {!!}
--        P.Rrec.braidA ww = {!!}
--        P.Rrec.commA ww = {!!}

--     RElimSet'.comm* decodeR x y {xs} b = 
--       ua→ (P.RelimProp.f ww)
--        where
--         ww : P.RelimProp
--                (λ z →
--                   PathP (λ i → nPerm₂ (len2 (comm x y xs i)) ≡ comm x y xs i)
--                   ((decodeR RElimSet'.∷* x) ((decodeR RElimSet'.∷* y) b) z)
--                   ((decodeR RElimSet'.∷* y) (((decodeR RElimSet'.∷* x) b))
--                    (P.permute (suc (len2 (y ∷2 xs))) zero tt z)))
--         P.RelimProp.isPropA ww _ = isPropSqP (λ _ _ → trunc) _ _ _ _
--         P.RelimProp.εA ww = {!!}
--         P.RelimProp.∷A ww k x = {!!}
--       -- where
--       --  ww :  RElimProp' λ xs → (b : Permu xs → nPerm₂ (len2 xs) ≡ xs) →
--       --        PathP
--       --        (λ i →
--       --           Permu (comm x y xs i) →
--       --           nPerm₂ (suc (suc (len2 xs))) ≡ comm x y xs i)
--       --        (λ e i → tt ∷2 tt ∷2 b (Permuε xs) i)
--       --        (λ e i → tt ∷2 tt ∷2 b (Permuε xs) i)
--       --  RElimProp'.[]* ww b = ua→ λ a →
--       --     {!!}
--       --  RElimProp'._∷*_ ww = {!!}
--       --  RElimProp'.trunc* ww = {!!}
--     RElimSet'.trunc* decodeR _ = {!!}
--     -- ∀ n → RElimSet' (λ z → Permu z → nPerm₂ n ≡ z)
--     -- RElimSet'.[]* (deccodeR zero) = λ _ → refl
--     -- (deccodeR zero RElimSet'.∷* x) x₁ x₂ = {!x₂!}
--     -- RElimSet'.comm* (deccodeR zero) = {!!}
--     -- RElimSet'.trunc* (deccodeR zero) = {!!}
--     -- deccodeR (suc n) = {!!}

--     decode : ∀ x → Permu x → nPerm₂ (len2 x) ≡ x
--     decode = RElimSet'.f (decodeR)
...
module Permu3 where
  -- PermuHR : RElim {A = Unit} (λ _ → hSet ℓ-zero)
  -- RElim.[]* PermuHR = Unit , isProp→isSet (isContr→isProp (isContrUnit))
  -- RElim._∷*_ PermuHR = {!!}
  -- RElim.comm* PermuHR = {!!}
  -- RElim.comm-inv* PermuHR = {!!}
  -- RElim.hexDiag* PermuHR = {!!}
  -- RElim.hexU* PermuHR = {!!}
  -- RElim.hexL* PermuHR = {!!}
  -- RElim.trunc* PermuHR = {!!}
  -- -- RElim.[]* PermuHR = P.Perm zero , P.trunc 
  -- -- (PermuHR RElim.∷* x) {xs} x₁ = P.Perm (suc (len2 xs)) , P.trunc
  -- -- RElim.comm* PermuHR x y b = TypeOfHLevel≡ 2 (ua (P.∷≃ _ (zero , tt)))
  -- -- RElim.comm-inv* PermuHR x y {xs} b =
  -- --   ΣSquareSet (λ _ → isProp→isSet (isPropIsSet))
  -- --      ((involPathSym (P.invo (zero , tt)))) 
  -- -- RElim.hexDiag* PermuHR = {!!}
  -- -- RElim.hexU* PermuHR = {!!}
  -- -- RElim.hexL* PermuHR = {!!}
  -- -- RElim.trunc* PermuHR _ = isGroupoidHSet
  ...
  Permu : Perm₂ → Type
  Permu xs = Fin₂ (nPerm₂ (len2 xs)) ≃ Fin₂ xs

  nPermu : ∀ n → Permu (nPerm₂ n)
  nPermu zero = idEquiv _
  nPermu (suc n) = congMaybeEquiv (nPermu n)

  encode : ∀ n x → nPerm₂ n ≡ x → Permu x
  encode n x p = subst Permu p (nPermu n)

  decodeR : RElimSet' (λ z → Permu z → nPerm₂ (len2 z) ≡ z)
  RElimSet'.[]* decodeR = λ _ → refl
  (decodeR RElimSet'.∷* x) {xs} X = {!!}
  RElimSet'.comm* decodeR = {!!}
  RElimSet'.trunc* decodeR = {!!}
...
--   sucPermu : ∀ xs → Permu xs → Permu (tt ∷2 xs) 
--   sucPermu = RElimSet'.f ww
--     where
--       ww : RElimSet' (λ z → Permu z → Permu (tt ∷2 z))
--       RElimSet'.[]* ww _ = P.ε
--       (ww RElimSet'.∷* _) _ x₂ = {!!}
--       RElimSet'.comm* ww x y {xs} b = {!!}
--        --  ua→ (P.RelimProp.f zz)
--        -- where
--        --   zz : P.RelimProp
--        --          (λ z →
--        --             P.Rrec.f (P.sucPermR (suc (suc (len2 xs)))) z ≡
--        --             (1 , tt) P.∷ P.Rrec.f (P.sucPermR (suc (suc (len2 xs)))) z)
--        --   P.RelimProp.isPropA zz _ = P.trunc _ _
--        --   P.RelimProp.εA zz = {!!}
--        --   P.RelimProp.∷A zz = {!!}
--       RElimSet'.trunc* ww = {!!}
...
--        --      P.ε
--        --      P.ε
--        --   RElimProp'.[]* ww = ua-gluePath _ {!!}
--        --   (ww RElimProp'.∷* x) x₁ = ua-gluePath _ {!!}
--        --   RElimProp'.trunc* ww _ = {!!}
--      RElimSet'.trunc* w z = snd (PermuH z)
...
-- -- toPGcomm n = EMfunct (P.sucPerm n , P.sucPermGH n) 
...
-- --     (P.sucPerm _  (P.sucPerm _  (P.sucPerm _ xs))) ◁
...
-- --       (λ z →
-- --          uncurry fromPG' (suc n , EMfunct (P.sucPerm n , P.sucPermGH n) z) ≡
...
-- --      EMfunct (P.sucPerm n , P.sucPermGH n) e) ≡
...
-- -- -- encodeFin₂ : ∀ n x → nPerm₂ n ≡ x → (Fin'' n ≃ Fin₂ x)
-- -- -- encodeFin₂ n x p = {!!}

-- -- -- decodeFin₂ : ∀ n x → (Fin'' n ≃ Fin₂ x) → nPerm₂ n ≡ x
-- -- -- decodeFin₂ zero = {!!}
-- -- -- decodeFin₂ (suc n) = {!!}
...
-- --         EMelimSet.bPathP ww = P.RelimProp.f {!!}
-- --           -- where
-- --           --   www : P.RelimProp
-- --           --           (λ z →
-- --           --              PathP
...
-- --           --        }) (emloop (iso.inv (pathToEquiv (λ i₂ → Fin₂ (x i₂)))) i₁)
-- --           --   P.RelimProp.∷A www k {xs} X i x i₁ =
