...
module Cubical.HITs.ListedFiniteSet.PermuGpdGlue where
...
hexUsqPa : (b : hSet ℓ-zero) →
  PathP (λ j → (a : Maybe (ua (swap0and1M b) j)) →
      Maybe (Maybe (Maybe (fst b))))
    (Mb.map-Maybe (fst (swap0and1M b)))
    (idfun _)
hexUsqPa b j nothing = nothing
hexUsqPa b j (just x) = just (ua-ungluePathExt (swap0and1M b) j x)

-- hexUsq : (b : hSet ℓ-zero) →
--            Square
--              (cong Maybe (ua (swap0and1M b)))
--              (ua (swap0and2M b))
--              (ua (swap0and1M (Maybe (fst b) , isOfHLevelMaybe zero (snd b))))
--              (ua (swap0and1M (Maybe (fst b) , isOfHLevelMaybe zero (snd b))))
-- hexUsq b i j = {!!}
--    -- Glue (Maybe (Maybe (Maybe (fst b))))
--    --     λ {
--    --      (i = i0) → (cong Maybe (ua (swap0and1M b))) j ,
--    --        {!!} , {!!}      
--    --    ; (j = i0) →
--    --      (ua (swap0and1M (Maybe (fst b) , isOfHLevelMaybe zero (snd b)))) i
--    --       , {!!} , {!!}
--    --    ; (i = i1)(j = i1) → _ , idEquiv (Maybe (Maybe (Maybe (fst b))))
--    --        -- {!!} , {!!} 
--    --    -- ; (j = i1) →
--    --    --     (ua (swap0and1M (Maybe (fst b) , isOfHLevelMaybe zero (snd b)))) i
--    --    --    , ua-ungluePathExt
...
fst ((Fin₂R RRec.∷* x) b) = Maybe (fst b) 
snd ((Fin₂R RRec.∷* x) b) = isOfHLevelMaybe zero (snd b)
fst (RRec.comm* Fin₂R x y b i) = ua (swap0and1M b) i
snd (RRec.comm* Fin₂R x y b i) =
  isProp→PathP (λ i → isPropIsSet {A = (ua (swap0and1M b) i)})
      (isOfHLevelMaybe zero (isOfHLevelMaybe zero (snd b)))
      (isOfHLevelMaybe zero (isOfHLevelMaybe zero (snd b))) i
-- TypeOfHLevel≡ 2 (ua (swap0and1M b))
RRec.comm-inv* Fin₂R x y b =
  ΣSquareSet (λ _ → isProp→isSet (isPropIsSet))
    (invoFinR b)
fst (RRec.hexDiag* Fin₂R x y z b i) = ua (swap0and2M b) i
snd (RRec.hexDiag* Fin₂R x y z b i) =
     isProp→PathP (λ i → isPropIsSet {A = (ua (swap0and2M b) i)})
      (isOfHLevelMaybe zero (isOfHLevelMaybe zero (isOfHLevelMaybe zero (snd b))))
      (isOfHLevelMaybe zero (isOfHLevelMaybe zero (isOfHLevelMaybe zero (snd b)))) i
