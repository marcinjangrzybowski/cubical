...
module Cubical.HITs.FreeGroup.NormalForm.Base3 where
...
module NormalForm (A : Type ℓ) where
 ...
 ¬Red[len≡1] .((x₃ ∷ xs) ++ x₄ ∷ ys) x (_·_ {x₃ ∷ xs} {x₄ ∷ ys} x₁ x₂) =
    snotz ((sym (+-suc (length xs) _) ∙ sym (length++ xs (x₄ ∷ ys))) ∙ injSuc x) 
 ...
 reduceRed .((x ∷ ([] ∷ʳ not₁ x)) ++ x₄ ∷ ys) (inl x₃) (_·_ {.(x ∷ ([] ∷ʳ not₁ x))} {x₄ ∷ ys} (cj x [] x₂) x₁) = {!x₁!}
 reduceRed .((x ∷ ((x₅ ∷ xs) ∷ʳ not₁ x)) ++ x₄ ∷ ys) (inl x₃) (_·_ {.(x ∷ ((x₅ ∷ xs) ∷ʳ not₁ x))} {x₄ ∷ ys} (cj x (x₅ ∷ xs) x₂) x₁) = {!!}
 reduceRed .((x ∷ (xs ∷ʳ not₁ x)) ++ ys) (inr x₃) (_·_ {.(x ∷ (xs ∷ʳ not₁ x))} {ys} (cj x xs x₂) x₁) = {!!}
 
 -- reduceRed : ∀ xs hr → Red xs → Red (reduce xs hr)
 -- reduceRed .((x ∷ []) ++ ys) (inl u) (_·_ {x ∷ []} {ys} _ _) =
 --   {!!}
 -- reduceRed .((x₂ ∷ x₄ ∷ xs) ++ ys) (inl x₃) (_·_ {x₂ ∷ x₄ ∷ xs} {ys} x x₁) = {!!}
 -- reduceRed .((x₂ ∷ xs) ++ ys) (inr x₃) (_·_ {x₂ ∷ xs} {ys} x x₁) = {!!}
