...
module Cubical.HITs.FreeGroup.NormalForm.Discrete where
...
module _ {A : Type ℓ} (_≟_ : Discrete A) where
 ...
 GroupStr[N] : GroupStr [N]
 GroupStr.1g GroupStr[N] = [] , _
 GroupStr._·_ GroupStr[N] (x , n) (x' , n') = {!!}
 GroupStr.inv GroupStr[N] (x , n) = invLi x , fst IsNormalisedInvLi n
 GroupStr.isGroup GroupStr[N] = {!!}
 
--  η' : (Bool × A) → Σ _ (fst ∘ IsNormalised) → Σ _ (fst ∘ IsNormalised) 
--  η' x ([] , _) = ([ x ] , IsNormalised[x] x)
--  η' x (x' ∷ xs , n) = fst np ++ xs , snd np
--   where np = nPair x x' n (IsRedex? x x')

--  η'-sec : ∀ x → section (η' x) (η' (not₁ x))
--  η'-sec x ([] , snd₁) = {!!}
--  η'-sec x (x₁ ∷ xs , snd₁) = {!!}
...
-- --  -- fst (nPair x x' (IsRedex? x x')) ++ xs
-- --  -- snd (η' x (x' ∷ xs , p)) = {!⊎.rec ? p!}
...
-- -- --     let (((rl , rlR) , (rr , rrR)) , p) =
-- -- --            RedWillReduceView (not b) a xs x refl
...
-- -- --   NF : ⟨ freeGroupGroup A ⟩ → Type ℓ
-- -- --   NF g = Σ _ λ l → (g ≡ fromList l) × ⟨ IsNormalised l ⟩

-- -- --   module _ (isSetA : isSet A) where

-- -- --    isPropNF : ∀ g → isProp (NF g)
-- -- --    isPropNF g (l , p , n) (l' , p' , n') =
-- -- --      let q = isoInvInjective (fst GroupIso-FG-L/↘↙)
-- -- --                [ l ]/ [ l' ]/ (sym p ∙ p')
-- -- --          z = nf-uR l' l {!n'!} n
-- -- --             {!PT.map (red++comm _ _ ∘' ≡ε) (fun (≡→red _ _) q)!}
-- -- --      in {!!}
...
-- -- -- -- -- mapRed : ∀ {ℓ ℓ'} {A : Type ℓ} {B : Type ℓ'} →
-- -- -- -- --    (f : A → B) → ∀ xs → Red A xs  → Red B (Li.map (map-snd f) xs) 
-- -- -- -- -- mapRed f .[] red[] = red[]
-- -- -- -- -- mapRed f .(x ∷ (xs ∷ʳ not₁ _ x)) (cj x xs x₁) =
-- -- -- -- --   subst (Red _) (cong ((fst x , f (snd x)) ∷_)
-- -- -- -- --     (map++ _ xs [ not₁ _ x ])) (cj (map-snd f x) _ (mapRed f xs x₁))
-- -- -- -- -- mapRed f .(xs ++ ys) ((xs · ys) x x₁) =
-- -- -- -- --   subst (Red _) (map++ _ xs ys) (_r·_ _ (mapRed f xs x) (mapRed f ys x₁))

-- -- -- -- -- -- freeGroupFunctor' : Functor (SET ℓ) (GroupCategory {ℓ})
-- -- -- -- -- -- F-ob freeGroupFunctor' = List/↘↙group ∘ fst
-- -- -- -- -- -- fst (F-hom freeGroupFunctor' f) =
-- -- -- -- -- --  SQ.rec squash/ ([_]/ ∘ Li.map (map-snd f))
-- -- -- -- -- --   λ a b p → eq/ _ _ [ subst (Red _) (sym (map++ _ a (invLi _ b)) ∙ {!!})
-- -- -- -- -- --     (mapRed f _ (≡ε p))  ]≡ε
-- -- -- -- -- -- IsGroupHom.pres· (snd (F-hom freeGroupFunctor' f)) = {!!}
-- -- -- -- -- -- IsGroupHom.pres1 (snd (F-hom freeGroupFunctor' f)) = refl
-- -- -- -- -- -- IsGroupHom.presinv (snd (F-hom freeGroupFunctor' f)) =
-- -- -- -- -- --   SQ.elimProp (λ _ → squash/ _ _)
-- -- -- -- -- --     λ a → cong [_]/ (rev-map-comm _ (Li.map (map-fst not) a) ∙
-- -- -- -- -- --        {!!})
-- -- -- -- -- -- F-id freeGroupFunctor' = Σ≡Prop (λ _ → isPropIsGroupHom _ _)
-- -- -- -- -- --   (funExt (SQ.elimProp (λ _ → squash/ _ _)
-- -- -- -- -- --     λ a → cong [_]/ (map-id a)))
-- -- -- -- -- -- F-seq freeGroupFunctor' f g = Σ≡Prop (λ _ → isPropIsGroupHom _ _)
-- -- -- -- -- --   (funExt (SQ.elimProp (λ _ → squash/ _ _)
-- -- -- -- -- --    λ a → cong [_]/ (sym (map-∘ _ _ _))))

-- -- -- -- -- -- open UnitCounit

-- -- -- -- -- -- aaa : freeGroupFunctor' ⊣ Forget {ℓ}
-- -- -- -- -- -- NatTrans.N-ob (_⊣_.η aaa) X x = [ [ true , x ] ]/
-- -- -- -- -- -- NatTrans.N-hom (_⊣_.η aaa) _ = refl
-- -- -- -- -- -- fst (NatTrans.N-ob (_⊣_.ε aaa) X) =
-- -- -- -- -- --   SQ.rec (GroupStr.is-set (snd X)) {!!} {!!}
-- -- -- -- -- -- snd (NatTrans.N-ob (_⊣_.ε aaa) X) = {!!}
-- -- -- -- -- -- NatTrans.N-hom (_⊣_.ε aaa) = {!!}
-- -- -- -- -- -- _⊣_.Δ₁ aaa = {!!}
-- -- -- -- -- -- _⊣_.Δ₂ aaa = {!ref!}
