...
--  [_ʻ_ʻ_] : ∀ x y → x ≡ y → pth x y 

-- qq : ∀ {ℓ} {A : Type ℓ} {x y : A} → pth x y → x ≡ y
-- qq [ _ ʻ _ ʻ x ] = x
...
module _ (tG : R.Term) where
 tryG : ℕ → R.Term → R.TC (FGT.GroupTerm R.Term)

 try1g : R.Term → R.TC (FGT.GroupTerm R.Term)
 try1g t = do
       R.noConstraints
        (R.checkType (R.con (quote is1g) []) (R.def (quote IsTrm) 
           ((varg tG) ∷ [ varg t ])))

       R.returnTC FGT.ε

 tryOp : ℕ → R.Term → R.TC (FGT.GroupTerm R.Term)
 ...
 tryInv : ℕ → R.Term → R.TC (FGT.GroupTerm R.Term)
 ...
 atom : R.Term → R.TC (FGT.GroupTerm R.Term)
 atom x = R.returnTC (FGT.η x)
...
macro
 infGT : R.Term → R.Term → R.TC Unit
 infGT tG hole = do
  (_ , (t0 , t1)) ←  (inferEnds tG hole)
  -- R.typeError {A = Unit} [ R.termErr t0 ]
  t0N ← R.reduce t0
  t1N ← R.reduce t1
  r0 ← tryG tG 100 t0N
  r1 ← tryG tG 100 t1N
  ul ← uniqeAtoms (FGT.atoms _ r0 ++ FGT.atoms _ r1)
 ...
 infix 2 _gs⟨_⟩_
 _gs⟨_⟩_ : R.Term → R.Term → R.Term → R.Term → R.TC Unit
 _gs⟨_⟩_ t0 tG t1 hole = do
  -- R.typeError [ R.termErr pTy ]
  t0N ← R.reduce t0
  t1N ← R.reduce t1
  r0 ← tryG tG 100 t0N
  r1 ← tryG tG 100 t1N
...
module TestGeneric (G : Group ℓ) (A : Type ℓ) (f : A → ⟨ G ⟩)
 ...
 zzz' =
  ...
  --∙ {!!}
   -- qq [ inv (g1 · g2 · f a1 · f a2)
   --    ʻ inv (f a2) · inv (f a1) · inv g2 · inv g1 ʻ
   --      GR.infGT G ]
    infGT G --GR.infGT G
