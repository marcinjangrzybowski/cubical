...
module Cubical.HITs.FreeGroup.NormalFormIndexed2 where
...
module NormalForm {A : Type ℓ} where
 ...
 data Redex : List (Bool × A) → Type ℓ where
  here : ∀ x xs → Redex (x ∷ not₁ x ∷ xs )
  there : ∀ {x xs} → Redex xs → Redex (x ∷ xs)

 ¬Redex[] : Redex [] → ⊥
 ¬Redex[] ()

 redStep : ∀ xs → Redex xs → List (Bool × A)
 redStep .(x ∷ not₁ x ∷ xs) (here x xs) = xs
 redStep .(x ∷ _) (there {x} y) = x ∷ redStep _ y 

 infixr 5 _∷_ 

 data Redexes : List (Bool × A) → Type ℓ where
  [] : ∀ {xs} → Redexes xs
  _∷_ : ∀ {xs} rx → Redexes (redStep xs rx) → Redexes xs
 
 𝒓 : ∀ xs → Redexes xs → List (Bool × A) 
 𝒓 xs [] = xs
 𝒓 xs (rx ∷ x) = 𝒓 _ x

 data Redexes' : List (Bool × A) → Type ℓ where
  [] : ∀ {xs} → Redexes' xs
  _∷_ : ∀ {xs} rx → Redexes' xs → Redexes' (redStep xs rx)

 _↓_ : List (Bool × A) → List (Bool × A) → Type ℓ 
 xs ↓ ys = Σ _ λ rxs → 𝒓 xs rxs ≡ ys

 -- cj↓ : ∀ x xs ys → (x ∷ (xs ∷ʳ not₁ x) ) ↓ xs
 -- cj↓ = {!!}
 ...
 ↓trans : isTrans _↓_
 ↓trans _ _ c ([] , p) = subst (_↓ c) (sym p)
 ↓trans _ _ _ (rx ∷ rdxs , p) u =
  let (u , v) = (↓trans _ _ _ (rdxs , p) u)
  in rx ∷ u , v

 Normal : List (Bool × A) → Type ℓ 
 Normal xs = ¬ Redex xs

 NormalRedexes : ∀ xs → Normal xs → (rdxs : Redexes xs) → rdxs ≡ [] 
 NormalRedexes xs x [] = refl
 NormalRedexes xs x (rx ∷ rdxs) = ⊥.rec (x rx)

 𝒓Normal : ∀ xs → Normal xs → ∀ rdxs → 𝒓 xs rdxs ≡ xs  
 𝒓Normal xs x [] = refl
 𝒓Normal xs x (rx ∷ rdxs) = ⊥.rec (x rx)

 ↓++ : ∀ xs ys xs' ys' → xs ↓ ys → xs' ↓ ys' → (xs ++ xs') ↓ (ys ++ ys') 
 ↓++ xs ys xs' ys' (r , p) (r' , p') = {!!}
...
--  reduceRed .(x ∷ ([] ∷ʳ not₁ x)) (inl x₂) (cj x [] x₁) = red[]
--  reduceRed .(x ∷ ([] ∷ʳ not₁ x)) (inr (inl ())) (cj x [] x₁)
--  reduceRed .(x ∷ ([] ∷ʳ not₁ x)) (inr (inr ())) (cj x [] x₁)
--  reduceRed .(x ∷ ((x₂ ∷ xs) ∷ʳ not₁ x)) (inl x₃) (cj x (x₂ ∷ xs) x₁) =
--    let ((xs' , ys') , (xs'R , ys'R , p)) = red∷ x₂ xs x₁
--        z' = (xs' · (x ∷ (ys' ∷ʳ not₁ x))) xs'R (cj x ys' ys'R)
--    in subst Red ((cong (λ x' → xs' ++ (x' ∷ (ys' ∷ʳ not₁ x))) (ΣPathP x₃) ∙
--         sym (++-assoc xs' ([ not₁ x₂ ] ++ ys') ([ not₁ x ]))) ∙
--         cong (_∷ʳ (not (fst x) , snd x)) (sym p)) z'
--  reduceRed .(x ∷ ((x₂ ∷ xs) ∷ʳ not₁ x)) (inr x₃) (cj x (x₂ ∷ xs) x₁) =
--    {!!}
...
--  -- data Red : (List (Bool × A)) → Type ℓ where
--  --  red[] : Red []
--  --  cj : ∀ x y → (IsRedex x y) → ∀ xs → Red xs →  Red (x ∷ (xs ∷ʳ y) )
--  --  _·_ : ∀ xs ys → Red xs → Red ys → Red (xs ++ ys)
