...
module Cubical.HITs.NCube.Modality where
...
CType : (ℓ : Level) → ℕ → Typeω 
CType ℓ n = C→ (Type ℓ) n

c→ : ∀ {ℓ} → ∀ n → CType ℓ n → Typeω
c→ zero A = Partial i1 (A 1=1)
c→ (suc n) A = ∀ i → c→ n (A i)
...
isContrCube :  ∀ {n} → isContr (NCube n)
isContrCube {n} = corner0 , corner0-≡
...
c→-app : ∀ {ℓ} → ∀ n → ∀ {A  : CType ℓ n} → c→ n A → (cu : NCube n) → C→-app A cu
c→-app {ℓ} zero {A} x cu = x  1=1
c→-app {ℓ} (suc n) {A} x cu = IappP (λ i → c→-app n (x i) (tail cu)) (head cu) 

C→elim : ∀ {ℓ} → ∀ n → ∀ {A  : Type ℓ} → (NCube n → A) → C→ A n 
C→elim (zero) x _ = x []
C→elim (suc n) x i = C→elim n (x ∘ (inside i ∷_))

c→elim : ∀ {ℓ} → ∀ n → ∀ {A  : CType ℓ n} → ((cu : NCube n) → C→-app A cu) → c→ n A 
c→elim zero x _ = x []
c→elim (suc n) x i = c→elim n ((x ∘ (inside i ∷_)))


C→map : ∀ {ℓ ℓ'} → ∀ n → ∀ {A  : Type ℓ} → ∀ {B  : Type ℓ'} → (A → B) → C→ A n → C→ B n
C→map zero f x _ = f (x 1=1)
C→map (suc n) f x i = C→map n f (x i)


-- isCU : ℕ → ∀ ℓ →  Type ℓ → Type ℓ
-- isCU n ℓ A = Σ (A → (NCube n → A)) isEquiv



CU : ∀ ℓ → ℕ → Modality ℓ
Modality.isModal (CU ℓ n) A = NCube n → (a : A) → singl a   
Modality.isModalIsProp (CU ℓ n) =
  isPropΠ λ _ → isPropΠ λ x → isContr→isProp ((x , refl) , λ y i → contrSingl (snd y) i)  
Modality.◯ (CU ℓ n) A = NCube n → A
Modality.◯-isModal (CU ℓ n) {A} x a = const (a x) , λ i x₁ → a (isPropCube x₁ x i )
Modality.η (CU ℓ n) a = const a
Modality.◯-elim (CU ℓ n) {B = B} B-modal x y =
  let z = fst (B-modal (Modality.η (CU ℓ n) (y corner0)) corner0 (x (y corner0)))
  in subst B (λ i x₁ → y (corner0-≡ x₁ i)) z

Modality.◯-elim-β (CU ℓ n) {B = B} B-modal f a = 
  (substRefl {B = B} {x = (λ x₁ → a)} (fst (B-modal (λ _ → a) corner0 (f a))))
   ∙ sym (snd (B-modal (λ _ → a) corner0 (f a)))
Modality.◯-=-isModal (CU ℓ n) x y x₁ a = a , refl

open Modality


CTy : ∀ ℓ → ℕ → Type (ℓ-suc ℓ)
CTy ℓ n = ◯-Types (CU ℓ n)

CΠ : ∀ ℓ → (n : ℕ) → {A B : Set ℓ} → ((a : A) → isModal (CU ℓ n) B) → isModal (CU ℓ n) (∀ x → B)
CΠ ℓ n = Π-isModal (CU ℓ n) 

-- c→map : ∀ {ℓ ℓ'} → ∀ n → ∀ {A  : CType ℓ n} → ∀ {B  : CType ℓ' n}
--         → (A → B) → c→ n A → c→ n B
-- c→map = ?
