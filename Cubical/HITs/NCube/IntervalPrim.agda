...
InsideOfBω (suc n) {A = A} x = PathP (λ i → ∀ c → ((CType-ev (suc n) A) i∷ i) c)
...
record Pair {ℓ} (A : Bool → Type ℓ) : Type ℓ where
  constructor _,p_
  field
    side0 : A false
    side1 : A true

side : ∀ {ℓ} {A : Bool → Type ℓ} → ∀ b → Pair {ℓ} (A) → A b
side false (a0 ,p a1) = a0
side true (a0 ,p a1) = a1

PaPair : ∀ {ℓ} (A : Interval' → Type ℓ) → Pair (A ∘ end) → Type ℓ
PaPair A x = PathP (λ i → A (inside i)) (side false x) (side true x) 


Boundary : ∀ {ℓ} → ∀ {n} → (NCube n → Type ℓ) → Type ℓ

InsideOf : ∀ {ℓ} → ∀ {n} → {A : NCube n → Type ℓ} → Boundary A → Type ℓ

NCube' : ∀ {ℓ} → ∀ {n} → (NCube n → Type ℓ) → Type ℓ
NCube' x = Σ (Boundary x) InsideOf

_f∷_ : ∀ {ℓ ℓ'} → {A : Type ℓ} → {B : Type ℓ'}
       → ∀ {n}
       → (Vec A (suc n) → B) → A → (Vec A n → B)
A f∷ a = A ∘ (a ∷_ )

Boundary {n = zero} _ = Lift Unit
Boundary {n = suc zero} A = Pair λ b → A ((end b) ∷ []) 
Boundary {n = suc (suc n)} A =
  Σ[ ends ∈  Pair (λ b → NCube' (A b∷ b)) ]
  PathP (λ i → Boundary (A i∷ i) )
    (fst (side false ends))
    (fst (side true ends))

InsideOf {n = zero} {A} x = A []
InsideOf {n = suc zero} {A} x = PaPair (A ∘ ( _∷ [])) x
InsideOf {n = suc (suc n)} {A} x = PaPair (NCube' ∘ (A f∷_)) (fst x)
...
-- →ⁿ : ∀ {ℓ} → ∀ {ℓ'} → ∀ n → Sig ℓ n → Type ℓ' → Type (ℓ-max ℓ ℓ')
-- →ⁿ {ℓ} {ℓ'} zero _ A = Lift {_} {ℓ} A
-- →ⁿ (suc zero) x A = x → A
-- →ⁿ (suc (suc n)) x A = (a : fst x) → →ⁿ (suc n) ((snd x) a) A

-- Πⁿ : ∀ {ℓ} → ∀ {ℓ'} → ∀ n → (s : Sig ℓ n) → →ⁿ n s (Type ℓ') → Type (ℓ-max ℓ ℓ')
-- Πⁿ {ℓ} zero s x = Lift {_} {ℓ} (lower x)
-- Πⁿ (suc zero) s x = ∀ s → x s
-- Πⁿ (suc (suc n)) s x = (a : fst s) →  Πⁿ (suc n) ((snd s) a) (x a)


-- Sig∷ : ∀ {ℓ} → ∀ {ℓ'} → ∀ n → (s : Sig ℓ n) → →ⁿ n s (Type ℓ') → Sig (ℓ-max ℓ ℓ') (suc n) 
-- Sig∷ {ℓ} zero s x = Lift {_} {ℓ} (lower x)
-- Sig∷ {ℓ} {ℓ'} (suc zero) s x = (Lift {_} {ℓ'} s) , {!!}
-- Sig∷ (suc (suc n)) s x = {!!}

→-overSig : ∀ {ℓ} → ∀ n → Sig ℓ n → Type (ℓ-suc ℓ) → (Type (ℓ-suc ℓ))
→-overSig zero _ A = A
→-overSig (suc zero) x A = x → A
→-overSig (suc (suc n)) x A = (a : fst x) → →-overSig (suc n) ((snd x) a) A


Π-overSig : ∀ {ℓ} → ∀ n → (s : Sig ℓ n) → →-overSig n s (Type ℓ) → Type ℓ
Π-overSig {ℓ} zero s x = x
Π-overSig (suc zero) s x = ∀ s → x s
Π-overSig (suc (suc n)) s x = (a : fst s) →  Π-overSig (suc n) ((snd s) a) (x a)


Sig∷ : ∀ {ℓ} → ∀ n → (s : Sig ℓ n) → →-overSig n s (Type ℓ) → Sig ℓ (suc n) 
Sig∷ {ℓ} zero s x = x
Sig∷ {ℓ} (suc zero) s x = s , x
Sig∷ (suc (suc n)) s x = (fst s) , (λ x₁ → Sig∷ (suc n) ((snd s) x₁) (x x₁))


trim-sig :  ∀ {ℓ} → ∀ n → (s : Sig ℓ n) → Sig ℓ (predℕ n)
trim-sig zero s = s
trim-sig (suc zero) s = lift _
trim-sig (suc (suc zero)) s = fst s
trim-sig (suc (suc (suc n))) s = fst s , λ x → trim-sig (suc (suc n)) (snd s x)

Π-sig : ∀ {ℓ} → ∀ n → (s : Sig ℓ n) → Type ℓ
Π-sig zero s = Lift Unit
Π-sig (suc zero) s = s
Π-sig (suc (suc n)) s = (a : fst s) → Π-sig (suc n) (snd s a)

-- -- -- (a : fst x) → Πⁿ ℓ (suc n) (snd x a)

Σ-sig : ∀ {ℓ} → ∀ n → Sig ℓ n → Type ℓ
Σ-sig 0 x = Lift Unit
Σ-sig 1 x = x
Σ-sig (suc (suc n)) x = Σ (fst x) (Σ-sig (suc n) ∘ snd x) 
...
-- 3^-lem : ∀ n → (3^ n) ≡ suc (predℕ (3^ n))
-- 3^-lem n = suc-predℕ _ (3^notz n)



combineSigs : ∀ {ℓ} → ∀ n → (I → Sig ℓ n) → Sig ℓ (n * 3)
combineSigs zero x = lift _
combineSigs (suc zero) x = (x i0) , (λ x₁ → (x i1) , (λ x₂ → PathP x x₁ x₂))
combineSigs (suc (suc n)) x =
               (fst (x i0)) ,
       (λ x₁ → (fst (x i1)) ,
       (λ x₂ → (PathP (λ i → fst (x i)) x₁ x₂) ,
         λ x₃ → combineSigs (suc n) λ i → snd (x i) (x₃ i) ))

BrdSig : ∀ {ℓ} → ∀ n → T[ CType ℓ n ] → Sig ℓ (3^ n)
BrdSig {ℓ} zero A = lowerω A
BrdSig {ℓ} (suc zero) A = combineSigs 1 λ x → lowerω (A x) 
BrdSig {ℓ} (suc (suc n)) A = combineSigs (3^ n * 3) (λ i → BrdSig {ℓ} (suc n) (A i))


Boundaryⁿ : ∀ {ℓ} → ∀ n → T[ CType ℓ n ] → Type ℓ
Boundaryⁿ n A = Σ-sig (predℕ (3^ n)) (trim-sig (3^ n) (BrdSig n A))

PathPⁿ : ∀ {ℓ} → ∀ n → T[ CType ℓ n ] → Type ℓ
PathPⁿ n A = Π-sig (3^ n) (BrdSig n A)

Cubical-Iso-PathPⁿ : ∀ {ℓ} → ∀ n → (A : T[ CType ℓ n ])
                      → Iso
                       (Π (CType-ev n A))
                       (Σ-sig (3^ n) (BrdSig {ℓ} n A))
Cubical-Iso-PathPⁿ zero A = iso (λ x → x []) (λ x x₁ → x) (λ b → refl)
                               λ a i x → a ((isPropCube [] x) i)

Iso.fun (Cubical-Iso-PathPⁿ (suc zero) A) x =
  (x (end false ∷ [])) , (((x (end true ∷ []))) , (λ i → x (inside i ∷ [])))
Iso.inv (Cubical-Iso-PathPⁿ (suc zero) A) x (end x₁ ∷ x₂) = snd (snd x) (Bool→I x₁)
Iso.inv (Cubical-Iso-PathPⁿ (suc zero) A) x (inside i ∷ x₂) = snd (snd x) i
Iso.rightInv (Cubical-Iso-PathPⁿ (suc zero) A) (fst₁ , fst₂ , snd₁) = refl
Iso.leftInv (Cubical-Iso-PathPⁿ (suc zero) A) a i (end false ∷ []) = a (end false ∷ [])
Iso.leftInv (Cubical-Iso-PathPⁿ (suc zero) A) a i (end true ∷ []) = a (end true ∷ [])
Iso.leftInv (Cubical-Iso-PathPⁿ (suc zero) A) a i (inside i₁ ∷ []) = a (inside i₁ ∷ [])

Cubical-Iso-PathPⁿ (suc (suc n)) A = {!!}

bbTest : PathPⁿ 4 ct[ 4 , (λ _ → ℕ) ]
bbTest = {!!}

-- BrdSig {ℓ} zero A = _
-- BrdSig {ℓ} (suc zero) A = (lowerω (A i0)) , (λ _ → lowerω (A i1))
-- BrdSig {ℓ} (suc (suc n)) A =
--   let z : I → Sig ℓ (predℕ (3^ (1 + n)))
--       z = λ i → BrdSig {ℓ} (suc n) (A i)

--       zz : I → Sig ℓ (suc (predℕ (3^ (1 + n))))
--       zz i = Sig∷ (predℕ (3^ n * 3)) (BrdSig (suc n) (A i)) (PathS (suc n) (A i))

--       zzz : Sig ℓ (3 + (predℕ (3^ n * 3) * 3))
--       zzz = combineSigs (suc (predℕ (3^ n * 3))) zz

--       zzzz : Sig ℓ (predℕ (3 + predℕ (3^ n * 3) * 3))
--       zzzz = {!!}

--       hh : 3 + (predℕ (3^ n * 3) * 3) ≡ (3^ n * 3 * 3)
--       hh = +-comm 3 (predℕ (3^ n * 3) * 3)
--             ∙ predℕ-+ (3^ n * 3) 3 (3^notz (suc n)) 

--       w : Sig ℓ (predℕ (3^ (2 + n)))
--       w = subst (Sig ℓ) (cong predℕ hh) zzzz

--   in w

-- PathS = {!!}

-- BrdSig ℓ (suc (suc n)) x =
--   subst (Sig ℓ) (cong predℕ (sym (*-comm (3^ (suc n)) 3))) {!!}



-- mkPiTy : ∀ {ℓ} → ∀ ℓ' → ∀ {n} → (A : Fin (suc n) → Type ℓ) → Type (ℓ-max ℓ (ℓ-suc ℓ'))
-- mkPiTy ℓ' {n = zero} A = A fzero → Type ℓ'
-- mkPiTy ℓ' {n = suc n} A = A fzero → mkPiTy ℓ' (A ∘ fsuc)


-- mkPi : ∀ {ℓ ℓ'} → ∀ {n} → (A : Fin (suc n) → Type ℓ) → (Π A → Type ℓ') → mkPiTy ℓ' A  
-- mkPi {n = zero} A a a0 = a (Fin1-elim a0)
-- mkPi {n = suc n} A x x₁ = mkPi {n = n} (A ∘ fsuc) h
--   where
--    h : Π (λ x₂ → A (fsuc x₂)) → Type _
--    h y = x hh
--      where
--        hh : Π A
--        hh (zero , snd₁) = subst A (toℕ-injective refl) x₁
--        hh (suc fst₁ , snd₁) = subst A (toℕ-injective refl) (y (fst₁ , pred-≤-pred snd₁))

-- combineL : ∀ {ℓ} → ∀ n → (A : Fin n → Type ℓ) → (B : Π A → Type ℓ) → Fin (suc n) → Type ℓ 
-- combineL n A B (zero , snd₁) = {!Σ ? ?!}
-- combineL n A B (suc fst₁ , snd₁) = {!!}

-- combine3n : ∀ {ℓ} → ∀ n
--               → (f₀ f₁ : Fin n → Type ℓ)
--               → (Π f₀ → Π f₁ → Fin n → Type ℓ)
--               → Fin (predℕ (n * 3)) → Type ℓ 
-- combine3n = {!!}
...
-- Boundaryₖ-S : ∀ {ℓ} → ∀ {n} → (NCube n → Type ℓ)
--               → ∀ (k : Fin (predℕ (3^ n)))
--               → {!piN ℓ-zero 5!}

-- Boundaryₖ : ∀ {ℓ} → ∀ {n} → (NCube n → Type ℓ) → Fin (predℕ (3^ n)) → Type ℓ
-- Boundaryₖ {n = zero} A x₁ = ⊥-elim {A = const (Type _)} (¬Fin0 x₁)
-- Boundaryₖ {n = suc zero} A (zero , snd₁) = A ((end false) ∷ [])
-- Boundaryₖ {n = suc zero} A (suc fst₁ , snd₁) = Pair (λ b → A ((end b) ∷ []))
-- Boundaryₖ {n = suc (suc n)} A (zero , snd₁) = A (corner0)
-- Boundaryₖ {n = suc (suc n)} A (suc fst₁ , snd₁) = {!!}
--   -- Σ (Boundaryₖ {n = suc (suc n)} A (fst₁ , <-weaken snd₁))
--   -- {!!}

-- Boundaryₖ-S = {!!}

-- --   let z : Fin (predℕ (predℕ (3^ (suc n)) * 3)) → Type _
-- --       z =
-- --           combine3n (predℕ (3^ (suc n)))
-- --           (Boundaryₖ {n = suc n} (A b∷ false))
-- --           (Boundaryₖ {n = suc n} (A b∷ true))
-- --           λ f₀ f₁ x → PathP (λ i →  (Boundaryₖ {n = suc n} (A i∷ i)) x) (f₀ x) (f₁ x)
...
-- --  where
-- --    f0 : Fin (suc (predℕ (3^ n * 3))) → Type _
-- --    f0 = {!!}

-- -- BP-test : Boundary {n = 4} (const ℕ)
-- -- BP-test = {!!}

-- -- -- Bnd : ∀ ℓ → ∀ n
-- -- --           → Σ[ Bd ∈ ((NCube n → Type ℓ) → Type ℓ) ]
-- -- --             Σ[ Cu ∈ (∀ {ty} → Bd ty → Type ℓ) ]
-- -- --              (∀ A → Iso (Π A) (Σ (Bd A) Cu))

-- -- -- fst (Bnd ℓ zero) _ = Lift Unit
-- -- -- fst (snd (Bnd ℓ zero)) {ty} _ = ty []
-- -- -- Iso.fun (snd (snd (Bnd ℓ zero)) A) x = lift _ , x []
-- -- -- Iso.inv (snd (snd (Bnd ℓ zero)) A) x [] = (snd x) 
-- -- -- Iso.rightInv (snd (snd (Bnd ℓ zero)) A) (fst₁ , snd₁) i = lift _ , snd₁
-- -- -- Iso.leftInv (snd (snd (Bnd ℓ zero)) A) a i [] = a []
...
-- -- --   fst' : (NCube (suc n) → Type ℓ) → Type ℓ
-- -- --   fst' A = Σ[ ends ∈ ((Σ ((fst prev) (A ∘ (end false ∷_))) (((fst (snd prev)) {ty = A ∘ (end false ∷_)})))
-- -- --                         ×
-- -- --                       (Σ ((fst prev) (A ∘ (end true ∷_))) (((fst (snd prev)) {ty = A ∘ (end true ∷_)})))) ]
-- -- --                 (PathP (λ i → fst (Bnd ℓ n) (λ x → A ((inside i) ∷ x)))
-- -- --                   (fst (fst ends))
-- -- --                   (fst (snd ends)))
               
-- -- --              -- (PathP (λ i → fst prev ((A i∷ i)))
-- -- --              -- (fst (Iso.fun ((snd (snd prev) (A ∘ (end false ∷_)))) (ends false)))             
-- -- --              -- (fst (Iso.fun ((snd (snd prev) (A ∘ (end true ∷_)))) (ends true))))


-- -- --   snd' : ∀ {ty : (NCube (suc n) → Type ℓ)} → fst' ty → Type ℓ
-- -- --   snd' {ty} x = PathP (λ i → Σ (fst prev (λ x₁ → ty (inside i ∷ x₁))) (fst (snd prev)))
-- -- --                  (fst (fst x))
-- -- --                  (snd (fst x))



-- -- --   iso' : (A : NCube (suc n) → Type ℓ) → Iso (Π A) (Σ (fst' A) (snd' {ty = A}))
-- -- --   iso' A = iso''

-- -- --     where

-- -- --       prev-iso : (i' : Interval') → _
-- -- --       prev-iso i' = snd (snd prev) λ x → A (i' ∷ x)

-- -- --       fun' : (i' : Interval') → _ 
-- -- --       fun' = Iso.fun ∘  prev-iso

-- -- --       inv' : (i' : Interval') → _ 
-- -- --       inv' = Iso.inv ∘  prev-iso

-- -- --       ret' : (i' : Interval') → _ 
-- -- --       ret' = Iso.rightInv ∘  prev-iso

-- -- --       sec' : (i' : Interval') → _ 
-- -- --       sec' = Iso.leftInv ∘  prev-iso 

-- -- --       iso'' : Iso (Π A) (Σ (fst' A) (snd' {ty = A}))
-- -- --       iso'' = {!!}

-- -- -- BoundaryP : ∀ {ℓ} → ∀ {n} → (A : NCube n → Type ℓ) → Type ℓ
-- -- -- BoundaryP {ℓ} {n} = fst (Bnd ℓ n)

-- -- -- InsideOfP : ∀ {ℓ} → ∀ {n} → (A : NCube n → Type ℓ) → BoundaryP A → Type ℓ
-- -- -- InsideOfP {ℓ} {n} A = fst (snd (Bnd ℓ n)) {A}

-- -- -- Cub≡Ins : ∀ ℓ → (λ {n : ℕ} {A : NCube n → Type ℓ} → Π A)
-- -- --                       ≡
-- -- --                 (λ {n : ℕ} {A : NCube n → Type ℓ} → Σ (BoundaryP A) (InsideOfP A)) 
-- -- -- Cub≡Ins ℓ i {n} {A} = isoToPath ((snd (snd (Bnd ℓ n))) A) i

-- -- -- Boundary : ∀ {ℓ} → ∀ n → (A : Type ℓ) → Type ℓ
-- -- -- Boundary {ℓ} n = fst (Bnd ℓ n) ∘ const

-- -- -- InsideOf : ∀ {ℓ} → ∀ {n} → {A : Type ℓ} → Boundary n A → Type ℓ
-- -- -- InsideOf {ℓ} {n} = fst (snd (Bnd ℓ n))

-- -- -- Bnd-test : ∀ ℓ → ((∀ n → NCube n → Type ℓ)) → {!!}
-- -- -- Bnd-test ℓ A = {!fst (snd  ( (Bnd ℓ 3)))  !}

-- -- -- Bnd-test' : Boundary 4 Bool
-- -- -- Bnd-test' = {!!}
...
-- -- -- --   fst' : (NCube (suc n) → Type ℓ) → Type ℓ
-- -- -- --   fst' A = Σ[ cyl ∈ (∀ i' → fst prev (A ∘ (i' ∷_))) ]
-- -- -- --              (∀ b → fst (snd prev) (cyl (end b)))

-- -- -- --   snd' : ∀ {ty : (NCube (suc n) → Type ℓ)} → fst' ty → Type ℓ
-- -- -- --   snd' x = PathP (λ i → fst (snd prev) (fst x (inside i)))
-- -- -- --              (snd x false) (snd x true)
...
-- -- -- --       iso'' : Iso (Π A) (Σ (fst' A) (snd' {ty = A}))
-- -- -- --       fst (fst (Iso.fun iso'' x)) i' =  fst ((fun' i') (x ∘ (i' ∷_)))
-- -- -- --       snd (fst (Iso.fun iso'' x)) b = snd ((fun' (end b)) (x ∘ ((end b) ∷_)))
-- -- -- --       snd (Iso.fun iso'' x) i = snd (fun' (inside i) (λ x₁ → x ((inside i) ∷ x₁)))

-- -- -- --       Iso.inv iso'' x (end x₁ ∷ x₂) = (inv' (end x₁) ((fst (fst x) (end x₁)) , snd (fst x) x₁)) x₂
-- -- -- --       Iso.inv iso'' x (inside i ∷ x₂) = (inv' (inside i) ((fst (fst x) (inside i)) , (snd x) i)) x₂

-- -- -- --       fst (fst (Iso.rightInv iso'' x i)) (end x₁) = fst (ret' (end x₁) ((fst (fst x) (end x₁)) , (snd (fst x) x₁)) i)   
-- -- -- --       fst (fst (Iso.rightInv iso'' x i)) (inside i₁) = fst (ret' (inside i₁) ((fst (fst x) (inside i₁)) , snd x i₁) i)       
-- -- -- --       snd (fst (Iso.rightInv iso'' x i)) b = snd (ret' (end b) ((fst (fst x) (end b)) , (snd (fst x) b)) i)      
-- -- -- --       snd (Iso.rightInv iso'' x i) i₁ = snd (ret' (inside i₁) ((fst (fst x) (inside i₁)) , (snd x) i₁ ) i)
      
-- -- -- --       Iso.leftInv iso'' a i (end x ∷ x₁) = sec' (end x) (a ∘ (end x ∷_)) i x₁
-- -- -- --       Iso.leftInv iso'' a i (inside i₁ ∷ x₁) = sec' (inside i₁) (a ∘ (inside i₁ ∷_)) i x₁ 
