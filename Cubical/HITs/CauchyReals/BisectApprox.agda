...
-- smaller-~lim : ∀ z (y : ℚ₊ → ℝ) (p : (δ ε : ℚ₊) → y δ ∼[ δ ℚ₊+ ε ] y ε)
--          (ε δ η : ℚ₊) v →
--         z ∼[ fst ε ℚ.- fst δ , v ] ( y δ )
--         → ∃[ δ' ∈ ℚ₊ ]
--          (fst δ' ℚ.≤ fst η)
--           × Σ _ λ v' → z ∼[ fst ε ℚ.- fst δ' , v' ] ( y δ' )
-- smaller-~lim z y p ε δ η v =
--   PT.rec squash₁
--     (uncurry $ λ δₓ → PT.map
--        λ (X , Y) →
--         let δ' = ℚ.min₊ (/2₊ δₓ) η
--             ≤ε-δ' : fst ε ℚ.+ (fst δ' ℚ.- fst δₓ)
--                        ℚ.≤ (fst ε ℚ.- fst δ')
--             ≤ε-δ' = ℚ.≤-o+ _ _ _
--                      ( subst2 ℚ._≤_
--                         {!!} (cong (fst δ' ℚ.+_)
--                          (sym (cong (ℚ.-_) (ℚ.x+x≡2x (fst δ')) ∙
--                          sym (𝐐'.-DistR· 2 (fst δ'))))
--                           ∙ lem--072)
--                         (ℚ.≤-o+ (2 ℚ.· ℚ.- fst (/2₊ δₓ)) _ (fst δ')
--                         (ℚ.≤-o· (ℚ.- fst (/2₊ δₓ)) _ 2 (ℚ.decℚ≤? {0} {2}) $
--                           ℚ.minus-≤ (fst δ') _ (ℚ.min≤ (fst (/2₊ δₓ)) (fst η)))))
--         in δ' , ((ℚ.min≤' _ _) ,
--             {!!} , (∼-monotone≤ (subst (ℚ._≤ (fst ε ℚ.- fst δ'))
--                      (sym lem--082) ≤ε-δ')
--               (triangle∼ Y (p _ _)))))
--   ∘S fst (rounded∼ _ _ _)

-- lim∈oi : ∀ a b y p → lim y p ∈ ointervalℙ a b →
--           ∃[ η ∈ ℚ₊ ] (∀ ε → fst ε ℚ.≤ fst η → y ε ∈ ointervalℙ a b  ) 
-- lim∈oi = {!!}
...
--  ointervalℙ'→ : ∀ {a b x} → x ∈ ointervalℙ' a b → x ∈ ointervalℙ a b 
--  ointervalℙ'→ x∈ = x∈
...
--  data _∼[_]∈o_  : ℝ∈o → ℚ₊ → ℝ∈o → Type

--  data ℝ∈o where
--   rat : ∀ r → rat r ∈ ointervalℙ' a b → ℝ∈o
  
--   lim : (x : ℚ₊ → ℝ∈o) →
--         (∀ ( δ ε : ℚ₊) → x δ ∼[ δ ℚ₊+ ε ]∈o x ε) → ℝ∈o
  
--   eqℝ : ∀ x y → (∀ ε → x ∼[ ε ]∈o y) → x ≡ y

--  data _∼[_]∈o_   where
--   rat-rat : ∀ q q∈ r r∈ ε
--     → (ℚ.- fst ε) ℚ.< (q ℚ.- r)
--     → (q ℚ.- r) ℚ.< fst ε → ((rat q q∈) ∼[ ε ]∈o (rat r r∈))
--   rat-lim : ∀ q q∈ y ε δ p v →
--          (rat q q∈) ∼[ fst ε ℚ.- fst δ , v ]∈o ( y δ )
--          → (rat q q∈) ∼[ ε ]∈o (lim y p )
--   lim-rat : ∀ x r r∈ ε δ p v →
--          (x δ) ∼[ fst ε ℚ.- fst δ , v ]∈o ( rat r r∈ )
--          → (lim x p) ∼[ ε ]∈o (rat r r∈ )
--   lim-lim : ∀ x y ε δ η p p' v →
--          (x δ) ∼[ (fst ε ℚ.- (fst δ ℚ.+ fst η)) , v ]∈o (y η )
--          → (lim x p) ∼[ ε ]∈o (lim y p' )
--   isProp∼ : ∀ r q r' → isProp (r ∼[ q ]∈o r')

--  isSetℝ∈o : isSet ℝ∈o
--  isSetℝ∈o = lem722 (λ r r' → ∀ ε →  r ∼[ ε ]∈o r')
--    (λ _ _ → isPropΠ λ _ → isProp∼ _ _ _)
--     eqℝ
--     {!!}
 
--  ℝ→ℝ∈o : ∀ x → x ∈ ointervalℙ a b → ℝ∈o
--  ℝ→ℝ∈o~ : ∀ ε x x∈ y y∈
--    → x ∼[ ε ] y
--    → ℝ→ℝ∈o x x∈ ∼[ ε ]∈o ℝ→ℝ∈o y y∈


--  ℝ→ℝ∈o (rat x) x∈ = rat x (→ointervalℙ' x∈)
--  ℝ→ℝ∈o (lim p y) x∈ =
--   rec→Set isSetℝ∈o {!!} {!!} (lim∈oi a b p y x∈)  
--  ℝ→ℝ∈o (eqℝ x x₂ x₃ i) x₁ = {!!}
...
-- <ᵣlim : ∀ a x y → a <ᵣ lim x y →
--              ∃[ δ ∈ ℚ₊ ] (∀ (ε : ℚ₊) →
--               (fst ε) ℚ.≤ (fst δ) → a <ᵣ x ε  )  
-- <ᵣlim a x y a<lim =
--   PT.map (λ (r , 0<r , r<lim-a) →
--       let δ =  (r , ℚ.<→0< _ (<ᵣ→<ℚ _ _ 0<r))
          
--       in /2₊ δ ,
--            λ ε ε<δ →
--             let zz = subst (ℚ._≤ r) (ℚ.·Comm _ _)
--                       (ℚ.x≤y·z→x·invℚ₊y≤z _ _ _
--                        (subst (fst ε ℚ.≤_) (ℚ.·Comm _ _) ε<δ))
--                 ll = isTrans<ᵣ _ _ _ (isTrans≤<ᵣ _ _ _ (≤absᵣ _ )
--                  (isTrans≡<ᵣ _ _ _ (minusComm-absᵣ _ _)
--                    (𝕣-lim-dist x y (2 ℚ₊· ε))))
--                     (isTrans≤<ᵣ _ _ _
--                       (≤ℚ→≤ᵣ _ _ zz) r<lim-a)
--             in subst2 _<ᵣ_
--                (-ᵣInvol _) (-ᵣInvol _ ∙
--                  cong x (ℚ₊≡ (ℚ.·Comm _ _
--                    ∙ (ℚ.[y·x]/y 2 (fst ε)))))
--                 (-ᵣ<ᵣ _ _ (<-o+-cancel _ _ _ ll))
--                 )
--     (denseℚinℝ 0 (lim x y -ᵣ a) (x<y→0<y-x _ _ a<lim))


-- lim<ᵣ : ∀ a x y → lim x y <ᵣ a →
--              ∃[ δ ∈ ℚ₊ ] (∀ (ε : ℚ₊) →
--               (fst ε) ℚ.≤ (fst δ) →  x ε  <ᵣ a  )  
-- lim<ᵣ a x y lim<a =   
--   PT.map (λ (r , 0<r , r<a-lim) →
--       let δ =  (r , ℚ.<→0< _ (<ᵣ→<ℚ _ _ 0<r))
...
--                       (ℚ.x≤y·z→x·invℚ₊y≤z _ _ _
--                        (subst (fst ε ℚ.≤_) (ℚ.·Comm _ _) ε<δ))
--                 ll = subst2 _<ᵣ_ (+ᵣComm _ _ ∙
--                     cong (λ ε → -ᵣ lim x y +ᵣ x ε)
--                       (ℚ₊≡ (ℚ.·Comm _ _ ∙ (ℚ.[y·x]/y 2 (fst ε)))))
--                      (+ᵣComm _ _)
--                      (isTrans<ᵣ _ _ _ (isTrans≤<ᵣ _ _ _ (≤absᵣ _ )
--                       ((𝕣-lim-dist x y (2 ℚ₊· ε))))
--                        (isTrans≤<ᵣ _ _ _
--                       (≤ℚ→≤ᵣ _ _ zz) r<a-lim))
                     
--             in <-o+-cancel _ _ _ ll)
--     (denseℚinℝ 0 (a -ᵣ lim x y) (x<y→0<y-x _ _ lim<a))


-- record Elimℝ∈ointerval {ℓ} {ℓ'} {a b : ℝ }  (A : ℝ → Type ℓ)
--                (B : ∀ {x y : ℝ} →
--                   A x → A y →
--                 ∀ ε → Type ℓ') : Type (ℓ-max ℓ ℓ') where
 
--  field
--   ratA : ∀ x → rat x ∈ ointervalℙ' a b  →  A (rat x)  
--   limA : ∀ x p → (𝒂 : ∀ q → (x q) ∈ ointervalℙ' a b → A (x q)) →
--              (∀ δ ε δ∈ ε∈ → B (𝒂 δ δ∈) (𝒂 ε ε∈)
--                (δ ℚ₊+ ε) )
--                 → (lim x p) ∈ ointervalℙ' a b → A (lim x p)
--   B-mon : ∀ {x y} Ax Ay ε ε' → fst ε ℚ.≤ fst ε' →
--      B {x} {y} Ax Ay ε → B {x} {y} Ax Ay ε'                 
--   eqA : ∀ {x y} p a a' → (∀ δ ε → B a a' (δ ℚ₊+ ε))
--    → (∀ ε → B a a' ε)
--    → PathP (λ i → A (eqℝ x y p i)) a a'

 
--   rat-rat-B : ∀ q r ε q∈ r∈ → ℚ.abs' (q ℚ.- r) ℚ.< fst ε
--      → B (ratA q q∈) (ratA r r∈) ε
     
--   rat-lim-B : ∀ q q∈ y ε δ p v (r : (rat q) ∼[ fst ε ℚ.- fst δ , v ] ( y δ ))
--       v' u' (yδ∈ : y δ ∈ ointervalℙ' a b) lim∈ →
--        B (ratA q q∈) (v' δ yδ∈) ((fst ε ℚ.- fst δ) , v)  →
--        B (ratA q q∈) (limA y p v' u' lim∈) ε --(rat-lim q y ε δ p v r)
...
--   isPropB : ∀ {x y} a a' ε → isProp (B {x} {y} a a' ε)

--  go : ∀ x → x ∈ ointervalℙ' a b → A x

--  go∼ : ∀ {x x' ε} x∈ x'∈ → (r : x ∼[ ε ] x') → ∀ {D} → depth∼ r ≡ D  →
--          B (go x x∈) (go x' x'∈) ε


--  go (rat x) = ratA x
--  go (lim x x₁) = limA x x₁ (λ q → go (x q))
--    λ δ ε δ∈ ε∈ → go∼ {x δ} {x ε} {δ ℚ₊+ ε} δ∈ ε∈ (x₁ _ _) refl

--  go (eqℝ x₁ x₂ x₃ i) x = 
--    -- hcomp {λ j → λ { (i = i0) → ? ; (i = i1) → ?}
--    --      {!!}
--   eqA {x₁} {x₂} x₃ (go x₁ (coei→0 (λ i → eqℝ x₁ x₂ x₃ i ∈ ointervalℙ' a b) i x))
--     (go x₂ ((coei→1 (λ i → eqℝ x₁ x₂ x₃ i ∈ ointervalℙ' a b) i x)))
--     (λ δ ε → go∼ {x₁} {x₂}
--      (coei→0 (λ i₁ → eqℝ x₁ x₂ x₃ i₁ ∈ ointervalℙ' a b) i x)
--      (coei→1 (λ i₁ → eqℝ x₁ x₂ x₃ i₁ ∈ ointervalℙ' a b) i x) (x₃ _)
--      refl)
--     (λ ε → go∼ {x₁} {x₂}
--       (coei→0 (λ i₁ → eqℝ x₁ x₂ x₃ i₁ ∈ ointervalℙ' a b) i x)
--       (coei→1 (λ i₁ → eqℝ x₁ x₂ x₃ i₁ ∈ ointervalℙ' a b) i x) (x₃ ε) refl) i

--  go∼ x∈ x'∈ (rat-rat q r ε x₁ x₂) x =
--   rat-rat-B _ _ _ _ _ (subst (ℚ._< fst ε)
--      (ℚ.abs'≡abs _) $ ℚ.absFrom<×< _ _ x₁ x₂)
--  go∼ x∈ x'∈ (rat-lim q y _ δ p v r) x = {!!}
--  go∼ x∈ x'∈ (lim-rat x₁ r _ δ p v r₁) x = {!!}
--  go∼ x∈ x'∈ (lim-lim x₁ y _ δ η p p' v r) x = {!!}
--  go∼ x∈ x'∈ (isProp∼ r q r' r₁ r₂ i) x = {!!}
 
--  -- go∼ x∈ x'∈ (rat-rat q r ε x₁ x₂) =
--  --   rat-rat-B _ _ _ _ _ (subst (ℚ._< fst ε)
--  --     (ℚ.abs'≡abs _) $ ℚ.absFrom<×< _ _ x₁ x₂) 
--  -- go∼ {_} {.(lim y p)} x∈ x'∈ (rat-lim q y ε δ p v r) = {!!}
--  --  -- PT.rec2 (isPropB _ _ _ )
--  --  --   (λ (δA , Xa) (δB , Xb) →
--  --  --     PT.rec (isPropB _ _ _)
--  --  --           (λ (δ' , (Y , Y' , Y'')) →
--  --  --             rat-lim-B _ _ _ _ δ' _ _ Y'' _
--  --  --               (λ v₁ v₂ v₃ v₄ → go∼ v₃ v₄ (p v₁ v₂))
--  --  --                {!fst Y!} x'∈  (go∼ {rat q} {y δ'} x∈ {!!} Y''))
--  --  --       (smaller-~lim _ y p ε _ (ℚ.min₊ δA δB) _ r))
--  --  --  (<ᵣlim a y p {!!})
--  --  --   (lim<ᵣ b y p {!!})
  
--  --   --  PT.rec {!!} {!!}
--  --   -- (smaller-~lim _ _  _ _ _ {!!} _ r) 
--  --     -- rat-lim-B _ _ _ _ δ _ v r _
--  --     --  (λ z z₁ z zz → go∼ z zz (p _ _))
--  --     --   {!!}  x'∈ (go∼ {rat q} {y δ} {(fst ε ℚ.- fst δ) , _} x∈ {!!} r)
--  -- go∼ x∈ x'∈ (lim-rat x₁ r ε δ p v r₁) = {!!}
--  -- go∼ x∈ x'∈ (lim-lim x₁ y ε δ η p p' v r) = {!!}
--  -- go∼ x∈ x'∈ (isProp∼ r q r' r₁ r₂ i) = {!!}
--  -- --(rat-rat q r ε x x₁) = {!!} --rat-rat-B q r ε x x₁
...
-- -- -- fromLipshitzℚ→ℝℙ : ∀ L a b → a <ᵣ b  
-- -- --                        → Σ _ (Lipschitz-ℚ→ℝℙ L (ointervalℙ a b))
-- -- --                        → Σ _ (Lipschitz-ℝ→ℝℙ L (ointervalℙ a b))
-- -- -- fromLipshitzℚ→ℝℙ L a b a<b (f , fL) =
-- -- --   f' , λ u u∈ v v∈ ε x → Elimℝ.go∼ w x u∈ v∈
-- -- --   where


-- -- --   module La-f (x : ℚ₊ → ℝ)
-- -- --          (p : (δ ε : ℚ₊) → x δ ∼[ δ ℚ₊+ ε ] x ε)
-- -- --       (y : (q : ℚ₊) → x q ∈ ointervalℙ a b → ℝ)
-- -- --       (X : (δ ε : ℚ₊) (u∈ : x δ ∈ ointervalℙ a b)
-- -- --        (v∈ : x ε ∈ ointervalℙ a b) →
-- -- --        y δ u∈ ∼[ L ℚ₊· (δ ℚ₊+ ε) ] y ε v∈)
-- -- --       ((Δa , A) : Σ ℚ₊ (λ δ → (ε : ℚ₊) → fst ε ℚ.≤ fst δ → a <ᵣ x ε))
-- -- --       ((Δb , B) : Σ ℚ₊ (λ δ → (ε : ℚ₊) → fst ε ℚ.≤ fst δ → x ε <ᵣ b))
-- -- --       where

-- -- --     γ₊ : ℚ₊
-- -- --     γ₊ = (ℚ.min₊ Δa Δb)
-- -- --     ε* : ℚ₊ → ℚ₊
-- -- --     ε* ε = (ℚ.min₊ γ₊ ((invℚ₊ L) ℚ₊· ε))

-- -- --     ε*≤ : ∀ ε → fst (L ℚ₊· ε* ε) ℚ.≤ (fst ε)
-- -- --     ε*≤ ε = ℚ.isTrans≤ _ _ _
-- -- --          (ℚ.≤-o· _ _ _ (ℚ.0≤ℚ₊ L)
-- -- --             (ℚ.min≤' _ _))
-- -- --          (ℚ.≡Weaken≤ _ _ (ℚ.y·[x/y] L (fst ε)))
    
-- -- --     ε*∈ : ∀ ε → x (ε* ε) ∈ ointervalℙ a b
-- -- --     ε*∈ ε = (A (ℚ.min₊ (ℚ.min₊ Δa Δb) (invℚ₊ L ℚ₊· ε))
-- -- --              (ℚ.isTrans≤ _ _ _
-- -- --                (ℚ.min≤ _ _) (ℚ.min≤ _ _)) ,
-- -- --             B (ℚ.min₊ (ℚ.min₊ Δa Δb) (invℚ₊ L ℚ₊· ε))
-- -- --              ((ℚ.isTrans≤ _ _ _
-- -- --                (ℚ.min≤ _ _) (ℚ.min≤' _ _))))
...
-- -- --       lim (λ ε → y (ε* ε) (ε*∈ ε))
-- -- --        λ δ ε →
-- -- --           let ε≤ = subst (ℚ._≤ fst (δ ℚ₊+ ε))
-- -- --                      (sym (ℚ.·DistL+ (fst L) _ _))
-- -- --                      (ℚ.≤Monotone+ _ _ _ _
-- -- --                        (ε*≤ δ) (ε*≤ ε))
-- -- --           in ∼-monotone≤ ε≤ (X _ _ (ε*∈ δ) (ε*∈ ε))

-- -- --   la : (x : ℚ₊ → ℝ) (p : (δ ε : ℚ₊) → x δ ∼[ δ ℚ₊+ ε ] x ε)
-- -- --       (y : (q : ℚ₊) → x q ∈ ointervalℙ a b → ℝ) →
-- -- --       ((δ ε : ℚ₊) (u∈ : x δ ∈ ointervalℙ a b)
-- -- --        (v∈ : x ε ∈ ointervalℙ a b) →
-- -- --        y δ u∈ ∼[ L ℚ₊· (δ ℚ₊+ ε) ] y ε v∈) →
        
-- -- --       lim x p ∈ ointervalℙ a b → 
-- -- --       ∥
-- -- --       Σ (Σ ℚ₊ (λ x₁ → (δ : ℚ₊) → fst δ ℚ.≤ fst x₁ → a <ᵣ x δ))
-- -- --       (λ _ → Σ ℚ₊ (λ δ → (ε : ℚ₊) → fst ε ℚ.≤ fst δ → x ε <ᵣ b))
-- -- --       ∥₁ → ℝ
-- -- --   la x p y X (<lim , lim<) =
-- -- --    PT.rec→Set isSetℝ
-- -- --      (uncurry (La-f.la-f x p y X) )
-- -- --      2-con-la-f

-- -- --     where

-- -- --     ηs = PT.map2 _,_
-- -- --        (<ᵣlim _ _ _ <lim) (lim<ᵣ _ _ _ lim<)

-- -- --     2-con-la-f : 2-Constant (uncurry (La-f.la-f x p y X))
-- -- --     2-con-la-f (δ , 𝑿 , 𝒀) (δ' , 𝑿' , 𝒀') = eqℝ _ _ λ ε →
-- -- --      let ε⋆ = ℚ.min₊
-- -- --                 (L ℚ₊· La-f.ε* x p y X δ (𝑿 , 𝒀) ε)
-- -- --                 (L ℚ₊· La-f.ε* x p y X δ' (𝑿' , 𝒀') ε)
-- -- --          ε⋆≤ε : fst ε⋆ ℚ.≤ fst ε
-- -- --          ε⋆≤ε = ℚ.isTrans≤ _ _ _ (ℚ.min≤ _ _)
-- -- --                  (ℚ.isTrans≤ _ _ _
-- -- --                    (ℚ.≤-o· _ _ _ (ℚ.0≤ℚ₊ L)
-- -- --                      (ℚ.min≤' _ _))
-- -- --                    ((ℚ.≡Weaken≤ _ _ (ℚ.y·[x/y] L (fst ε)))))
-- -- --          ε*≡ε*'0 : fst (/4₊ (La-f.ε* x p y X δ (𝑿 , 𝒀) ε))
-- -- --                      ℚ.≤ (fst (La-f.γ₊ x p y X δ (𝑿 , 𝒀))) 
-- -- --          ε*≡ε*'0 =
-- -- --             ℚ.isTrans≤ _ _ _
-- -- --               (ℚ.≤-o· _ 1 
-- -- --                  (fst ((La-f.ε* x p y X δ (𝑿 , 𝒀) ε)) )
-- -- --                   (ℚ.0≤ℚ₊ (La-f.ε* x p y X δ (𝑿 , 𝒀) ε) )
-- -- --                    ℚ.decℚ≤? )
-- -- --                     (ℚ.isTrans≤ _ _ _
-- -- --                       (ℚ.≡Weaken≤ _ _ (ℚ.·IdR _))
-- -- --                        (ℚ.min≤ _ _))
-- -- --          ε*≡ε*' : (La-f.ε* x p y X δ (𝑿 , 𝒀) (/4₊ ε⋆))
-- -- --                     ≡ (La-f.ε* x p y X δ' (𝑿' , 𝒀') (/4₊ ε⋆))
-- -- --          ε*≡ε*' = ℚ₊≡ (
-- -- --            (ℚ.minComm _ _ ∙
-- -- --              ℚ.≤→min ((invℚ₊ L ℚ₊· /4₊ ε⋆) .fst)
-- -- --                 (fst (La-f.γ₊ x p y X δ (𝑿 , 𝒀)))
-- -- --                 (ℚ.isTrans≤ ((invℚ₊ L ℚ₊· /4₊ ε⋆) .fst)
-- -- --                    (fst (/4₊ (La-f.ε* x p y X δ (𝑿 , 𝒀) ε))) _
-- -- --                   {!!} ε*≡ε*'0)
-- -- --                -- (ℚ.isTrans≤ _ _ _
...
-- -- --      in lim-lim _ _
-- -- --         _ (/4₊ ε⋆) (/4₊ ε⋆) _ _
-- -- --          (snd (ℚ.<→ℚ₊ _ _ (subst (ℚ._< fst ε)
-- -- --            (cong fst (sym (ℚ./4₊+/4₊≡/2₊ ε⋆)))
-- -- --             (ℚ.isTrans<≤ _ _ _ (x/2<x ε⋆)
-- -- --               ε⋆≤ε ))))
-- -- --                 ((refl subst∼[ refl ]
-- -- --                   cong (uncurry y)
-- -- --                     (Σ≡Prop (λ q → ∈-isProp (ointervalℙ a b) (x q))
-- -- --                      ε*≡ε*'))
-- -- --                   (refl∼ _ _))
...
-- -- --       lim x p ∈ ointervalℙ a b → ℝ
-- -- --   la' x p y X (<lim , lim<) =
-- -- --     la x p y X (<lim , lim<) ηs 
...
-- -- --   rlB' : (q : ℚ) (y : ℚ₊ → ℝ) (ε δ : Σ ℚ 0<_)
-- -- --       (p : (δ₁ ε₁ : ℚ₊) → y δ₁ ∼[ δ₁ ℚ₊+ ε₁ ] y ε₁)
-- -- --       (v : 0< (fst ε ℚ.- fst δ))
-- -- --       (r : rat q ∼[ (fst ε ℚ.- fst δ) , v ] y δ)
-- -- --       (v' : (q₁ : ℚ₊) → y q₁ ∈ ointervalℙ a b → ℝ)
-- -- --       (u'
-- -- --        : (δ₁ ε₁ : ℚ₊) (u∈ : y δ₁ ∈ ointervalℙ a b)
-- -- --          (v∈ : y ε₁ ∈ ointervalℙ a b) →
-- -- --          v' δ₁ u∈ ∼[ L ℚ₊· (δ₁ ℚ₊+ ε₁) ] v' ε₁ v∈) →
-- -- --       ((u∈ : rat q ∈ ointervalℙ a b) (v∈ : y δ ∈ ointervalℙ a b) →
-- -- --        f q u∈ ∼[ L ℚ₊· ((fst ε ℚ.- fst δ) , v) ] v' δ v∈) →
-- -- --       (u∈ : rat q ∈ ointervalℙ a b) (v∈ : lim y p ∈ ointervalℙ a b) →
-- -- --        ∀ xx → f q u∈ ∼[ L ℚ₊· ε ] (la y p v' u' v∈ xx)
-- -- --   rlB' q y ε δ p v r v' u' x u∈ v∈ = PT.elim
-- -- --    {A =   (Σ ℚ₊ (λ x₁ → (δ₁ : ℚ₊) → fst δ₁ ℚ.≤ fst x₁ → a <ᵣ y δ₁))
-- -- --           × (Σ ℚ₊ (λ δ₁ → (ε₁ : ℚ₊) → fst ε₁ ℚ.≤ fst δ₁ → y ε₁ <ᵣ b))}
-- -- --     (λ _ → isProp∼ _ _ _ )
-- -- --      λ ((δᵃ , Xa) , (δᵇ , Xb)) →
-- -- --       PT.rec (isProp∼ _ _ _)
-- -- --        (λ (δ*₊ , X) →
-- -- --         PT.rec (isProp∼ _ _ _)
-- -- --          (λ (0<ε-δ-δ* , X) →
-- -- --           let η = ℚ.min₊ (ℚ.min₊ δᵃ δᵇ) (/4₊ δ*₊)
-- -- --               δ⋆ = {!!}
-- -- --           in PT.rec  (isProp∼ _ _ _)
-- -- --             (λ (τ , Xτ)  →
-- -- --              let zz =
-- -- --                       triangle∼ (invEq (∼≃abs<ε _ _ _)
-- -- --                        (fL q u∈ τ {!!}
-- -- --                         ((fst ε ℚ.- ([ 1 / 2 ]  ℚ.· fst δ*₊)) , {!!})
-- -- --                          (<ᵣ→<ℚ _ _ (
-- -- --                            isTrans≡<ᵣ _ _ _ (cong rat (ℚ.abs'≡abs _ ))
-- -- --                             (fst (∼≃abs<ε (rat q) (rat τ)
-- -- --                           ((fst ε ℚ.- ([ 1 / 2 ] ℚ.· fst δ*₊)) , {!!}))
-- -- --                            (∼-monotone≤ {!!}
-- -- --                              (triangle∼
-- -- --                                (triangle∼ X (p δ η))
-- -- --                                 (invEq (∼≃abs<ε _ _ _)
-- -- --                                  (isTrans≡<ᵣ _ _ _
-- -- --                                    (absᵣPos _ (x<y→0<y-x _ _ (snd Xτ)))
-- -- --                                     ((a-b<c⇒a-c<b _ _ _
-- -- --                                       (isTrans≤<ᵣ _ _ _
-- -- --                                       (isTrans≤≡ᵣ _ _ _
...
-- -- --                        (𝕣-lim-self _ _  _ _))
-- -- --               in subst∼ {!!} zz)
-- -- --            (denseℚinℝ (maxᵣ a (y η -ᵣ rat (fst (/4₊ δ*₊)))) (y η)
-- -- --               {!!}
-- -- --             -- (Xa η
-- -- --             --  {!!}
-- -- --             --   --(ℚ.isTrans≤ _ _ _ (ℚ.min≤ _ _) (ℚ.min≤ _ _))
-- -- --             --  )
-- -- --              )
-- -- --            -- (denseℚinℝ _ _ (Xb η
-- -- --            --   (ℚ.isTrans≤ _ _ _ (ℚ.min≤ _ _) (ℚ.min≤' _ _))))
...
-- -- --        --     zz0 : _ ∼[ ({!!} ℚ₊+
-- -- --        --                   {!!})
-- -- --        --                  ℚ₊+ (η ℚ₊+ φ) ] La-f.la-f y p v' u' (fst xxx) (snd xxx)
-- -- --        --     zz0 = triangle∼
-- -- --        --          (triangle∼ {!fL ?!}
-- -- --        --            {!!})
-- -- --        --          (𝕣-lim-self _ _  η φ)
-- -- --        --     zz : f q u∈ ∼[ L ℚ₊· ε ] La-f.la-f y p v' u' (fst xxx) (snd xxx) 
-- -- --        --     zz = {!!}
-- -- --        -- in zz
...
-- -- --        f q u∈ ∼[ L ℚ₊· ((fst ε ℚ.- fst δ) , v) ] v' δ v∈) →
-- -- --       (u∈ : rat q ∈ ointervalℙ a b) (v∈ : lim y p ∈ ointervalℙ a b) →
-- -- --       f q u∈ ∼[ L ℚ₊· ε ] (la' y p v' u' v∈)
-- -- --   rlB q y ε δ p v r v' u' x u∈ v∈ = 
-- -- --      rlB' q y ε δ p v r v' u' x u∈ v∈ _
-- -- --   w : Elimℝ (λ x → x ∈ (ointervalℙ a b) → ℝ)
-- -- --         λ u v ε X → ∀ u∈ v∈ → u u∈ ∼[ L ℚ₊· ε  ] v v∈
-- -- --   w .Elimℝ.ratA = f
-- -- --   w .Elimℝ.limA = la'

-- -- --   w .Elimℝ.eqA {x} {x'} p y y' X Y =
-- -- --      congP (λ i f → f ∘ (transport⁻-fillerExt
-- -- --        (λ i → eqℝ x x' p i ∈ ointervalℙ a b)) i)
-- -- --          (funExt (λ x∈ →
-- -- --           let x∈' = (subst-∈ (ointervalℙ a b)
-- -- --                  (eqℝ x x' p ) x∈)
-- -- --           in eqℝ (y x∈) (y' x∈')
-- -- --             λ ε →
-- -- --               subst∼ (ℚ.y·[x/y] L (fst ε)) (Y (invℚ₊ L ℚ₊· ε) x∈ x∈') ))
-- -- --       ▷ funExt λ x∈ → cong y' (snd (ointervalℙ a b x') _ _)
-- -- --   w .Elimℝ.rat-rat-B q r ε x x₁ u∈ v∈ =
-- -- --     invEq (∼≃abs<ε _ _ _)
-- -- --       (fL q u∈ r v∈ ε (ℚ.absFrom<×< _ _ x x₁))
-- -- --   w .Elimℝ.rat-lim-B = rlB
-- -- --   w .Elimℝ.lim-rat-B x r ε δ p v u v' u' x₁ u∈ v∈ =
-- -- --      (sym∼ _ _ _ (rlB r x ε δ p v (sym∼ _ _ _ u) _ _
-- -- --       (λ u∈₁ v∈₁ → sym∼ _ _ _ (x₁ v∈₁ u∈₁) ) v∈ u∈)) 
-- -- --   w .Elimℝ.lim-lim-B x y ε δ η p p' v r v' u' v'' u'' x₁ u∈ v∈ =
-- -- --     {!lim-lim _ _ _ ? ? _ _
-- -- --       ?
-- -- --       ?!}
-- -- --   w .Elimℝ.isPropB _ _ _ _ = isPropΠ2
-- -- --    λ _ _ → isProp∼ _ _ _


-- -- --   f' : ∀ x → x ∈ (ointervalℙ a b) → ℝ
-- -- --   f' = Elimℝ.go w
...
module IsBilipschitz' (a b : ℚ)  (a<b : a ℚ.< b)
 ...
 module Inv
  ...
  module _ (y : ℚ) (y∈ : rat y ∈ (intervalℙ fa fb))  where
   ...
   stepS' Δ s = w (ℚ.Dichotomyℚ y fMid)
    ...
    w (inr x) = ww
     ...
     ww .fst .Step.a'∈ = newa'∈ 
     ww .fst .Step.b'∈ = b'∈
     ww .fst .Step.a'≤ = invEq (≤ᵣ'≃≤ᵣ _ _) fnewa'≤y
     ww .fst .Step.≤b' = ≤b'
  ...
  a*<b* n = subst (ℚ._< fst (b* (suc n)))
    (ℚ.+Comm _ _)
    (<ᵣ→<ℚ _ _ (fst (x+y<x'-y'≃x+y'<x'-y
       (rat (Δ₀ ℚ.· [ 1 / 1+ (suc (suc n)) ]))
       (rat (Δ₀ ℚ.· [ 1 / 1+ (suc (suc n)) ]))
       (rat b) (rat a))
    (isTrans<≡ᵣ _ _ _
      (isTrans≡<ᵣ _ _ _
        (cong rat (sym (ℚ.·DistL+ Δ₀ _ _)
          ∙ cong (Δ₀ ℚ.·_) (ℚ.n/k+m/k _ _ _)))
        (<ℚ→<ᵣ _ _ (ℚ.<-o· _ _ _ (ℚ.0<ℚ₊ Δ₀₊)
          (invEq ([n/m]<1 _ _) (ℕ.<-k+ ℕ.zero-<-suc)))))
      (rat·ᵣrat _ _ ∙ ·IdR _))))
     ...
     isTrans<ᵣ _ _ _
       ...
       (isTrans≤<ᵣ _ _ _
         (fst (x+y≤x'-y'≃x+y'≤x'-y _ _ _ _)
           (isTrans≤≡ᵣ _ _ _ (apartFrom-Invlipschitz-ℝ→ℝℙ
           K _ f lip⁻¹F _ _ _ _
  ...
  fb*-asc n =
   ...
   w = isTrans≡≤ᵣ _ _ _ (+ᵣComm _ _) $
        fst (x+y≤x'-y'≃x+y'≤x'-y (rat (fst (Ψ₊ ℚ₊· δₙ n))) _
        (f (rat (b* (suc n) .fst)) (snd (b* (suc n))))  _) $
         isTrans≤≡ᵣ _ _ _
        (isTrans≡≤ᵣ _ _ _
      ...
      PT.rec (isProp<ᵣ (flg x) (flg (rat (fst (fb* (suc n)))) +ᵣ rat (fst ε)))
       (λ (η' , <η' , η'<x) →
        ...
        ((denseℚinℝ (x -ᵣ rat (fst (invℚ₊ K ℚ₊· ε))) x
          (isTrans<≡ᵣ _ _ _
            ((<ᵣ-o+ _ _ _
              (-ᵣ<ᵣ _ _ (snd (ℚ₊→ℝ₊ (invℚ₊ K ℚ₊· ε)))))) (+IdR _))))
