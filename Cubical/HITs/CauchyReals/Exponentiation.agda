...
module EquivFromRestr 
 ...
 clamp' x =
   PT.map (λ (1+ n , X) →
        n
     , isTrans<≡ᵣ _ _ _ (-ᵣ<ᵣ _ _ (isTrans≤<ᵣ _ _ _ (≤absᵣ _)
...
module _ (f : ℚ → ℝ) (B : ℕ → ℚ₊)  where
 ...
 module BoundedLipsch (bl : boundedLipschitz) where
  ...
  seq⊂ℝ : Seq⊆-abs<N Seq⊆.s⊇ (λ _ → Unit , isPropUnit)
  seq⊂ℝ x _ = 
    PT.map
      (λ (1+ n , X) → 
  ...
  open Seq⊆→.FromIntersection Seq⊆→Power
    isSetℝ (λ _ → Unit , isPropUnit) seq⊂ℝ public
...
nth-root-slope-incr zero x Δ  = ≡ᵣWeaken≤ᵣ _ _ (cong rat lem--063)
                             ...
                             (X _ _
                               (subst (_∼[ δ ]
                                ...
                                (rat (fst Δ)) δ
                                   (invEq (∼≃abs<ε _ _ _)
                                     (isTrans≡<ᵣ _ _ _
                                       (minusComm-absᵣ _ _ ∙ cong absᵣ (+IdR _)
...
uContRootℚ : ∀ n → IsUContinuousℚℙ (λ x → (0 <ᵣ x) , squash₁ )
                ...
                (cong₂ (λ v∈ u∈ →
                   fst (root (1+ n) (rat (x ℚ.+ Δ .fst) , v∈)) -ᵣ
                    fst (root (1+ n) (rat x , u∈)))
                     (squash₁ _ _) (squash₁ _ _))
                 (nth-root-slope-incr _
                   (x , (ℚ.<→0< _ (<ᵣ→<ℚ _ _ u∈))) _) )
             X'))
  
uContRoot : ∀ n → IsUContinuousℙ (λ x → (0 <ᵣ x) , squash₁ )
                   (curry (fst ∘ root n))
...
pre^ℚ-aprox : ∀ a b → ℚApproxℙ'' (λ x → (0 <ᵣ x) , squash₁)
                                 (λ x → (0 <ᵣ x) , squash₁)
                    λ x 0<x → (root b ((x , 0<x) ^ℤ a))
pre^ℚ-aprox a b =
  ℚApproxℙ∘ (λ x → (0 <ᵣ x) , squash₁)
            (λ x → (0 <ᵣ x) , squash₁)
            (λ x → (0 <ᵣ x) , squash₁)
     (curry (root b))
      (curry ((_^ℤ a)))
...
module _ (x : ℝ) (0<x : 0 <ᵣ x) where
 ...
 _^ℚc_ = SQ.Elim.go w 
  where
  w : Elim (λ _ → ℝ₊)
  w .Elim.isSetB _ = isSetΣ isSetℝ λ _ → isProp→isSet squash₁
...
IsContinuous^ℚ : ∀ q → IsContinuousWithPred (λ _ → _ , squash₁)
                          (curry (fst ∘ (_^ℚ q))) 
...
invℝ₊[^ℤ] x (pos zero) = ℝ₊≡ (cong rat ℚ.decℚ?)
...
^ℚ-Monotone {_ , 0<x} {_ , 0<y} = uncurry $ ElimProp.go w
 where
 w : ElimProp _
 w .ElimProp.isPropB _ = isPropΠ2 λ _ _ → isSetℝ _ _
 w .ElimProp.f (pos (suc n) , b) 0<q x≤y = 
   ...
   fst (invℝ₊-<-invℝ₊ (x ^ℚ q) (x ^ℚ r))
     (subst2 _<ᵣ_
       ...
       (^ℚ-StrictMonotoneR {invℝ₊ x}
         1/x<1 q r q<r))
   where
   1/x<1 = isTrans≡<ᵣ _ _ _ (cong rat ℚ.decℚ?)
...
slope-incr : (z : ℝ₊) (1<z : 1 <ᵣ fst z) → (a b c : ℚ)
  ...
  ((fst (z ^ℚ c) -ᵣ fst (z ^ℚ a))
    ／ᵣ₊ ℚ₊→ℝ₊ (ℚ.<→ℚ₊ _ _  a<c ))
slope-incr z 1<z a b c a<b = flip
  λ a<c → ⊎.rec
     (≡ᵣWeaken≤ᵣ _ _ ∘
       cong
         (λ (b , a<b) →
...
slope-incr' : (z : ℝ₊) (1<z : 1 <ᵣ fst z) → (a b c : ℚ)
  ...
  ((fst (z ^ℚ c) -ᵣ fst (z ^ℚ b))
    ／ᵣ₊ ℚ₊→ℝ₊ (ℚ.<→ℚ₊ _ _  b<c ))
slope-incr' z 1<z a b c = ⊎.rec
    (λ a≡b b<c a<c →
      ≡ᵣWeaken≤ᵣ _ _
        (cong
         (λ (b , b<c) →
           ((fst (z ^ℚ c) -ᵣ fst (z ^ℚ b)) ／ᵣ₊ ℚ₊→ℝ₊ (ℚ.<→ℚ₊ b c b<c)))
            (Σ≡Prop {B = λ b → b ℚ.< c} (flip ℚ.isProp< _)
                a≡b)) )
    (λ a<b b<c a<c →
      <ᵣWeaken≤ᵣ _ _ $
       slope-incr'-strict z 1<z a b c a<b b<c a<c )
...
slope-monotone : (z : ℝ₊) (1<z : 1 <ᵣ fst z) → (a b a' b' : ℚ)
  ...
  ((fst (z ^ℚ b') -ᵣ fst (z ^ℚ a'))
    ／ᵣ₊ ℚ₊→ℝ₊ (ℚ.<→ℚ₊ _ _  a'<b' ))
slope-monotone z 1<z a b a' b' a<b a'<b' a≤a' b≤b' =
  isTrans≤ᵣ _ _ _
    (slope-incr z 1<z a b b' a<b b≤b' (ℚ.isTrans<≤ _ _ _ a<b b≤b'))
      (slope-incr' z 1<z a a' b' a≤a' a'<b' (ℚ.isTrans<≤ _ _ _ a<b b≤b'))
