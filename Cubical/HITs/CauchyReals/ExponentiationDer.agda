...
--   (((fst b' ^ⁿ n) -ᵣ (fst a' ^ⁿ n))
--     ·ᵣ (fst b -ᵣ fst a))
-- ratioMonotone^ zero a b a' b' a≤a' b≤b' = ≡ᵣWeaken≤ᵣ _ _
--     ((𝐑'.0LeftAnnihilates' _ _ (+-ᵣ _))
--     ∙ sym (𝐑'.0LeftAnnihilates' _ _ (+-ᵣ _)))
-- ratioMonotone^ (suc zero) a b a' b' a≤a' b≤b' = ≡ᵣWeaken≤ᵣ _ _ $
--   ·ᵣComm _ _ ∙ cong₂ _·ᵣ_
--     (cong₂ _-ᵣ_ (sym (·IdL _)) (sym (·IdL _)))
--     (cong₂ _-ᵣ_ (·IdL _) (·IdL _))
-- ratioMonotone^ (suc (suc N)) a b a' b' a≤a' b≤b' =
--   subst2 _≤ᵣ_
--      (·ᵣAssoc _ _ _ ∙ cong (_·ᵣ (fst b' -ᵣ fst a')) (·ᵣComm _ _ ∙ fa.[b-a]·S≡bⁿ-aⁿ))
...
--  w : (n₁ : ℕ) → n₁ ℕ.< suc (suc N) →
--       geometricProgression (fst b ^ⁿ (suc N))
--         (fst a ／ᵣ[ fst b , inl (snd b) ]) n₁ ≤ᵣ
--       geometricProgression (fst b' ^ⁿ (suc N))
--         (fst a' ／ᵣ[ fst b' , inl (snd b') ]) n₁
--  w n x =
...
-- ratio-monotone-ₙ√ : (n : ℕ) (a b a' b' : ℝ₊)
--   → (a≤a' : fst a ≤ᵣ fst a') →  (b≤b' : fst b ≤ᵣ fst b') → 
...
--   ((fst (root (1+ n) b) -ᵣ fst (root (1+ n) a))
--     ·ᵣ (fst b' -ᵣ fst a' ))
-- ratio-monotone-ₙ√ n a b a' b' a≤a' b≤b' =
--  subst2 _≤ᵣ_
--     (·ᵣComm _ _ ∙ cong₂ _·ᵣ_ refl
--      (cong₂ _-ᵣ_
...
invDerivativeℙPos x f f' f'Pos fPos isEquivF incrG incrF  fC gC dF =
  ...
  pp x' u v =
   ...
   qq = cong₂ f (L𝐑.lem--05) (toPathP (squash₁ _ _))
         ∙ cong fst (secEq e _) 
  ...
  subst2 
        ...
        (cong ((f' ·ᵣ_) ∘ -ᵣ_ ∘ fst ∘ invℝ₊)
          (ℝ₊≡ {_ , ℝ₊· (f x x∈ , fPos x x∈) (f (x +ᵣ 0) x+0∈ , fPos _ _)}
            {_ , _} (cong (f x x∈  ·ᵣ_)
              (cong₂ f (+IdR x) (toPathP (squash₁ _ _)))))
           ∙ ·-ᵣ _ _)
            w
 where
...
--   -- IsCauchySequence'-via-~seq
 ...
 slope-monotone-preLn a b a' b'
   ...
   b-a = (_ , x<y→0<y-x _ _ a<b )
   ww : _
   ww (_ , lip·') (_ , lip·) = subst2 _≤ᵣ_ pL pR
         (fromCauchySequence'≤ _ _ _ _ w)

    where
    pL- = map-fromCauchySequence' _ _ _ _ (snd -ᵣR)
    pL· = map-fromCauchySequence' _ _ _ _ lip·'
    pL₂ = mapNE-fromCauchySequence' sumR _ _ _ _
    ...
    pR- = map-fromCauchySequence' _ _ _ _ (snd -ᵣR)
    pR· = map-fromCauchySequence' _ _ _ _ lip·
    pR₂ = mapNE-fromCauchySequence' sumR _ _ _ _
...
-- Invlipschitz-ℝ→ℝ→injective


-- --  -- preLnBdLipℙ : Lipschitz-ℚ→ℝℙ
-- --  --    {!!}
-- --  --    (intervalℙ (rat [ 1 / fromNat (suc (suc Z)) ])
-- --  --       (fromNat (suc (suc Z))))
-- --  --    λ x (1/Z≤ , ≤Z) →
-- --  --       preLn (rat x , isTrans<≤ᵣ _ _ _ (<ℚ→<ᵣ _ _ (ℚ.0<pos _ _)) 1/Z≤) ≤Z 1/Z≤
-- --  -- preLnBdLipℙ q q∈ r r∈ ε x =
-- --  --   isTrans≡<ᵣ _ _ _ w {!!}
