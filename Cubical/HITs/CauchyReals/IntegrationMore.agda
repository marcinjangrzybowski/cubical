...
module IntegrationUC
 ...
 FTOC⇒ : ∀ x₀ (f : ℝ → ℝ) → (ucf : IsUContinuous f) 
                 → (∀ x x₀≤x →
                       derivativeOfℙ (pred≥ x₀) ,
                         (λ x x₀≤x →
                           fst (Integrate-UContinuous x₀ x x₀≤x f ucf))
                         at x , x₀≤x is f x)
 FTOC⇒ x₀ f ucf x x₀≤x ε =
  PT.map (λ (δ , X) → ℚ₊→ℝ₊ δ , ftoc δ X)
   (IsUContinuous-εᵣ f ucf
    (ℚ₊→ℝ₊ ([ 1 / 2 ] , _ ) ₊·ᵣ ε))
  where
  ftoc : ∀ δ → ((u v : ℝ) → u ∼[ δ ] v → absᵣ (f u -ᵣ f v) <ᵣ _)
   → (r : ℝ) (r∈ : r ∈ (λ x₁ → pred≥ x₀ (x +ᵣ x₁)))
      (x＃r : 0 ＃ r) →
      absᵣ (0 -ᵣ r) <ᵣ fst (ℚ₊→ℝ₊ δ) →
      absᵣ
      (f x -ᵣ
  ...
  ftoc δ X h r∈ 0＃h x₁ =
     ...
     (isTrans≡<ᵣ _ _ _
       ...
       (isTrans<≡ᵣ _ _ _
         (isTrans≤<ᵣ _ _ _ (IntegralAbs x'* x* x*∈
            (λ x₂ → f x -ᵣ f x₂)
           (IsUContinuous-ᵣ₂ (λ _ → f x) f (IsUContinuousConst (f x)) ucf) (rat [ 1 / 2 ] ·ᵣ fst ε)
         X')
         (isTrans≡<ᵣ _ _ _
           (sym (·ᵣAssoc _ _ _))
           (<ᵣ-·ᵣo  _ _ (ε ₊·ᵣ (_ , 0<x*-x'*)) decℚ<ᵣ? )))
         (·IdL _ ∙ cong (fst ε ·ᵣ_) x*-x'*≡∣h∣ ∙ ·ᵣComm _ _)))
   ...
   X' : (x' : ℝ) →
         x' ∈ intervalℙ x'* x* →
         absᵣ (f x -ᵣ f x') ≤ᵣ rat [ 1 / 2 ] ·ᵣ fst ε
   X' x' (≤x' , x'≤) =
 ...
 FTOC⇐ : ∀ a b (a<b : a <ᵣ b) (f F : ℝ → ℝ) → ∀ ucf 
           → uDerivativeOfℙ (intervalℙ a b) , (λ x _ → F x) is
               (λ x _ → f x)
           → fst (Integrate-UContinuous a b (<ᵣWeaken≤ᵣ _ _ a<b) f ucf)
               ≡ F a -ᵣ F b
 FTOC⇐ a b a<b f F ucf udc =
   let z = FTOC⇒ a f ucf b (<ᵣWeaken≤ᵣ _ _ a<b)
   in {!!}
