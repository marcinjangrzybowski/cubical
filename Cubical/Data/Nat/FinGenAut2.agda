...
secPlInvPlfun : section plInv plFun
secPlInvPlfun = SC.elim _ _ w
 where


 SplitHlp : ∀ k x p → Σ Type λ T → T 
 SplitHlp k x p =
  let (n , (q , r)) = snd (plFun (inl {n = k} (x , p)))
      n≤k = ¬<-≥ (k) n (λ x₁ → r (k) x₁ p) 
  in (n < k) ⊎ (n ≡ k) , ≤-split {n} {k}  n≤k

 ww' : ∀ k x p → (fst (SplitHlp k x p)) → plInv (plFun (inl {n = k} (x , p))) ≡ inl {n = k} (x , p)
 ww' zero x p _ = refl
 ww' (suc k) x p sh = 
  let (n , (q , r)) = snd (plFun (inl {n = suc k} (x , p)))
      n≤k : n ≤ suc k
      n≤k = ¬<-≥ (suc k) n (λ x₁ → r (suc k) x₁ p) 
  in ⊎.rec 
     (λ n≤k →
      let qq = (λ x₁ x₂ → q x₁ (≤-trans {n} {k} {x₁} n≤k x₂))
          pp = ww' k x qq (snd (SplitHlp k x qq))
          (n' , (q' , r')) = snd (plFun (inl {n = k} (x , qq)))
      in cong plInv (Σ≡Prop (snd ∘ isFinGen≃')
           {u = plFun (inl {n = suc k} (x , p))}
           {v = plFun (inl {n = k} (x , qq))}
           (λ _ → x)) ∙∙ (pp) ∙∙ (push (x , qq) ∙
            cong′ (inl {n = suc k} ∘' (x ,_))
              (snd (constFromIsoH x (suc k)) _ p)

             )
              )
     (λ n=sk i → 
         inl {n = n=sk i} (x ,
         isProp→PathP (λ i → snd (constFromIsoH x (n=sk i)))
             q p i)
             ) sh

 ww : ∀ k x p → plInv (plFun (inl {n = k} (x , p))) ≡ inl {n = k} (x , p)
 ww k x p = ww' k x p (snd (SplitHlp k x p))
 
 wwS : ∀ k x p (q : 
      (plFun (inl {n = k} (x , p))) ≡
      (plFun (inl {n = suc k} (x , (λ x₁ x₂ → p x₁ (<-weaken {k} x₂))))))
      → ∀ sh →
              Square                
              (ww k x p)
              {a₁₁ = inl (x , (λ x₁ x₂ → p x₁ (<-weaken {k} x₂)))}
              (ww' (suc k) x (λ x₁ x₂ → p x₁ (<-weaken {k} x₂)) sh)
              (cong plInv q)
              (push (x , p))
 wwS k x p q (inl x₁) i j =
   let p' = ((λ x₂ x₃ →
             fst (snd (snd (plFun (inl {n = suc k}
                (x , (λ x₄ x₅ → p x₄ (<-weaken {k} x₅)))))))
             x₂ (≤-trans {fst (snd (plFun (inl {n = suc k} (x , (λ x₄ x₅ → p x₄ (<-weaken {k} x₅))))))} {k} {x₂} x₁ x₃)))
       zz = snd (constFromIsoH x k)
         p
         p'
   in hcomp
     (λ l → λ {
       (j = i0) → plInv (isSet→isSet'
                    (isSetΣ (isSet-SetsIso isSetℕ isSetℕ)
                            (λ x₂ → isProp→isSet (snd (isFinGen≃' x₂))))
                    (cong plFun (λ i → (inl {n = k} (x , zz i))))
                    q
                    (λ _ → plFun (inl {n = k} (x , p)))
                    (sym (Σ≡Prop (snd ∘ isFinGen≃')
           {u = plFun (inl {n = suc k} (x , _))}
           {v = plFun (inl {n = k} (x , _))}
           (λ _ → x))) l i)
      ...
      ;(j = i1) →
          hcomp
            (λ l' → λ {
              (l = i0) → inl {n = k} (x ,
                    snd (constFromIsoH x k) p p' i) 
             ;(l = i1) → push {n = k} (x ,
                    
                    isProp→SquareP
                      (λ _ _ → snd (constFromIsoH x k))
                      (λ _ → p)
                      (snd (constFromIsoH x k) p' p)
                      (snd (constFromIsoH x k) p p')
                      (λ _ → p) l' i
                    ) i
             ;(i = i0) → inl {n = k} (x , p)
              })
              (push {n = k} (x , snd (constFromIsoH x k) p p' i) (i ∧ l))
      ;(i = i0) → ww' k x p (snd (SplitHlp k x p)) j 
       })
     (ww' k x (zz i) (snd (SplitHlp k x (zz i))) j)
     
 wwS k x p q (inr x₁) =
  ⊥.rec (
    snd
     (snd (snd (plFun (inl {n = suc k} (x , (λ x₁ x₂ → p x₁ (<-weaken {k} x₂)))))))
     k (subst (k <_) (sym x₁) (≤-refl k)) p)
    
 w : ElimData permSeq (λ z → plInv (plFun z) ≡ z)
 ElimData.finl w {k} (x , p) = ww k x p

 ElimData.fpush w {k} (x , p) =
   wwS k x p (cong plFun (push {n = k} (x , p)))
     ((snd (SplitHlp (suc k) x λ x₁ x₂ → p x₁ (<-weaken {k} x₂))))
...
Iso.fun plIso = plInv
Iso.inv plIso = plFun
Iso.rightInv plIso = secPlInvPlfun      
