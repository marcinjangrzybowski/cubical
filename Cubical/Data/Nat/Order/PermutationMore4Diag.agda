{-# OPTIONS --safe #-}
module Cubical.Data.Nat.Order.PermutationMore4Diag where
...
Flooop-comm-sqL' : ∀ n k l → A.commT (fst k) (fst l)
  → (x : Fin n) →
...
aloopcommm n k i j =
    hcomp (λ l → λ {
    (i = i0) → hcomp (λ l' → λ {
                (l = i0) → (𝕡comm (zero , _) (suc (suc (fst k)) , snd k) _ j (l'))
              ; (l = i1) → 𝕡loop (zero , _) (j ∨ ~ l')                
              ; (j = i0) → 𝕡loop (zero , _) (l ∧ ~ l')
              ; (j = i1) → 𝕡invol (suc (suc (fst k)) , snd k) l' l
              }) (𝕡comp (zero , _) (suc (suc (fst k)) , snd k) j l)
   ;(i = i1) → 𝕡invol (zero , _) (~ l) j
   ;(j = i0) → 𝕡loop (suc (suc (fst k)) , snd k) i
   ;(j = i1) → 𝕡loop (suc (suc (fst k)) , snd k) (i ∨ ~ l)
   }) (hcomp (λ l → λ
         { (i = i0) → 𝕡looop (suc (suc (fst k)) , snd k) (zero , _) j
         ; (i = i1) → 𝕡loop (zero , tt) (~ j ∨ ~ l)
         ; (j = i0) → 𝕡loop (suc (suc (fst k)) , snd k) i
         ; (j = i1) → 𝕡loop (zero , tt) (i ∧ ~ l)
         }) (𝕡comp (suc (suc (fst k)) , snd k) (zero , _) j i ))
...
RElim.hexDiag* RfromFM2⊤' _ _ _ p i = {!!}
...
-- -- toFromFM2⊤-fst : ∀ n → ∀ z → len2 (toFM2⊤ (n , z)) ≡ n
-- -- toFromFM2⊤-fst zero = ?
-- -- toFromFM2⊤-fst (suc n) = ?
-- --  -- ℕ.elim {!!}
-- --  --  {!!}

-- -- RtoFromFM2⊤-fst : ∀ n → R𝕡elimSet {n = suc n} (λ z → len2 (toFM2⊤ (suc n , z)) ≡ suc n)
-- -- R𝕡elimSet.isSetA (RtoFromFM2⊤-fst zero) _ = isProp→isSet (isSetℕ _ _)
-- -- R𝕡elimSet.abase (RtoFromFM2⊤-fst zero) = refl
-- -- R𝕡elimSet.aloop (RtoFromFM2⊤-fst zero) ()
-- -- R𝕡elimSet.alooop (RtoFromFM2⊤-fst zero) ()

-- -- RtoFromFM2⊤-fst (suc n) = w
-- --  where
-- --   h : (x : ℙrm (suc n)) → len2 (toFM2⊤ (suc n , x)) ≡ suc n
-- --   h = R𝕡elimSet.f {n = suc n} (RtoFromFM2⊤-fst n)

-- --   w : R𝕡elimSet (λ z → len2 (toFM2⊤ (suc n , z)) ≡ suc n)
-- --   R𝕡elimSet.isSetA w _ = isProp→isSet (isSetℕ _ _)
-- --   R𝕡elimSet.abase w = ?
-- --   R𝕡elimSet.aloop w (zero , k<) i j = {!!}
-- --   R𝕡elimSet.aloop w (suc k , k<) i j = ?
-- --    -- cong suc (h (𝕡loop (k , k<) i)) j
-- --   R𝕡elimSet.alooop w = {!!}
...
-- --        (λ z →
-- --           fromFM2⊤ (tt ∷2 z) ≡
-- --           (suc (fst (fromFM2⊤ z)) , uncurry sucℙrm (fromFM2⊤ z)))
-- --  RElimSet.[]* w = refl
-- --  w RElimSet.∷* tt = cong (λ (n , p) → suc n , sucℙrm n p)
-- --  RElimSet.comm* w tt tt {xs} X i j =
-- --      hcomp
-- --        (λ l' → λ {
-- --          (i = i0) → cong (λ (n , p) → suc (suc n) ,
-- --                        sucℙrm (suc n) (sucℙrm n p)) X j
-- --          -- (w RElimSet.∷* tt) ((w RElimSet.∷* tt) X) j
-- --         ;(i = i1) → cong (λ (n , p) → suc (suc n) ,
-- --                        sucℙrm (suc n) (sucℙrm n p)) X j
-- --         ;(j = i0) → {!!}
-- --         ;(j = i1) → {!!}
-- --         }) 
...
-- -- -- i = i0 ⊢ (w RElimSet.∷* tt) ((w RElimSet.∷* tt) X) j
-- -- -- i = i1 ⊢ (w RElimSet.∷* tt) ((w RElimSet.∷* tt) X) j
-- -- -- j = i0 ⊢ fromFM2⊤ (tt ∷2 comm tt tt xs i)
-- -- -- j = i1 ⊢ suc (fst (fromFM2⊤ (comm tt tt xs i))) ,
-- -- --          uncurry sucℙrm (fromFM2⊤ (comm tt tt xs i))
...
--         -- (λ l' → λ {
--         --  (i = i0) → loop≡ (suc (suc n)) (zero , _) (~ l') (j)
--         -- ;(i = i1) → loop≡ (suc (suc n)) (zero , _) (~ l') (j)
--         -- ;(j = i0) → loop≡ (suc (suc n)) (zero , _) (~ l') (j)
--         -- ;(j = i1) → (𝕡comp (zero , _) (zero , _) i (~ l'))
--         -- })
--         -- (sucℙrm _ (sucℙrm _ (base≡ n j)))
...
-- looop≡ (suc (suc (suc (suc n)))) (zero , k<) (suc (suc l) , l<) i j =
--    hcomp
--      (λ l' → λ {
--          (i = i1) → (base≡ (4 + n)) j
--         ;(i = i0) → loop≡ (4 + n) (zero , _) (~ l') j
--         ;(j = i1) → {!!} --𝕡comp (0 , _) (suc (suc l) , l<) i (~ l')
--         ;(j = i0) →  {!!}
--         })
--      ( sucℙrm _ (sucℙrm _
--         (loop≡ (suc (suc n)) (l , l<) (~ i) j))) 
...
-- looop≡ (suc n) (suc (suc k) , k<) (zero , l<) = {!!}
...
-- -- -- -- -- -- --  -- ∷𝕍'R : ∀ n → A → R𝕡elim {n = n} λ (p : ℙrm n) → 𝕍 n p → 𝕍 (suc n) (sucℙrm n p) 
-- -- -- -- -- -- --  -- R𝕡elim.isGroupoidA (∷𝕍'R n a) = {!!}
-- -- -- -- -- -- --  -- R𝕡elim.abase (∷𝕍'R n a) = a ,_
-- -- -- -- -- -- --  -- R𝕡elim.aloop (∷𝕍'R (suc (suc n)) a) k i x = {!!}
-- -- -- -- -- -- --  -- R𝕡elim.alooop (∷𝕍'R n a) = {!!}
-- -- -- -- -- -- --  -- R𝕡elim.acomp (∷𝕍'R n a) = {!!}
-- -- -- -- -- -- --  -- R𝕡elim.ainvol (∷𝕍'R n a) = {!!}
-- -- -- -- -- -- --  -- R𝕡elim.acomm (∷𝕍'R n a) = {!!}
-- -- -- -- -- -- --  -- R𝕡elim.abraid (∷𝕍'R n a) = {!!}
