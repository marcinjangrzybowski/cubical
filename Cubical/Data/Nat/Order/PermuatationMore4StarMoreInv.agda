{-# OPTIONS --safe #-}
module Cubical.Data.Nat.Order.PermuatationMore4StarMoreInv where
...
module Iso-𝔽in→-Fin→/↔ {A : Type ℓ} (isGroupoidA : isGroupoid A) where
 ...
 -- zz-aloopP : ∀ n k → 
 --    SquareP (λ i j' → 𝔽in {n = n} (𝕡loop k (~ j')) →
 --                      𝔽in {n = n} (𝕡loop k i))
 -- zz-aloopP = ?
 ...
 zz-aloop : ∀ n k → 
    SquareP (λ i j' → 𝔽in {n = n} (𝕡loop k (~ j')) →
                      𝔽in {n = n} (𝕡loop k i))
    {idfun _}
    (symP (ua-ungluePathExt (adjT'≃ {n = n} k)))
    -- (ua-gluePathExt (adjT'≃ {n = n} k))
    {_} {idfun _}
    {!ua-ungluePathExt (adjT'≃ {n = n} k)!}
    -- (λ i x → glue
    ...
    --      ∘' ua-ungluePathExt (adjT'≃ {n = n} k) i))
    -- (ua-ungluePathExt (adjT'≃ {n = n} k))
 zz-aloop n k = isSet→SquareP (λ i j' → isSet→ (snd (𝔽inH n (𝕡loop k i))))
           _ _ _ _
