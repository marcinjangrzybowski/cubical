...
module Cubical.Data.FinData.GTrun where
...
-- --   squash// : ∀ (x y : A // Rt) (p q : x ≡ y) (r s : p ≡ q) → r ≡ s
 ...
 data ZZ : Type (ℓ-max ℓ ℓb) where
  [_]z : (A → B) → ZZ
  loopz : (e : A ≃ A) → PathP (λ i → (ua e i → B) → ZZ) [_]z [_]z
  compz : (e f : A ≃ A) → SquareP (λ i j → (ua-CompFill∙ₑ e f i j → B) → ZZ)
                             (loopz e)
                             (loopz (e ∙ₑ f))
                             refl
                             (loopz f)
 ...
 Rrec.feq rZZfrom// {a = a} {b = b} (↔s F≃ l≡) =
   cong [_]z (sym (funExt ((cong a ∘ (retEq F≃)))))
    ∙' λ i → loopz F≃ i (l≡ i ∘ ua-ungluePathExt F≃ i)
     

 Rrec.fsq rZZfrom// (↔s F≃ f≡) (↔s G≃ g≡) i =
   {!!} ∙∙
     (λ j → compz F≃ G≃ i j {!!})
   -- (λ j → loopz (equivEq {e = F≃} {f = F≃ ∙ₑ
   --     {!invEquiv (unglueEquiv A (~ i ∨ i) (λ { (i = i0) → (_ , G≃)
   --                                 ; (i = i1) → (_ , idEquiv _) }))!}} {!!} i) j {!!})
   ∙∙ {!!}
