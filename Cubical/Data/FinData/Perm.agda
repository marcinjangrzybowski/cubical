...
module Cubical.Data.FinData.Perm where
...
open import Cubical.Data.Graph as G
...
record _↔_ {ℓ} {n : ℕ} {A : Type ℓ} (x y : Fin n → A) : Type ℓ where
  constructor prm
  field
    ↔i : Iso (Fin n) (Fin n)
    ↔p : ∀ k → (x k) ≡ y (fun ↔i k)
...
isRefl↔ : BinaryRelation.isRefl (_↔_ {n = n} {A = A})
↔i (isRefl↔ a) = idIso 
↔p (isRefl↔ a) k = refl

isSym↔ : BinaryRelation.isSym (_↔_ {n = n} {A = A})
↔i (isSym↔ a b (prm i _)) = invIso i 
↔p (isSym↔ a b (prm i p)) k = cong b (sym (rightInv i k)) ∙ sym (p _)

isTrans↔ : BinaryRelation.isTrans (_↔_ {n = n} {A = A})
↔i (isTrans↔ a b c (prm i p) (prm j q)) = compIso i j
↔p (isTrans↔ a b c (prm i p) (prm j q)) k = p _ ∙ q _


lengthFM : FMSet A → ℕ
lengthFM = Rec.f isSetℕ zero (λ _ → suc) λ _ _ _ → refl

F→→FMSet : (Fin n → A) → Σ (FMSet A) λ x → lengthFM x ≡ n
F→→FMSet {zero} {A = A} _ = []fm , refl
F→→FMSet {suc n} {A = A} f =
 let (l , p) = F→→FMSet (f ∘ suc)
 in f zero ∷fm l , cong suc p

F→/↔ : ℕ → (A : Type ℓ) → Type ℓ
F→/↔ n A = (Fin n → A) / _↔_ 

FMVec : ℕ → (A : Type ℓ) → Type ℓ
FMVec n A = Σ (FMSet A) λ x → lengthFM x ≡ n


insertAt : ∀ {n} → A → Fin (suc n) → (Fin n → A) → (Fin (suc n) → A)
insertAt a zero f zero = a
insertAt a zero f (suc x) = f x
insertAt {n = suc n} a (suc k) f zero = f zero
insertAt {n = suc n} a (suc k) f (suc x) = insertAt a k (f ∘ suc) x

removeAt : ∀ {n} → Fin (suc n) → (Fin (suc n) → A) → (Fin n → A)
removeAt zero f = f ∘ suc
removeAt (suc k) f zero = f zero
removeAt (suc k) f (suc x) = removeAt k (f ∘ suc) x

insertAt-insertAt : ∀ {n} → (k : Fin (suc n)) → (f : Fin (suc n) → A)
   → insertAt (f k) k (removeAt k f) ≡ f
insertAt-insertAt zero f i zero = f zero
insertAt-insertAt zero f i (suc x₁) = f (suc x₁)
insertAt-insertAt {n = suc n} (suc k) f i zero = f zero
insertAt-insertAt {n = suc n} (suc k) f i (suc x₁) =
  insertAt-insertAt {n = n} k (f ∘ suc) i x₁



-- _↔'_ : ∀ {ℓ} {n : ℕ} {A : Type ℓ} (x y : Fin n → A) → Type ℓ
-- _↔'_ {n = zero} _ _ = Unit*
-- _↔'_ {n = one} x y = x zero ≡ y zero
-- _↔'_ {n = suc (suc n)} x y =
--   {!!}
--     × Σ (Fin (suc (suc n)) × Fin (suc (suc n)))
--     λ (k , k') → (x zero ≡ y k) × (y zero ≡ x k')  
...
popTransposition : ∀ n → (e : Iso (Fin (suc n)) (Fin (suc n)))
                     → Σ (Iso (Fin n) (Fin n)) λ e'
                       → e ≡ compIso (transposeHead' (Iso.fun e zero)) (sucPermIso e')
                        
popTransposition = {!!}


fromTranspositionString : ∀ {n} → List (Fin n × Fin n) → Iso (Fin n) (Fin n)
fromTranspositionString [] = idIso
fromTranspositionString ((l , k) ∷ x₁) = compIso (transposition l k) (fromTranspositionString x₁) 

toTranspositionString : ∀ {n} → Iso (Fin n) (Fin n) → List (Fin n × Fin n)
toTranspositionString {n = zero} _ = []
toTranspositionString {n = suc n} e =
  let (e' , p') = (popTransposition _ e)
  in (zero , (Iso.fun e zero)) ∷ mapList (map-× suc suc) (toTranspositionString e') 

toTranspositionString≡ : ∀ {n} → (e : Iso (Fin n) (Fin n)) →
     e ≡ fromTranspositionString (toTranspositionString e) 
toTranspositionString≡ {zero} e = SetsIso≡-ext isSetFin isSetFin (⊥.rec ∘ ¬Fin0) (⊥.rec ∘ ¬Fin0)
toTranspositionString≡ {suc n} e =
  let (e' , p') = (popTransposition _ e)
  in p' ∙ cong (compIso (transposeHead' (fun e zero)))
      (cong (sucPermIso) (toTranspositionString≡ e') ∙
        {!!})

insertAt'FM : (x : A) → (xs : FMSet A) → Fin (lengthFM (xs)) → singl (x ∷fm xs) 
insertAt'FM x = 
  ElimProp.f
    (isPropΠ λ _ → isContr→isProp (isContrSingl _)) --(⊥.elim ∘ ¬Fin0)
    ((⊥.elim ∘ ¬Fin0))

     (λ y {xs} f → λ { zero → _ , (comm _ _ _)
                   ; (suc k) → _ , comm _ _ _ ∙ cong (y ∷fm_) (snd (f k)) })


transpositionFM≡ : ∀ {n} → (x : A) → (k k' : Fin (suc n)) → (f : Fin n → A)
                            →  F→→FMSet (insertAt x k f)
                               ≡ F→→FMSet (insertAt x k' f)
transpositionFM≡ x zero k' f = {!k' !}
transpositionFM≡ x (suc k) k' f = {!!}

_↔T_ : ∀ {ℓ} {n : ℕ} {A : Type ℓ} (x y : Fin n → A) → Type ℓ
_↔T_ {n = zero} _ _ = Unit*
_↔T_ {n = suc n} x y = Σ (Fin (suc n) × Fin (suc n))
              λ (k , k') → (x k ≡ y k') × (removeAt k x ↔T removeAt k' y)

_↔T'_ : ∀ {ℓ} {n : ℕ} {A : Type ℓ} (x y : Fin n → A) → Type ℓ
_↔T'_ {n = zero} _ _ = Unit*
_↔T'_ {n = suc n} x y = Σ (Fin (suc n) × Fin (suc n))
              λ (k , k') → (x k ≡ y k') × (removeAt k x ≡ removeAt k' y)

-- _↔T'FM_ : ∀ {ℓ} {n : ℕ} {A : Type ℓ} (x y : FMVec n A) → Type ℓ
-- _↔T'FM_ = {!!}

TranspositionsGraph : (A : Type ℓ) → ∀ n → G.Graph ℓ ℓ
TranspositionsGraph {ℓ = ℓ} A n =
  record { Node = Fin n → A
           ; Edge = _↔T'_ }

toTranspositionStringTG : ∀ {n} → (x y : Fin n → A) → x ↔ y
   → G.Path (TranspositionsGraph A n) x y
toTranspositionStringTG {n = zero} x y _ = G.pcons G.pnil tt*
toTranspositionStringTG {n = suc n} x y (prm e p) =
 let (e' , p') = (popTransposition _ e)
     zz = toTranspositionStringTG {n = n} (x ∘ suc) (removeAt (fun e zero) y)
          (prm e' (λ k → p (suc k) ∙ cong y (funExt⁻ (cong (Iso.fun) p') (suc k))
              ∙ {!!} ))
 in G.pcons {w = insertAt (x zero) zero (removeAt (fun e zero) y)}
      {!zz!}
       ((zero , fun e zero) , (p zero , λ _ → removeAt (fun e zero) y))
       

insertAt'FM≡ : ∀ {n} → (x : A) → (xs :  Fin n → A) → (k k' : Fin (suc n)) →
                F→→FMSet (insertAt x k xs) ≡
                F→→FMSet (insertAt x k' xs)
insertAt'FM≡ = {!!}

-- insertAt'FM≡' : (x : A) → (xs : FMSet A) → (k k' : Fin (lengthFM xs)) →
--                 fst (insertAt'FM x xs k) ≡
--                 fst (insertAt'FM x xs k')
-- insertAt'FM≡' = {!!}

toFMP : ∀ n → GraphHom (TranspositionsGraph A n) (≡Graph (FMVec n A))
_$_ (toFMP n) = F→→FMSet
_<$>_ (toFMP zero) = λ _ → refl
toFMP (suc n) <$> ((k , k') , (p , p')) = 
  
       (cong F→→FMSet (sym (insertAt-insertAt k _) ) ∙∙
         (λ i → insertAt'FM≡ (p i) (p' i) k k' i)
          ∙∙ cong F→→FMSet (insertAt-insertAt k' _))
...
--        ({!!} ∙∙ ((λ i → insertAt'FM≡' (p i) (fst (zz i)) {!k!} {!k'!} i)) ∙∙ {!!})

-- 
-- TranspositionsGraph : (A : Type ℓ) → ∀ n → G.Graph ℓ ℓ
-- TranspositionsGraph {ℓ = ℓ} A n =
--   record { Node = Fin n → A
--            ; Edge = λ x y → Σ (Fin n × Fin n)
--               λ (k , k') → ∀ j → x j ≡ y (Iso.fun (transposition k k') j)   }


-- toFMP : ∀ n → GraphHom (TranspositionsGraph A n) (≡Graph (FMVec n A))
-- _$_ (toFMP n) = F→→FMSet
-- toFMP .(suc _) <$> ((zero , k') , p) = {!!}
-- toFMP .(suc _) <$> ((suc k , zero) , p) = {!!}
-- toFMP .(suc _) <$> ((suc k , suc k') , p) =
--   Σ≡Prop (λ _ → isSetℕ _ _)
--      (cong₂ _∷fm_ (p zero) (cong fst (toFMP _ <$> ((k , k') , p ∘ suc)) ))


-- DiagMor

-- insertAt'FM≡ : ∀ {n} → (x x' : A) → (xs :  Fin n → A) → (k : Fin (suc n)) (k' : Fin (suc (suc n))) →
--                 F→→FMSet (insertAt x' k' (insertAt x k  xs)) ≡
--                 F→→FMSet (insertAt x k' (insertAt x' k  xs))
-- insertAt'FM≡ = {!!}



-- F→→FMSetTrans : (a b : Fin (suc n) → A) →
--       a ↔ b →
--       (a zero ∷fm fst (F→→FMSet (λ x → a (suc x))))
--       ≡
--       (b zero ∷fm fst (F→→FMSet (λ x → b (suc x))))
-- F→→FMSetTrans {zero} a b (prm i p) with (fun i zero) | p zero
-- ... | zero | p' = cong (_∷fm []fm) p' 
-- F→→FMSetTrans {suc n} a b (prm i p) =
--   let (i' , p') = (popTransposition _ i)
--   in cong (a zero ∷fm_) (F→→FMSetTrans {n} (a ∘ suc) {!!}
--        (prm i' λ k → {!!}))
--       ∙ cong₂ _∷fm_ {!!} {!!}
--       ∙ {!!}
      
--     -- cong (b zero ∷fm_) (F→→FMSetTrans {n} {!!} (b ∘ suc) (prm i' {! p ∘ suc!}))

-- -- fromFM' : (xs : FMSet A) → (Fin (lengthFM xs) → A) / _↔_
-- -- fromFM' =
-- --   FM.Elim.f
-- --    [ (⊥.elim ∘ ¬Fin0) ]/
-- --    w
-- --    (λ _ _ → SQ.elimProp (λ _ → squash/ _ _)
-- --      λ f → eq/ _ _ (prm swap0and1Iso λ { zero → refl ; one → refl ; (suc (suc k)) → refl }))
-- --    λ _ → squash/

-- --    where
-- --      w : ∀ x {xs : FMSet _} →
-- --            (Fin (lengthFM xs) → _) / _↔_ →
-- --            (Fin (suc (lengthFM xs)) → _) / _↔_
-- --      w x {xs} = SQ.rec squash/
-- --       ([_]/ ∘ ww) www
-- --        where
-- --          ww : _
-- --          ww _ zero = x
...
-- --          www : _
-- --          www a b (prm i p) =
-- --           eq/ (ww a ) (ww b) (prm (sucPermIso i) λ { zero → refl ; (suc k) → p k })


-- -- -- IsoF→/↔FM : Iso (F→/↔ n A) (FMVec n A)
-- -- -- IsoF→/↔FM {zero} = w
-- -- --   where
-- -- --     w : Iso (F→/↔ zero _) (FMVec zero _)
-- -- --     fun w _ = []fm , refl
-- -- --     inv w _ = [ (⊥.elim ∘ ¬Fin0) ]/
-- -- --     rightInv w b = Σ≡Prop (λ x → isSetℕ _ _) {!!} 
-- -- --     leftInv w = {!!}
...
-- -- --     module PI = Iso (IsoF→/↔FM {n = n} {A = A})

-- -- --     toFM = SQ.rec (isSetΣ trunc (λ _ → isProp→isSet (isSetℕ _ _)))
-- -- --       F→→FMSet λ a b r → Σ≡Prop (λ x → isSetℕ _ _) (F→→FMSetTrans a b r)


-- -- --     fromFM : (xs : FMSet A) (x : lengthFM xs ≡ suc n) → (Fin (suc n) → A) / _↔_
-- -- --     fromFM xs x = {!fromFM' xs!}


-- -- --     w : Iso (F→/↔ (suc n) _) (FMVec (suc n) _)
-- -- --     fun w = toFM
-- -- --     inv w = uncurry fromFM
-- -- --     rightInv w = {!!}
-- -- --     leftInv w = {!!}
...
-- -- -- -- tabulate∘' : ∀ {n m} → (e : Fin n → A)
-- -- -- --                (g : Fin m → Fin (length (tabulate n e)))  →
-- -- -- --                 tabulate m (lookup (tabulate n e) ∘ g)  ≡
-- -- -- --                   tabulate m (e ∘ subst Fin (length-tabulate _ _) ∘ g)

-- -- -- -- tabulate∘' {m = zero} _ _ = refl
-- -- -- -- tabulate∘' {n = n} {m = suc m} e g =
-- -- -- --   cong {x = lookup (tabulate n e)}
-- -- -- --        {y = e ∘ subst Fin (length-tabulate _ _)}
-- -- -- --        (λ f → tabulate (suc m) (f ∘ g))
-- -- -- --     (sym (funExt
-- -- -- --       λ x → fromPathP
-- -- -- --         λ i → lookup-tabulate n e (~ i) (coe1→i (λ j → Fin (length-tabulate n e (~ j))) i x ))
...
-- -- -- --   -- _++//_ = GQ.rec isTrans↔
-- -- -- --   --    (isGroupoidΠ λ _ → squash//)
-- -- -- --   --    (foldr (λ a → (a ∷//_) ∘_) (idfun _))
-- -- -- --   --    (λ {a} {b} r → {!!})
-- -- -- --   --    {!!} 
...
-- -- -- --     permute→FMSet2≡M : (xs : List A) → ∀ {n} → (e : Fin n ≃ Fin (length xs))
-- -- -- --          → List A
-- -- -- --     permute→FMSet2≡M [] {zero} e = []
-- -- -- --     permute→FMSet2≡M [] {suc n} e = ⊥.rec (¬Fin0 (fst e zero))
-- -- -- --     permute→FMSet2≡M (x ∷ xs) {zero} e = ⊥.rec (¬Fin0 (invEq e zero))
-- -- -- --     permute→FMSet2≡M (x ∷ xs) {suc n} eP =
-- -- -- --      let r@(prm e _) = ↔permute (x ∷ xs) eP       
-- -- -- --          (e' , _) = unwindPermHead e
-- -- -- --          k' = equivFun e zero
-- -- -- --          xs' = permute xs (invEquiv e')
-- -- -- --          pL = sym (length-tabulate _ (lookup (x ∷ xs) ∘ invEq (sucPerm e')))
-- -- -- --      in permute (x ∷ xs') (invEquiv (rot≃ (subst Fin pL k')))

-- -- -- --     permute→FMSet2≡L : (xs : List A) → ∀ {n} → (e : Fin n ≃ Fin (length xs))
-- -- -- --          → List→FMSet2 xs ≡ List→FMSet2 (permute→FMSet2≡M xs {n} e)
-- -- -- --     permute→FMSet2≡L [] {zero} e = refl
-- -- -- --     permute→FMSet2≡L [] {suc n} e = ⊥.rec (¬Fin0 (fst e zero))
-- -- -- --     permute→FMSet2≡L (x ∷ xs) {zero} e = ⊥.rec (¬Fin0 (invEq e zero))
-- -- -- --     permute→FMSet2≡L (x ∷ xs) {suc n} eP =
-- -- -- --      let r@(prm e _) = ↔permute (x ∷ xs) eP       
-- -- -- --          (e' , _) = unwindPermHead e
-- -- -- --          k' = equivFun e zero
-- -- -- --          xs' = permute xs (invEquiv e')        
...
-- -- -- --     permute→FMSet2≡R : (xs : List A) → ∀ {n} → (e : Fin n ≃ Fin (length xs))
-- -- -- --          → permute→FMSet2≡M xs {n} e ≡ permute xs e
-- -- -- --     permute→FMSet2≡R [] {zero} e = refl
-- -- -- --     permute→FMSet2≡R [] {suc n} e = ⊥.rec (¬Fin0 (fst e zero))
-- -- -- --     permute→FMSet2≡R (x ∷ xs) {zero} e = ⊥.rec (¬Fin0 (invEq e zero))
-- -- -- --     permute→FMSet2≡R (x ∷ xs) {suc n} eP =
-- -- -- --      let r@(prm e _) = ↔permute (x ∷ xs) eP       
...
-- -- -- --     permute→FMSet2≡ xs e =
-- -- -- --       permute→FMSet2≡L xs e
-- -- -- --        ∙ cong List→FMSet2 (permute→FMSet2≡R xs e)
...
-- -- -- --        in permute→FMSet2≡ a (invEquiv (F≃ r)) ∙ cong List→FMSet2 (fst r')
...
-- -- -- --     -- ↔→FMSet≡2TransEq : (a b c : List A) → (p : a ↔ b) → (q : b ↔ c)
-- -- -- --     --   → permute→FMSet2≡M (permute→FMSet2≡M a (invEquiv (F≃ p)))
-- -- -- --     --      (fst (↔→FMSet2≡COMM _ (permute→FMSet2≡M a (invEquiv (F≃ p))) b
-- -- -- --     --                        (permute→FMSet2≡R a (invEquiv (F≃ p))
-- -- -- --     --                           ∙ (cong (tabulate (length b)) (funExt (l≡ p ∘ (invEq (F≃ p)))))
-- -- -- --     --                           ∙ (cong (tabulate (length b) ∘ (lookup b ∘_)) (funExt (secEq (F≃ p))))
-- -- -- --     --                               ∙ tabulate-lookup b)
-- -- -- --     --                        (invEquiv (F≃ q))))
-- -- -- --     --      ≡ permute→FMSet2≡M a (invEquiv (F≃ p ∙ₑ F≃ q))

-- -- -- --     -- ↔→FMSet≡2TransEq [] [] [] _ _ = refl
-- -- -- --     -- ↔→FMSet≡2TransEq [] [] (_ ∷ _) _ q = ⊥.rec (¬nil↔cons q)
-- -- -- --     -- ↔→FMSet≡2TransEq [] (_ ∷ _) _ p _ = ⊥.rec (¬nil↔cons p)
-- -- -- --     -- ↔→FMSet≡2TransEq (_ ∷ _) [] _ p _ = ⊥.rec (¬cons↔nil p)
-- -- -- --     -- ↔→FMSet≡2TransEq (_ ∷ _) (_ ∷ _) [] _ q = ⊥.rec (¬cons↔nil q)
-- -- -- --     -- ↔→FMSet≡2TransEq a@(x ∷ xs) b@(y ∷ ys) c@(z ∷ zs) p q = 
-- -- -- --     --   tabulate∘' {!!} {!!}
-- -- -- --     --      ∙ {!!} 
                       


-- -- -- --   -- --   -- ↔→FMSet≡2TransSq : (a b c : List A) → (p : a ↔ b) → (q : b ↔ c)
-- -- -- --   -- --   --   → let a' = permute→FMSet2≡M a (invEquiv (F≃ p))
-- -- -- --   -- --   --         (z , zz , P) = ↔→FMSet2≡COMM _ a' b
-- -- -- --   -- --   --                        (permute→FMSet2≡R a (invEquiv (F≃ p))
-- -- -- --   -- --   --                           ∙ (cong (tabulate (length b)) (funExt (l≡ p ∘ (invEq (F≃ p)))))
-- -- -- --   -- --   --                           ∙ (cong (tabulate (length b) ∘ (lookup b ∘_)) (funExt (secEq (F≃ p))))
-- -- -- --   -- --   --                               ∙ tabulate-lookup b)
-- -- -- --   -- --   --                        (invEquiv (F≃ q))
-- -- -- --   -- --   --         h0 = permute→FMSet2≡L a (invEquiv (F≃ p))
-- -- -- --   -- --   --         h1 = permute→FMSet2≡L (permute→FMSet2≡M a (invEquiv (F≃ p))) z

-- -- -- --   -- --   --     in {!!}

                       
-- -- -- --   -- --   -- ↔→FMSet≡2TransSq a b c p q = {!!}
...
-- -- -- --   -- --         aM = permute→FMSet2≡M (permute→FMSet2≡M a (invEquiv (F≃ p))) z
          
-- -- -- --   -- --         a== : aM ≡ permute→FMSet2≡M a (invEquiv (F≃ p ∙ₑ F≃ q))
-- -- -- --   -- --         a== = {!!}


-- -- -- --   -- --         sqMain : Square
-- -- -- --   -- --                     (h0 ∙ h1)
-- -- -- --   -- --                     (permute→FMSet2≡L a (invEquiv (F≃ p ∙ₑ F≃ q)))
-- -- -- --   -- --                     refl
-- -- -- --   -- --                     (cong List→FMSet2 a==)
-- -- -- --   -- --         sqMain = {!!}
...
-- -- -- --   -- --     in  
-- -- -- --   -- --         _ ≡⟨ cong (_∙ (↔→FMSet2≡ b c q))
-- -- -- --   -- --                (sym (assoc  _ _ _)
-- -- -- --   -- --                  ∙ cong (permute→FMSet2≡L a _ ∙_)
-- -- -- --   -- --                     (sym (cong-∙ List→FMSet2 _ _)) ) ⟩
-- -- -- --   -- --         _ ≡⟨ sym (assoc _ _ _) ⟩
-- -- -- --   -- --         _ ≡⟨ cong (permute→FMSet2≡L a _ ∙_)
-- -- -- --   -- --               (assoc _ _ _ ∙ cong (_∙ (cong List→FMSet2 (fst q')))
-- -- -- --   -- --                 (P ∙ sym (assoc h00 _ _)
-- -- -- --   -- --                   ∙ cong (h00 ∙_) (sym (cong-∙ List→FMSet2 _ _))) ∙
-- -- -- --   -- --                sym (assoc _ _ _)) ⟩
-- -- -- --   -- --         -- _ ≡⟨ {!!} ⟩
-- -- -- --   -- --         _ ≡⟨ assoc h0 h1 _ ∙ cong ((h0 ∙ h1) ∙_) (sym (cong-∙ {x = aM} List→FMSet2 h4 h5)) ⟩
-- -- -- --   -- --         _ ≡⟨
-- -- -- --   -- --                -- (λ i j → hcomp (λ k → λ { (j = i0) → List→FMSet2 a
-- -- -- --   -- --                --         ; (j = i1) →
-- -- -- --   -- --                --             List→FMSet2
-- -- -- --   -- --                --               (isSet→isSet' (isSetLA)
-- -- -- --   -- --                --                (h4 ∙ h5)
...
-- -- -- --   -- --                         {!!}
-- -- -- --   -- --                               ⟩
-- -- -- --   -- --         _ ≡⟨ cong (permute→FMSet2≡L a (invEquiv (F≃ p ∙ₑ F≃ q)) ∙_)
-- -- -- --   -- --                (cong-∙ List→FMSet2 h2 h3) ⟩
-- -- -- --   -- --         _ ≡⟨⟩ (assoc _ _ _)
-- -- -- --   -- --    where
-- -- -- --   -- --          p' = factor↔Σ' _ _ p
-- -- -- --   -- --          q' = factor↔Σ' _ _ q
