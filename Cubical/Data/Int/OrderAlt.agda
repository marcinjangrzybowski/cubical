{-# OPTIONS --safe #-}
module Cubical.Data.Int.OrderAlt where
...
isProp≤ {m} {n} (k , p) (l , q)
  ...
  where
    lemma : k ≡ l
    lemma = injPos (inj-z+ {m} {pos k} (p ∙ sym q))

isProp< : isProp (m < n)
isProp< {m} {n} = isProp≤  {sucℤ m} {n}

zero-≤pos : 0 ≤ pos l
zero-≤pos {l} = _ , refl

¬-pos<-zero : ¬ (pos k) < 0
¬-pos<-zero {k} (i , p) = snotz (injPos (p))

negsuc<-zero : negsuc k < 0
negsuc<-zero {k} = k , cong (_+ pos k) (sucℤ[negsuc]-pos k) ∙ +InvL (pos k)  


¬pos≤negsuc : ¬ (pos k) ≤ negsuc l
¬pos≤negsuc {k} {l} (i , p) = posNotnegsuc (k ℕ.+ i) l (p)

negsuc<pos : negsuc k < pos l
negsuc<pos {zero} {zero}   = 0 , refl
negsuc<pos {zero} {suc l}  = suc l , refl -- sym ( (pos (suc l)))
