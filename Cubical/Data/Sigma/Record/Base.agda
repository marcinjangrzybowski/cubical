...
Sig*-subst' : ∀ {ℓ} → ∀ {n} → (v₁ v₂ : Vec Interval n)
                   → Sig* ℓ v₁ → Sig* ℓ v₂  
Sig*-subst' {n = zero} v₁ v₂ x = _
Sig*-subst' (x₁ ∷ []) (x₂ ∷ []) x = x
Sig*-subst' {n = suc (suc zero)} (zero ∷ _ ∷ []) (zero ∷ _ ∷ []) x = x
Sig*-subst' {n = suc (suc zero)} (zero ∷ _ ∷ []) (one ∷ _ ∷ []) x = x
Sig*-subst' {n = suc (suc zero)} (zero ∷ _ ∷ []) (seg i ∷ _ ∷ []) x = x
Sig*-subst' {n = suc (suc zero)} (one ∷ _ ∷ []) (zero ∷ _ ∷ []) x = x
Sig*-subst' {n = suc (suc zero)} (one ∷ _ ∷ []) (one ∷ _ ∷ []) x = x
Sig*-subst' {n = suc (suc zero)} (one ∷ _ ∷ []) (seg i ∷ _ ∷ []) x = x
Sig*-subst' {n = suc (suc zero)} (seg i ∷ _ ∷ []) (zero ∷ _ ∷ []) x = x
Sig*-subst' {n = suc (suc zero)} (seg i ∷ _ ∷ []) (one ∷ _ ∷ []) x = x
Sig*-subst' {n = suc (suc zero)} (seg i ∷ _ ∷ []) (seg i₁ ∷ _ ∷ []) x = x
Sig*-subst' {n = suc (suc n)} (j₁ ∷ x₃ ∷ x₅ ∷ v₁) (j₂ ∷ x₄ ∷ x₆ ∷ v₂) x =
     I-elim _ ( 
      λ i₁ → I-elim (λ i₂ → Sig* _ (i₂ ∷ x₄ ∷ x₆ ∷ v₂ ))
             (λ i₂ → {!!}
             ) j₂
      ) j₁ 
