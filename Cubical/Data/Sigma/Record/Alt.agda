...
RecordImplementation-Iso : ∀ {ℓ} → (ri₁ ri₂ : RecordImplementation ℓ) → Type (ℓ-suc ℓ)
RecordImplementation-Iso {ℓ} ri₁ ri₂ =
  Σ (Iso (fst ri₁) (fst ri₂))
  λ (iso to from ri li) →
     Σ ( ∀ (s₁ : fst ri₁) → Iso (snd ri₁ s₁) (snd ri₂ (to s₁)))
  λ r-iso₁ →
    Σ ( ∀ (s₂ : fst ri₂) → Iso (snd ri₁ (from s₂)) (snd ri₂ s₂))
    λ r-iso₂ → Unit
...
RB : ∀ {ℓ} → ∀ {n} → Vec Bool n → RecordImplementation ℓ
RB {n = zero} _ = Lift Unit , λ _ → Lift Unit
RB {n = suc zero} x = Type _ , λ  Ty → Ty
RB {n = suc (suc n)} (false ∷ v) = RI↑-L (RB v)
RB {n = suc (suc n)} (true ∷ v) = RI↑-R (RB v)


RI↑-R-iso : ∀ {ℓ} → (ri₁ ri₂ : RecordImplementation ℓ)
               → RecordImplementation-Iso ri₁ ri₂
               → RecordImplementation-Iso (RI↑-R ri₁) (RI↑-R ri₂)
fst (RI↑-R-iso ri₁ ri₂ x) =
 let (iso to fr li ri , iso1 , iso2 , _) = x

  in iso
        (λ x → to (fst x) , (snd x ∘ Iso.inv (iso1 (fst x))))
        (λ x → fr (fst x) , (snd x ∘ Iso.fun (iso2 (fst x))))
        (λ b i → (li (fst b) i) , λ x → {!   !})
        λ a i → ri (fst a) i , λ x → {!!}


fst (snd (RI↑-R-iso ri₁ ri₂ x)) s₁ =
 let (iso to fr li ri , iso1 , iso2 , _) = x
     (iso to' fr' li' ri') = iso1 (fst s₁)
  in 
  iso
   (λ x₁ → to' (fst x₁) , {!(snd x₁)!})
   (λ x₁ → fr' (fst x₁) , snd x₁)
...
fst (snd (snd (RI↑-R-iso ri₁ ri₂ x))) s₂ =
   let (iso to fr li ri , iso1 , iso2 , _) = x
       (iso to' fr' li' ri') = iso2 (fst s₂)
  in 
  iso
   (λ x₁ → to' (fst x₁) , (snd x₁))
   (λ x₁ → fr' (fst x₁) , {!!})
...
snd (snd (snd (RI↑-R-iso ri₁ ri₂ x))) = {!!}

RB-LR : ∀ {ℓ} → ∀ n → ∀ v₁ v₂
      → RecordImplementation-Iso
        (RB {ℓ} {n = n} v₁)  (RB {ℓ} {n = n} v₂)  
RB-LR zero v₁ v₂ = idIso , (const idIso) , (const idIso) , _
RB-LR (suc zero) v₁ v₂ = idIso , (λ s₁ → idIso) , (λ s₁ → idIso) , _
RB-LR (suc (suc zero)) (false ∷ v₁) (false ∷ v₂) = idIso , (λ s₁ → idIso) , (λ s₁ → idIso) , _
RB-LR (suc (suc zero)) (false ∷ v₁) (true ∷ v₂) = idIso , (λ s₁ → idIso) , (λ s₁ → idIso) , _
RB-LR (suc (suc zero)) (true ∷ v₁) (false ∷ v₂) = idIso , (λ s₁ → idIso) , (λ s₁ → idIso) , _
RB-LR (suc (suc zero)) (true ∷ v₁) (true ∷ v₂) = idIso , (λ s₁ → idIso) , (λ s₁ → idIso) , _



RB-LR (suc (suc (suc n))) (true ∷ v₁) (true ∷ v₂) = RI↑-R-iso _ _ (RB-LR _ v₁ v₂)
RB-LR (suc (suc (suc n))) (_ ∷ v₁) (_ ∷ v₂) = {!!}

-- let (iso to fr li ri , iso1 , iso2 , _) = RB-LR (suc (suc n)) v₁ v₂
