...
module Cubical.Data.Sigma.Nested.NestedPrim where
...
module _ {ℓ} {A B : Type (ℓ-suc ℓ)} (A≃B : A ≃ B) (Ar : A → Type ℓ) (Br : B → Type ℓ) where
  ...
  over-≃-fun' x i ab = hcomp (λ j → λ
                            { (i = i0) → Ar (((λ i₁ → snd A≃B .equiv-proof (transp (λ i₂ → B) i₁ (fst A≃B ab)) .fst .fst) ∙ (retEq A≃B ab)) j)
                            ; (i = i1) → Br ab
                         })
                         (ua (x (coei→1 (λ k → ua A≃B k) i ab)) i)
...
module _ {ℓ} {A B C : Type (ℓ-suc ℓ)} (Ar : A → Type ℓ) (Br : B → Type ℓ) (Cr : C → Type ℓ) where
  assoc-Σ-curry :  Iso (Σ (Σ A λ a → Ar a → B) λ x → Σ (Ar (fst x)) (λ x₁ → Br (snd x x₁)) → C)
                        (Σ A λ a → Ar a → Σ B λ x → Br x → C) 
  Iso.fun assoc-Σ-curry s = (fst (fst s)) , λ x → (snd (fst s)) x , λ x₁ → snd s (_ , x₁)
  Iso.inv assoc-Σ-curry s = (fst s , λ x → fst (snd s x)) , λ y → snd (snd s (fst y)) (snd y)
  Iso.rightInv assoc-Σ-curry _ = refl
  Iso.leftInv assoc-Σ-curry _ = refl
  ...
  assoc-Σ-curryP'≃ :   ((a : _) → Ty0 a)
                      ≃
                        (((a : _) → Ty1 a)) 
  assoc-Σ-curryP'≃ = (equivΠ (isoToEquiv (assoc-Σ-curry)) (λ _ → Σ-assoc-≃))



  assoc-Σ-curryΣ : ∀ a → Ty0 a ≃ Ty1 (Iso.fun assoc-Σ-curry a)  
  assoc-Σ-curryΣ a = Σ-assoc-≃

  assoc-Σ-curryΣ- : ∀ a → Ty0 (Iso.inv assoc-Σ-curry a) ≃ Ty1 a   
  assoc-Σ-curryΣ- a = Σ-assoc-≃
  

  assoc-Σ-curryP : PathP (λ i → isoToPath assoc-Σ-curry i → Type ℓ)
                       Ty0
                       Ty1
  assoc-Σ-curryP = equivFun (over-≃ (isoToEquiv assoc-Σ-curry) Ty0 Ty1) assoc-Σ-curryΣ- 
               ...
               sym (transportComposite (λ j → Br (transportRefl (equivFun (isoToEquiv IsoAB) xx) (~ j)))
                ...
                ∙∙
                  {!!} ≡⟨ {!!} ⟩
                       ...
                       transp (λ j → Ar (snd ? j))
                                                i0 (invEq (AB-Rec≃ (Iso.fun IsoAB xx)) x) ∎ 
...
--   --                        ≡
--   --                        ((λ j →
--   --                            ua (AB-Rec≃ (fst (invEquiv (Σ-cong-equiv-fst {B = λ v → Ar v → C} (isoToEquiv (invIso IsoAB))) .fst (xx , yy)))) (~ j))
--   --                         ∙ (cong Ar (snd (equivCtr (isoToEquiv (invIso IsoAB)) xx))))
--   --           ww' = (λ ii → (cong Br (sym (transportRefl (equivFun (isoToEquiv IsoAB) xx)))
--   --                         ∙
--   --                         ((λ j → equivFun AB-RecP-≃ AB-Rec≃ (~ j) (transp (λ i → isoToPath IsoAB (i ∧ ~ j)) j xx) ))
--   --                      ))
...
--   lem-pentaApp : (IsoAB : Iso A B)
--             → (AB-Rec≃ : ∀ a → Ar (equivFun (isoToEquiv (invIso IsoAB)) a) ≃ Br a)

--             → Path (Σ A (λ a → Ar a → C) ≡ Σ B (λ a → Br a → C))
--               (ua (compEquiv (invEquiv (Σ-cong-equiv-fst (isoToEquiv (invIso IsoAB))))
--                              (Σ-cong-equiv-snd λ a → cong≃ (λ X → X → C) (AB-Rec≃ a))
--                              ))
--               (
--               cong {A = Σ (Type (ℓ-suc ℓ)) λ x → x → Type ℓ } (λ a → Σ (fst a) λ x → (snd a) x → C)
--                     (λ i → (isoToPath IsoAB i) , equivFun (over-≃ (isoToEquiv IsoAB) Ar Br) AB-Rec≃ i))
--   lem-pentaApp IsoAB AB-Rec≃ = lem-penta IsoAB AB-Rec≃ ((over-≃ (isoToEquiv IsoAB) Ar Br)) 
...
--   hlp1 = compEquiv (invEquiv (Σ-cong-equiv-fst (isoToEquiv (invIso  (assoc-Σ-curry Ar Br Cr )))))
...
--                                      (λ x₅ →
--                                         Br x₅ → Σ C (λ x₆ → Cr x₆ → D)))))
--                    (compEquiv (isoToEquiv (assoc-Σ-curry _ _ Dr )) (isoToEquiv (assoc-Σ-curry _ (λ v → Br v) λ x → Σ (Cr (fst x)) λ x₁ → Dr (snd x x₁))) )
--                     (compEquiv hlp1
--                          (compEquiv (isoToEquiv (assoc-Σ-curry Ar (λ z → Σ (Br (fst z)) (λ x₁ → Cr (snd z x₁))) Dr))
--                                   (Σ-cong-equiv-snd λ a → equivΠCod λ a₁ → isoToEquiv (assoc-Σ-curry _ _ Dr))))
--   penta-Σ-curry-≃ =

--    equivEq (funExt λ x → ΣPathP (refl , funExt λ x₁ → ΣPathP (refl , (funExt (λ x₂ → ΣPathP (refl , funExt
   
--        λ x₃ → sym (transport⁻Transport {A = D} refl _) ∙ cong (λ x → transport⁻Transport {A = D} refl x i0)
--             (
--       λ i' → snd x
--       (transp
--        (λ j →
--           Σ (Σ (Ar (fst (fst (fst x)))) (λ x₅ → Br (snd (fst (fst x)) x₅)))
--           (λ x₅ → Cr (snd (fst x) (fst x₅ , snd x₅))))
--        (~ i')
--        ((transp (λ i → Ar (fst (fst (fst x)))) (~ i') x₁ ,
--          transp
--          (λ i →
--             Br
--             (snd (fst (fst x))
--              (transp (λ i₁ → Ar (fst (fst (fst x)))) (~ i ∨ (~ i')) x₁)))
--          (~ i') x₂)
--         ,
--         transp
--         (λ i →
...
--                 Br
--                 (snd (fst (fst x))
--                  (transp (λ i₂ → Ar (fst (fst (fst x)))) (~ i₁ ∨ ~ i ∨ (~ i')) x₁)))
--              (~ i ∨ (~ i')) x₂)))
--         (~ i') x₃))) 
--      )))) ))
...
--                     (λ i → (isoToPath (assoc-Σ-curry Ar Br Cr) i) , assoc-Σ-curryP Ar Br Cr i)
--   lemL = lem-pentaApp _ _ (assoc-Σ-curry Ar Br Cr) (assoc-Σ-curryΣ- Ar Br Cr)
--     -- where
--     --   w' : (x : Σ
--     --              (Σ (Σ A (λ a → Ar a → B))
--     --               (λ x → Σ (Ar (fst x)) (λ x₁ → Br (snd x x₁)) → C))
...
--   lemR : ∀ {B C : Type (ℓ-suc ℓ)} → (Iso-BC : Iso B C) → ua (Σ-cong-equiv-snd λ a → equivΠCod λ a₁ → isoToEquiv Iso-BC)
--            ≡
--          (cong (Σ A) (funExt λ y → cong (λ x → Ar y → x) (isoToPath Iso-BC)))
...
--                                      (λ x₅ →
--                                         Br x₅ → Σ C (λ x₆ → Cr x₆ → D)))))
--                    (isoToPath (assoc-Σ-curry _ _ Dr ) ∙  isoToPath (assoc-Σ-curry _ (λ v → Br v) λ x → Σ (Cr (fst x)) λ x₁ → Dr (snd x x₁)) )
                    
--                     (cong {A = Σ (Type (ℓ-suc ℓ)) λ x → x → Type ℓ } (λ (a : Σ (Type (ℓ-suc ℓ)) (λ x → x → Type ℓ)) → Σ (fst a) λ x → (snd a) x → D)
--                         (λ i → (isoToPath (assoc-Σ-curry Ar Br Cr) i) , assoc-Σ-curryP Ar Br Cr i)

--                        ∙∙ isoToPath (assoc-Σ-curry Ar (λ z → Σ (Br (fst z)) (λ x₁ → Cr (snd z x₁))) Dr)
--                           ∙∙ cong (Σ A) (funExt λ y → cong (λ x → Ar y → x) (isoToPath (assoc-Σ-curry _ _ Dr))))
--   penta-Σ-curry = 
...
--                                      (λ x₅ →
--                                         Br x₅ → Σ C (λ x₆ → Cr x₆ → D))))}
--                 (_∙∙ (isoToPath (assoc-Σ-curry Ar (λ z → Σ (Br (fst z)) (λ x₁ → Cr (snd z x₁))) Dr)) ∙∙_) lemL (lemR (assoc-Σ-curry Br Cr Dr)))
--       )
...
--   ParSig (assocPar x x₁ x₂ i) = isoToPath (assoc-Σ-curry (ParRec x ) (ParRec x₁) (ParRec x₂)) i
