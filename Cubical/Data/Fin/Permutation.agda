{-# OPTIONS --safe #-}
module Cubical.Data.Fin.Permutation where
...
data Perm (n : ℕ) : Type where
  ...
  invo : ∀ k xs → k ∷ k ∷ xs  ≡ xs
  braid : ∀ k → ∀ k< → ∀ xs →  
            (k , <-weaken k<) ∷ (suc k , k<) ∷ (k , <-weaken k<) ∷ xs
         ≡ (suc k , k<) ∷ (k , <-weaken k<) ∷ (suc k , k<) ∷ xs
...
record RelimProp {ℓ} {n : ℕ} (A : (Perm n) → Type ℓ) : Type ℓ where
  ...
  f (braid k k< x i) =
     isProp→PathP (λ i → isPropA (braid k k< x i))
      (∷A (k , <-weaken k<) (∷A (suc k , k<) (∷A (k , <-weaken k<) (f x))))
      (∷A (suc k , k<) (∷A (k , <-weaken k<) (∷A (suc k , k<) (f x)))) i
  f (comm k l x x₁ i) =
...
record Rrec {ℓ} {n : ℕ} (A : Type ℓ) : Type ℓ where
  field
    ...
    braidA : ∀ k → ∀ k< → ∀ x →
         ∷A (k , <-weaken k<) (∷A (suc k , k<) (∷A (k , <-weaken k<) x))
       ≡ ∷A (suc k , k<) (∷A (k , <-weaken k<) (∷A (suc k , k<) x))
...
swap0and1ℕ : ℕ → ℕ
swap0and1ℕ zero = suc zero
swap0and1ℕ (suc zero) = zero
swap0and1ℕ k@(suc (suc _)) = k

sucFunℕ : (ℕ → ℕ) → (ℕ → ℕ)
sucFunℕ x zero = zero
sucFunℕ x (suc x₁) = suc (x x₁)

adjTranspositionℕ : ℕ → ℕ → ℕ
adjTranspositionℕ zero = swap0and1ℕ
adjTranspositionℕ (suc x) = sucFunℕ (adjTranspositionℕ x)

isInvolSwap0and1ℕ : isInvolution swap0and1ℕ
isInvolSwap0and1ℕ zero = refl
isInvolSwap0and1ℕ (suc zero) = refl
isInvolSwap0and1ℕ (suc (suc x)) = refl

mkFinFun : ∀ {n m} → (f : ℕ → ℕ) → (∀ k → k < n → f k < m) → Fin n → Fin m 
mkFinFun f x  = uncurry λ _ p → _ , x _ p


swap0and1 : ∀ {n} → Fin (suc (suc n)) → Fin (suc (suc n))
swap0and1 {n} = mkFinFun swap0and1ℕ
 λ { zero _ → snd fone
   ; (suc zero) _ → snd fzero
   ; (suc (suc k)) p → p }

isInvolSwap0and1 : ∀ {n} → isInvolution (swap0and1 {n})
isInvolSwap0and1 (zero , snd₁) = Fin-fst-≡ refl
isInvolSwap0and1 (suc zero , snd₁) = Fin-fst-≡ refl
isInvolSwap0and1 (suc (suc fst₁) , snd₁) = Fin-fst-≡ refl

swap0and1≃ : ∀ {n} → Fin (suc (suc n)) ≃ Fin (suc (suc n))
swap0and1≃ {n} = involEquiv {f = swap0and1} (isInvolSwap0and1 {n})

sucFunRespIsInvolution : ∀ f →
     isInvolution f → isInvolution (sucFunℕ f)
sucFunRespIsInvolution f x zero = refl
sucFunRespIsInvolution f x (suc x₁) = cong suc (x x₁)

isInvolutionAdjTranspositionℕ : ∀ k → isInvolution (adjTranspositionℕ k)
isInvolutionAdjTranspositionℕ zero = isInvolSwap0and1ℕ
isInvolutionAdjTranspositionℕ (suc k) = sucFunRespIsInvolution _
 (isInvolutionAdjTranspositionℕ k)


-- adjTransposition< : ∀ {n} → (k : Fin (predℕ n)) → 
--                 (k₁ : ℕ) → k₁ < n → adjTranspositionℕ (fst k) k₁ < n
-- adjTransposition< {n} = uncurry
--   (ℕ.elim
--      (λ k'< → ℕ.elim (λ k< → subst (_ <_)
--       (sym (suc-predℕ _ λ x → ¬-<-zero (subst (zero <_) x k<)))
--         (suc-≤-suc k'<))
--           {!!} )
--      {!!})

adjTransposition : ∀ {n} → Fin (predℕ n) → Fin n → Fin n
adjTransposition {n} k = mkFinFun (adjTranspositionℕ (fst k))
  ?
  -- (curry (F.elim (uncurry λ k' k'< → adjTranspositionℕ (fst k) k' < n)
  --   ({!!}) {!!}))



isInvolAdjTransposition : ∀ {n} → ∀ k → isInvolution (adjTransposition {n} k)
isInvolAdjTransposition (k , _) =
  Fin-fst-≡ ∘ isInvolutionAdjTranspositionℕ k ∘ fst
  

adjTransposition≃ : ∀ {n} → Fin (predℕ n) → ⟨ Sym n ⟩
adjTransposition≃ k = involEquiv {f = adjTransposition k} 
 (isInvolAdjTransposition k)


adjTranspositionBraid : ∀ {n} → ∀ k → ∀ k< →
      adjTransposition≃ {n} (k , <-weaken k<) ∙ₑ
      adjTransposition≃ (suc k , k<) ∙ₑ
      adjTransposition≃ (k , <-weaken k<)
      ≡
      adjTransposition≃ (suc k , k<) ∙ₑ
      adjTransposition≃ (k , <-weaken k<) ∙ₑ
      adjTransposition≃ (suc k , k<)
adjTranspositionBraid zero k< = 
  equivEq (funExt (Fin-fst-≡ ∘
    λ { (zero , _) → refl ; ((suc zero) , _) → refl
     ; (((suc (suc zero))) , _ ) → refl
     ; ((suc (suc (suc k))) , _ ) → refl }))
 
adjTranspositionBraid (suc k) k< = 
   equivEq (funExt (Fin-fst-≡ ∘
     λ { (zero , _) → refl
       ; (suc k' , k<') → cong suc
         (cong fst (funExt⁻ (cong fst (adjTranspositionBraid k (<-weaken k<)))
           (k' , <-weaken k<'))) }))

adjTranspositionComm : ∀ {n} → ∀ k l → commT (fst k) (fst l) →
      adjTransposition≃ {n = n} k ∙ₑ adjTransposition≃ l ≡
      adjTransposition≃ l ∙ₑ adjTransposition≃ k
adjTranspositionComm (zero , _) (suc (suc l) , _) z =
  equivEq  (funExt (Fin-fst-≡ ∘
     λ { (zero , _) → refl
       ; (suc zero , _) → refl
       ; (suc (suc _) , _) → refl }))
adjTranspositionComm (suc k , _) (suc (suc l) , _) z =
  {!!}

to≃ : ∀ {n} → Perm n → fst (Sym n)
to≃ {n = n} = Rrec.f (to≃R)
  where

   open GroupStr (snd (Sym n))

   to≃R : Rrec {n = n} (fst (Sym n))
   Rrec.isSetA to≃R = is-set
   Rrec.εA to≃R = 1g
   Rrec.∷A to≃R k = adjTransposition≃ k ∙ₑ_
   Rrec.invoA to≃R k x =
    equivEq (cong (fst x ∘_) (funExt (isInvolAdjTransposition k)))
   Rrec.braidA to≃R k k< x = 
    equivEq (cong ((fst x) ∘_) (cong fst (adjTranspositionBraid k k<)))
   Rrec.commA to≃R k l z x = 
    equivEq (cong ((fst x) ∘_) (cong fst (adjTranspositionComm k l z)))
