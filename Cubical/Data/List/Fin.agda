{-# OPTIONS --safe #-}
module Cubical.Data.List.Fin where
...
open import Cubical.Data.Fin.Permutation
...
-- -- =3 : ℕ → ℕ → ℕ → Type₀ 
-- -- =3 zero zero zero = Unit
-- -- =3 zero zero (suc x₂) = ⊥
-- -- =3 zero (suc x₁) x₂ = ⊥
-- -- =3 (suc x) zero x₂ = ⊥
-- -- =3 (suc x) (suc x₁) zero = ⊥
-- -- =3 (suc x) (suc x₁) (suc x₂) = Unit
...
-- adj' x l@[] = l
-- adj' x l@(x₁ ∷ []) = l
-- adj' zero (x₁ ∷ x₂ ∷ l) = x₂ ∷ x₁ ∷ l
-- adj' (suc x) (x₁ ∷ l@(_ ∷ _)) = x₁ ∷ adj' x l


-- adj'Invo : ∀ k → (l : List A) → adj' k (adj' k l) ≡ l
-- adj'Invo k [] = refl
-- adj'Invo k (x ∷ []) = refl
-- adj'Invo zero (x ∷ x₁ ∷ l) = refl
-- adj'Invo (suc k) (x ∷ x₁ ∷ []) = refl
-- adj'Invo one (x ∷ x₁ ∷ x₂ ∷ l) = refl
-- adj'Invo (suc (suc k)) (x ∷ ll@(x₁ ∷ x₂ ∷ l)) =
--  cong (x ∷_) ((adj'Invo (suc k) ll))

-- permute' : ∀ {n} → Perm n → (l : List A) → List A
-- permute' {n = n} = P.Relim.f (w n)
--   where
--     w : ∀ n → Relim {n = n} (λ _ → (l : List _) → List _)
--     Relim.isSetA (w n) = {!!}
--     Relim.εA (w n) l = l
--     Relim.∷A (w n) k x l = adj' (toℕ k) (x l)
--     Relim.invoA (w n) k x' = funExt λ l → adj'Invo (toℕ k) (x' l)
--     Relim.braidA (w n) = {!!}
--     Relim.commA (w n) = {!!}
...
-- substFin {suc n} {zero} x x₁ = ⊥.rec (ℕsnotz x)
-- substFin {suc n} {suc m} x zero = zero
-- substFin {suc n} {suc m} x (suc x₁) = suc (substFin (cong predℕ x) x₁)

-- -- ·' : ∀ {n m} → n ≡ m → Perm n → Perm m → Perm m
-- -- ·' x ε x₂ = x₂
-- -- ·' x (x₁ ∷ x₃) x₂ =  substFin (cong predℕ x) x₁ ∷ (·' x x₃ x₂) 
-- -- ·' x (invo k x₁ i) x₂ = invo (substFin (cong predℕ x) k) (·' x x₁ x₂) i 
-- -- ·' {suc n} {zero} x (braid k x₁ i) x₂ = {!!}
-- -- ·' {suc (suc n)} {one} x (braid k x₁ i) x₂ = {!!}
-- -- ·' {suc (suc .(suc _))} {two} x (braid zero x₁ i) x₂ = {!!}
-- -- ·' {suc (suc .(suc _))} {suc (suc (suc m))} x (braid zero x₁ i) x₂ =
-- --      braid zero (·' x x₁ x₂) i
-- -- ·' {suc (suc .(suc _))} {two} x (braid (suc k) x₁ i) x₂ = {!!}
-- -- ·' {suc (suc (suc .(suc _)))} {suc (suc (suc m))} x (braid one x₁ i) x₂ = {!!}
-- -- ·' {suc (suc (suc .(suc _)))} {suc (suc (suc m))} x (braid (suc (suc k)) x₁ i) x₂ = {!m n !}
-- --  -- braid {!!} {!!} i
-- --   -- braid {!!} (·' x x₁ x₂) i
-- -- ·' x (comm k l x₁ x₃ i) x₂ = {!!}
-- -- ·' x (trunc x₁ x₃ x₄ y i i₁) x₂ = {!!}

-- isTrans↔ : BinaryRelation.isTrans (_↔_ {A = A})
-- F≃ (isTrans↔ a b c (prm e p) (prm f q)) = {!!}
-- l≡ (isTrans↔ a b c (prm e p) (prm f q)) = {!!}

-- -- isTrans↔' : ∀ n → BinaryRelation.isTrans (↔N {A = A} n)
-- -- F≃ (isTrans↔' n a b c (prm e eqL₁ eqR₁ p) (prm f eqL₂ eqR₂ q)) = f · e
-- -- eqL (isTrans↔' n a b c (prm e eqL₁ eqR₁ p) (prm f eqL₂ eqR₂ q)) = {!!}
-- -- eqR (isTrans↔' n a b c (prm e eqL₁ eqR₁ p) (prm f eqL₂ eqR₂ q)) = {!!}
-- -- l≡ (isTrans↔' {A = A} n a b c (prm e eqL₁ eqR₁ p) (prm f eqL₂ eqR₂ q)) = {!!}
-- --   where
-- --     w : ∀ n → P.Relim {n = n} λ f → (e : Perm n) →
-- --            (a b c : List A) → (p : permute' e a ≡ b)
-- --             (q : permute' f b ≡ c)
-- --             → permute' (f · e) a ≡ c
-- --     Relim.isSetA (w n) = {!!}
-- --     Relim.εA (w n) e a b c p q = p ∙ q
-- --     Relim.∷A (w n) k {xs}  X e a b c p q =
-- --       let X' = X e a (permute' e a) _ refl refl
-- --       in (cong (adj' (toℕ k)) X') ∙∙ cong (permute' (k ∷ xs)) p ∙∙ q 
-- --     Relim.invoA (w n) = {!!}
-- --     Relim.braidA (w n) = {!!}
-- --     Relim.commA (w n) = {!!}
