...
module Cubical.Data.List.HITs.Groupoid.Comm2 where
...
data ℙ {ℓ} (A : Type ℓ) : Type ℓ where
 ...
 𝕡invol : ∀ xs ys zs ws →
    Square (𝕡loop xs ys zs ws) (sym (𝕡loop xs zs ys ws))
           (cong 𝕡base (sym (++-pentagon-diag xs ys zs ws)))
           (cong 𝕡base (++-pentagon-diag xs zs ys ws))
 ...
 𝕡hexU : ∀ ls xs ys zs rs  →
   Square          
     ...
     (𝕡hexDiag ls xs ys zs rs)
     (λ i → 𝕡base (ls ++ xs ++ (++-assoc ys zs rs i)))         
     
     (λ i → 𝕡base ((++-assoc ls ys zs (~ i) ++ xs) ++ rs))
  ...
  record H1 : Type (ℓ-max ℓ ℓb) where
   ...
   record H2 : Type (ℓ-max ℓ ℓb) where
    ...
    record H3 : Type (ℓ-max ℓ ℓb) where
     ...
     field
      binvol : ∀ xs ys zs ws →
        SquareP (λ i j → B (𝕡invol xs ys zs ws i j))
         (bloop xs ys zs ws)
         (symP (bloop xs zs ys ws))
         (cong bbase ((sym (++-pentagon-diag xs ys zs ws))))
         (cong bbase (++-pentagon-diag _ _ _ _))
      bhexU : ∀ ls xs ys zs rs →
        SquareP (λ i j → B (𝕡hexU ls xs ys zs rs i j))
          (bloop ls xs (ys ++ zs) rs)
          (bhexDiag ls xs ys zs rs)
          (cong bbase (λ i → ls ++ xs ++ ++-assoc ys zs rs i))
          (cong bbase λ i → (++-assoc ls ys zs (~ i) ++ xs) ++ rs)
     ...
     module _ (isGroupoidB : ∀ x → isGroupoid (B x)) where
      f₃ : ∀ x → B x
      f₃ (𝕡base x) = bbase x
      f₃ (𝕡loop xs ys zs ws i) = bloop xs ys zs ws i
      f₃ (𝕡invol xs ys zs ws i i₁) = binvol xs ys zs ws i i₁
