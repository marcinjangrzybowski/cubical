...
module _ (isGroupoidA : isGroupoid A) where
  ...
  pent' : (xs ys zs ws : List A) →
              sym (++-assoc xs (ys ++ zs) ws) ∙
                    (λ i₁ →  ++-assoc xs ys zs (~ i₁) ++ ws)
                  ≡
                  ((((λ i₁ → xs ++ ++-assoc ys zs ws i₁)
                  ∙'
                    (sym
                       (++-assoc xs ys (zs ++ ws))))
                   )) ∙ sym (++-assoc (xs ++ ys) zs ws)
  pent' xs ys zs ws j i =
    hcomp
      (λ k →
        λ { (i = i0) → xs ++ (ys ++ zs) ++ ws
          ; (i = i1) →
              hcomp
                (λ k' →
                  λ { (j = i0) → ++-assoc xs ys zs (k' ∧ ~ k) ++ ws
                     ; (j = i1) → ++-assoc (xs ++ ys) zs ws (~ k)
                     ; (k = i1) → ((xs ++ ys) ++ zs) ++ ws
                     })
                (++-assoc (xs ++ ys) zs ws (j ∧ ~ k))
          })
      (hcomp
      (λ k →
        λ { (i = i0) → compPath-filler'
                 ((++-assoc xs ys (zs ++ ws))) ((λ i₁ → xs ++ ++-assoc ys zs ws (~ i₁))) j k

          ; (j = i0) → ++-pentagon-□ xs ys zs ws k (~ i)
          ; (j = i1) → ((((λ i₁ → xs ++ ++-assoc ys zs ws i₁)
                  ∙'
                    (sym
                       (++-assoc xs ys (zs ++ ws))))
                   )) (i ∨ ~ k)
          })
      (++-pentagon-△ xs ys zs ws (~ j) (~ i)))

  toList : List A → B.List A
  toList = recList _ 3 w isGroupoidListA
   where
   w : RecList.RecList (B.List A) 3
   RecList.H1.b[] (fst (fst w)) = B.[]
   RecList.H1.b[ fst (fst w) ] = B.[_]
   RecList.H1._b++_ (fst (fst w)) = B._++_
   RecList.H2.b++ur (snd (fst w)) = BP.++-unit-r
   RecList.H2.b++ul (snd (fst w)) _ = refl
   RecList.H2.b++-assoc (snd (fst w)) = BP.++-assoc
   RecList.H2.b++-pent-diag (snd (fst w)) = (B.elimL (BP.++-assoc) λ {a} l f _ _ → cong (a B.∷_) ∘ (f _ _))
   RecList.H3.b++-triangle (snd w) = B.elimL (λ _ → refl) λ {a} _ → congSqP (λ _ _ → a B.∷_) ∘_
   RecList.H3.b++-pent-△ (snd w) = B.elimL (λ by bz bw i j → BP.++-assoc by bz bw (j ∨ (~ i)))
       (λ {a} _ x _ _ → congSqP (λ _ _ → a B.∷_) ∘ (x _ _))
   RecList.H3.b++-pent-□ (snd w) =
           (B.elimL (λ by bz bw i j → BP.++-assoc by bz bw (j ∧ (~ i)))
           λ {a} _ x _ _ _ → congSqP (λ _ _ → a B.∷_) (x _ _ _))
  ...
  fromList-toList = elimList _ 2 w λ _ → trunc _ _
   where
   ...
   h = B.elimL (sym ∘ ++-unit-l ∘ fromList)
           (λ _ → (_∙ (sym (++-assoc _ _ _)) ∘ cong (_ ∷_ )) ∘_)
   ...
   w : ElimList.ElimList (λ z → fromList (toList z) ≡ z) 2
   ElimList.H1.b[] (fst w) = refl
   ElimList.H1.b[ fst w ] = ++-unit-r ∘ [_]
   ElimList.H1._b++_ (fst w) x y = h _ _ ∙ cong₂ _++_ x y
   ElimList.H2'.b++ur (snd w) = h1
   ElimList.H2'.b++ul (snd w) =
     (λ b i j → hcomp
         (λ k →
           λ { (i = i1) → b (j ∧ k)
