{-# OPTIONS --safe #-}
module Cubical.Data.List.FinDataSimplier where
...
open import Cubical.Data.Nat renaming (snotz to ℕsnotz ; znots to ℕznots) hiding (_·_)
...
-- -- x ≡ y ∘ fst (fst (to≃ n) F≃)
  ...
  -- compP↔ : ∀ n → (a b c : Fin n → A) → (e f : fst (Perm n)) →
  --              (p : PathP (λ i → ua (to≃' n e) i → A) a b)
  --            → (q : PathP (λ i → ua (to≃' n f) i → A) b c)
  --            → PathP (λ i → ua (to≃' n (e · f)) i → A) a c
  -- compP↔ zero a b c e f p q = toPathP (funExt (⊥.rec ∘ ¬Fin0))
  -- compP↔ (suc n) a b c e f p q i =
  --   comp
  --     (λ j → uaPermSq ((to≃' (suc n)) e) (((to≃' (suc n)) f)) j i → A)
  --     (λ j → λ { (i = i0) → a
  --              ; (i = i1) → q j
  --              })
  --     (p i)
  ...
  compP↔ zero a b c e f = cong (_∘' fst (to≃' zero f))  
  compP↔ (suc n) a b c e f = cong (_∘' fst (to≃' (suc n) f))

  isTrans↔ₙ : ∀ n → BinaryRelation.isTrans (_↔ₙ_ {n = n}) 
  isTrans↔ₙ n a b c (prm e p) (prm f q) =
    prm (f · e) (compP↔ n a b c e f p ∙ q )
...
--     in (f zero) ∷2 x , cong suc p
  ...
  -- glueSwap0and1 : ∀ n → PathP (λ i → Fin n → ua (swap0and1≃ {n = n}) i)
  --   (λ x → suc (suc x))
  --   (λ x → suc (suc x))
  -- glueSwap0and1 n i x =
  --     ua-glue (swap0and1≃ {n = n}) i (λ { (i = i0) → suc (suc x) }) (inS (suc (suc x)))

  -- glueSwap0and1 : ∀ n → PathP (λ i → Fin (suc (suc n)) → ua (swap0and1≃ {n = n}) i)
  --   swap0and1
  --   (idfun _)
  -- glueSwap0and1 n i zero =
  --  ua-glue (swap0and1≃ {n = n}) i (λ { (i = i0) → one }) (inS zero)
  -- glueSwap0and1 n i one =
  --     ua-glue (swap0and1≃ {n = n}) i (λ { (i = i0) → zero }) (inS one)
  -- glueSwap0and1 n i (suc (suc x)) =
  --   ua-glue (swap0and1≃ {n = n}) i (λ { (i = i0) → suc (suc x) }) (inS (suc (suc x)))

  -- glueAdjT : ∀ n → (k : Fin n)
  --    → PathP (λ i → Fin (suc n) → ua (adjTransposition k) i)
  --   (fst (adjTransposition k))
  --   (idfun _)
  -- glueAdjT n k x = {!!}
  ...
  record Elim≡ (B : Type ℓ) : Type ℓ where
    ...
    fR= (suc n) (trunc e e₁ x₁ y₁ i i₁) x y p j =
     ...
     -- isSet→isSet' (isSetΠ (λ _ → grpB _ _))
     --   (λ i₂ x₂ x₃ → fR= (suc n) (x₁ i₂) x y x₂ x₃?)
     --   (λ i₂ x₂ x₃ → {!!})
     --   (λ i₂ x₂ x₃ → {!!})
     --   (λ i₂ x₂ x₃ → {!!}) i i₁

-- i = i0 ⊢ fR= (suc n) (x₁ i₁) x y p = ?2 (r = n) (n = e) (e = e₁)
--                                      (e₁ = x₁) (x₁ = y₁) (y₁ = i) (i = i0) p
-- i = i1 ⊢ fR= (suc n) (y₁ i₁) x y p = ?2 (r = n) (n = e) (e = e₁)
--                                      (e₁ = x₁) (x₁ = y₁) (y₁ = i) (i = i1) p
-- i₁ = i0 ⊢ fR= (suc n) e x y p = ?2 (r = n) (n = e) (e = e₁)
--                                 (e₁ = x₁) (x₁ = y₁) (y₁ = i) (i = i₁) (i₁ = i0) p
-- i₁ = i1 ⊢ fR= (suc n) e₁ x y p = ?2 (r = n) (n = e) (e = e₁)
--                                  (e₁ = x₁) (x₁ = y₁) (y₁ = i) (i = i₁) (i₁ = i1) p

      -- grpB (b (suc n) x) (b (suc n) y)
      --   {!λ j → fR= (suc n) e x y p j!} {!!}
      --   (λ i₁ j → {!fR= (suc n) (x₁ i₁) x y p j!}) {!!} i i₁ j

-- i = i0 ⊢ fR= (suc n) (x₁ i₁) x y p j
-- i = i1 ⊢ fR= (suc n) (y₁ i₁) x y p j
-- i₁ = i0 ⊢ fR= (suc n) e x y p j
-- i₁ = i1 ⊢ fR= (suc n) e₁ x y p j
-- j = i0 ⊢ b (suc n) x
-- j = i1 ⊢ b (suc n) y
    ...
    Rrec.fsq (fR (suc n)) (prm e p) (prm f q) =
      compPath-filler _ _ ▷
             (λ i → fR= (suc n) e _ (p (~ i)) (λ j → p (~ i ∧ j))
           ∙ fR= (suc n) f (p (~ i)) _
             (compPath-filler' (cong ((_∘' fst (to≃' (suc n) f))) p) q i))
...
--               (k : Fin n)
--              → (a b : Fin (suc n) → A)
--              → a ∘ fst (adjTransposition k) ≡ b 
--            → tabulateFM2 a ≡
--              tabulateFM2 b
--   toFM2≡UClem .(suc _) zero a b p i =
--     comm (p i one) (p i zero) (tabulateFM2 (p i ∘' suc ∘' suc)) i
--   toFM2≡UClem .(suc _) (suc k) a b p =
--          cong₂ _∷2_ (funExt⁻ p zero)
--       (toFM2≡UClem _ k (a ∘' suc) (b ∘' suc) (cong (_∘' suc) p))
  
--   toFM2≡UC : ∀ n → 
--              (e : fst (Perm n))
--              → (a b : Fin n → A)
--              → a ∘ (fst ((to≃' n) e)) ≡  b 
--            → tabulateFM2 a ≡ tabulateFM2 b
--   toFM2≡UC zero e a b x = refl
--   toFM2≡UC (suc n) (η k) a b p = toFM2≡UClem n k a b p
--   toFM2≡UC (suc n) (e · e₁) a b p =
--     toFM2≡UC (suc n) e₁ a (a ∘' fst (to≃' (suc n) e₁))
--            refl
--       ∙ (toFM2≡UC (suc n) e (a ∘' fst (to≃' (suc n) e₁))
--           b p)
--   toFM2≡UC (suc n) ε a b =  cong tabulateFM2 
--   toFM2≡UC (suc n) (inv e) a b p =
--      sym (toFM2≡UC (suc n) e b a
--      (cong (_∘' fst (to≃' (suc n) e)) (sym p)
--        ∙  cong (a ∘'_ ) (funExt (retEq (to≃' (suc n) e)))))
--   toFM2≡UC (suc n) (PresentedGroup.assoc e e₁ e₂ i) a b p =
--     sym (GL.assoc
--       (toFM2≡UC (suc n) e₂ a (a ∘' fst (to≃' (suc n) e₂)) refl)
--       (toFM2≡UC (suc n) e₁ (a ∘' fst (to≃' (suc n) e₂)) _  refl)
--       (toFM2≡UC (suc n) e (a ∘' fst (to≃' (suc n) e₂) ∘'
--                                  fst (to≃' (suc n) e₁))
--           b p)) i
--   toFM2≡UC (suc n) (idr e i) a b p = {!!}
--   toFM2≡UC (suc n) (idl e i) a b p = {!!}
--   toFM2≡UC (suc n) (invr e i) a b p = {!!}
--   toFM2≡UC (suc n) (invl e i) a b p = {!!}
--   toFM2≡UC (suc .(_ + suc _)) (rel (zero invo) i) a b p =
--     {!!}
--   toFM2≡UC (suc .(_ + suc (suc _))) (rel (zero braid) i) a b p = {!!}
--   toFM2≡UC (suc .(_ + suc (suc _))) (rel (zero (comm x)) i) a b p = {!!}
--   toFM2≡UC (suc n) (trunc e e₁ x₁ y i i₁) a b p = {!!}
...
-- --            → tabulateFM2 a ≡ tabulateFM2 b
-- --   toFM2≡UC zero e a b x = refl
-- --   toFM2≡UC one e a b p = cong (_∷2 []) (cong a (snd (isContrFin1) _) ∙ funExt⁻ p zero)
-- --   toFM2≡UC (suc (suc n)) (η zero) a b p i =
-- --     comm (p i one) (p i zero) (tabulateFM2 (p i ∘' suc ∘' suc)) i
-- --   toFM2≡UC (suc (suc n)) (η (suc x)) a b p =
-- --     cong₂ _∷2_ (funExt⁻ p zero)
-- --       (toFM2≡UC (suc n) (η x) (a ∘' suc) (b ∘' suc) (cong (_∘' suc) p))
-- --   toFM2≡UC (suc (suc n)) (e · e₁) a b p =
-- --      toFM2≡UC (suc (suc n)) e₁ a (a ∘' fst (to≃' (suc (suc n)) e₁))
-- --            refl
-- --       ∙ (toFM2≡UC (suc (suc n)) e (a ∘' fst (to≃' (suc (suc n)) e₁))
-- --           b p)
-- --   toFM2≡UC (suc (suc n)) ε a b p = cong tabulateFM2 p
-- --   toFM2≡UC (suc (suc n)) (inv e) a b p =
-- --     sym (toFM2≡UC (suc (suc n)) e b a
-- --      (cong (_∘' fst (to≃' (suc (suc n)) e)) (sym p)
-- --        ∙  cong (a ∘'_ ) (funExt (retEq (to≃' (suc (suc n)) e)))))
-- --   toFM2≡UC (suc (suc n)) (PresentedGroup.assoc e e₁ e₂ i) a b p =
-- --     sym (GL.assoc
-- --       (toFM2≡UC (suc (suc n)) e₂ a (a ∘' fst (to≃' (suc (suc n)) e₂)) refl)
-- --       (toFM2≡UC (suc (suc n)) e₁ (a ∘' fst (to≃' (suc (suc n)) e₂)) _  refl)
-- --       (toFM2≡UC (suc (suc n)) e (a ∘' fst (to≃' (suc (suc n)) e₂) ∘'
-- --                                  fst (to≃' (suc (suc n)) e₁))
-- --           b p)) i
-- --   toFM2≡UC (suc (suc n)) (idr e i) a b p j =
-- --     hcomp (λ k → λ { (i = i0) → toFM2≡UC (suc (suc n)) e a b p (k ∧ j)
-- --                    ; (j = i0) → tabulateFM2 a
-- --                    ; (j = i1) → toFM2≡UC (suc (suc n)) e a b p k
-- --                    })
-- --           (tabulateFM2 a)
-- --   toFM2≡UC (suc (suc n)) (idl e i) a b p = {!!}
-- --   toFM2≡UC (suc (suc n)) (invr e i) a b p = {!!}
-- --   toFM2≡UC (suc (suc n)) (invl e i) a b p = {!!}
-- --   toFM2≡UC (suc (suc n)) (rel ix i) a b = w' (suc n) ix a b i
-- --     where
-- --       wTy : ∀ n → (ix : PermR' n) → Type ℓ
-- --       wTy zero _ = Unit*
-- --       wTy (suc n) ix = (a b : Fin (suc (suc n)) → A) →
-- --            PathP (λ i → a ∘ fst (to≃' (suc (suc n)) (rel ix i)) ≡ b →
-- --                  tabulateFM2 a ≡ tabulateFM2 b)
-- --               (toFM2≡UC (suc (suc n))
-- --                  (rlt (Fin ((suc n))) (PermR' ((suc n)))
-- --                   (relatorPerm (suc (suc n))) ix)
-- --                  a b)
-- --               (toFM2≡UC (suc (suc n)) ε a b)

-- --       wTyInvo : ∀ n k → wTy (k + suc n) (zero {n = suc n} {k = k} invo)
-- --       wTyInvo = {!!}

-- --       w' : ∀ n → ∀ ix  → wTy n ix
-- --       w' .(k + suc _) (zero {k = k} invo) =
-- --          {!!}
-- --       w' .(k + suc (suc _)) (zero {k = k} braid) = {!!}
-- --       w' .(k + suc (suc _)) (zero {k = k} (comm x)) = {!!}

-- --       -- w : (ix : PermR' (suc n)) → (a b : Fin (suc (suc n)) → A)
-- --       --       → PathP (λ i → a ∘ fst (to≃' (suc (suc n)) (rel ix i)) ≡ b →
-- --       --            tabulateFM2 a ≡ tabulateFM2 b)
-- --       --         (toFM2≡UC (suc (suc n))
-- --       --            (rlt (Fin ((suc n))) (PermR' ((suc n)))
...
-- --   toFM2≡UC (suc (suc n)) (trunc e e₁ x y i i₁) a b =
-- --      {!!}
...
-- --   -- toFM2r : ∀ n → GQ.Rrec {Rt = isTrans↔ₙ n} (FMSet2 A)
-- --   -- Rrec.Bgpd (toFM2r n) = trunc
-- --   -- Rrec.fb (toFM2r n) = tabulateFM2 {n}
-- --   -- Rrec.feq (toFM2r n) (prm e p) =
-- --   --   toFM2≡UC n e _ _ p

-- --   -- Rrec.fsq (toFM2r zero) (prm e p) (prm f q) = refl
-- --   -- Rrec.fsq (toFM2r one) (prm e p) (prm f q) = compPath-filler _ _ ▷
-- --   --      (sym (cong-∙ (_∷2 []) _ _) ∙
-- --   --       cong (cong (_∷2 [])) {!!}) --todo: apply homotopy-neutral 
-- --   -- Rrec.fsq (toFM2r (suc (suc n))) {a = a} {b} {c} (prm e p) (prm f q) =
-- --   --   compPath-filler _ _ ▷
-- --   --     (λ i → toFM2≡UC (suc (suc n)) e a (p (~ i)) (λ j → p (~ i ∧ j))
-- --   --          ∙ toFM2≡UC (suc (suc n)) f (p (~ i)) c
-- --   --            (compPath-filler' (cong ((_∘' fst (to≃' (suc (suc n)) f))) p) q i))

-- --   -- toFM2 : ∀ n → Fin→//↔ n → FMSet2 A
-- --   -- toFM2 n = GQ.Rrec.f (toFM2r n)


-- --   -- toFM2r-lenR : ∀ n → RelimProp (λ z → len2 (toFM2 n z) ≡ n)
-- --   -- RelimProp.truncB (toFM2r-lenR n) z = isSetℕ (len2 (toFM2 n z)) n
-- --   -- RelimProp.fb (toFM2r-lenR zero) _ = refl
-- --   -- RelimProp.fb (toFM2r-lenR (suc n)) _ = cong suc (RelimProp.fb (toFM2r-lenR n) _)

-- --   -- toFM2r-len : ∀ n → ∀ x → len2 (toFM2 n x) ≡ n
-- --   -- toFM2r-len n = RelimProp.f (toFM2r-lenR n)


-- -- --   module _ (x : A) where

-- -- --     rAppendLem : ∀ n (a b : Fin n → A) 
-- -- --           → (e : fst (Perm n))
-- -- --           → (p : a ∘ fst (to≃' n e) ≡ b)
-- -- --           → 
-- -- --               Path (Fin→//↔ (suc n))
-- -- --                 ([ cons→ x a ]//)
-- -- --                 ([ cons→ x b ]//)
-- -- --     rAppendLem zero a b e p = cong [_]// (refl =→ =■)
-- -- --     rAppendLem (suc n) a b (η k) p =
-- -- --         eq// (prm (η (suc k))
-- -- --          ((refl =→ refl) ∙ cong (cons→ x) p)) 
-- -- --     rAppendLem (suc n) a b (e · e₁) p =
-- -- --            rAppendLem (suc n) a _ e₁  refl
-- -- --         ∙ rAppendLem (suc n) _ b e p 
-- -- --     rAppendLem (suc n) a b ε p =
-- -- --       cong [_]// (cong (cons→ x) p)
-- -- --     rAppendLem (suc n) a b (inv e) p =
-- -- --       let z = rAppendLem (suc n) b a e
-- -- --               (cong (_∘' fst (to≃' (suc n) e)) (sym p)
-- -- --                 ∙ cong (a ∘'_) (funExt (retEq (to≃' (suc n) e))))
-- -- --       in sym z
-- -- --     rAppendLem (suc n) a b (PresentedGroup.assoc e e₁ e₂ i) p =
-- -- --       GL.assoc
-- -- --         (rAppendLem (suc n) a _ e₂ refl)
-- -- --         (rAppendLem (suc n) _ _ e₁ refl)
-- -- --         (rAppendLem (suc n) _ b e p)
-- -- --         (~ i)
-- -- --     rAppendLem (suc n) a b (idr e i) p = {!!}
-- -- --     rAppendLem (suc n) a b (idl e i) p = {!!}
-- -- --     rAppendLem (suc n) a b (invr e i) p = {!!}
-- -- --     rAppendLem (suc n) a b (invl e i) p = {!!}
-- -- --     rAppendLem (suc n) a b (rel ix i) = w ix i
-- -- --       where
-- -- --         w : (ix : PermR' n) →
-- -- --           PathP (λ i → a ∘ fst (to≃' (suc n) (rel ix i)) ≡ b →
-- -- --             Path (Fin→//↔ (suc (suc n))) [ cons→ x a ]// [ cons→ x b ]//)
-- -- --           (rAppendLem (suc n) a b
-- -- --          (rlt (Fin n) (PermR' n)
-- -- --           (relatorPerm (suc n)) ix))  
-- -- --            (cong [_]// ∘ (cong (cons→ x)))
-- -- --         w (zero z) i p = {!!}
        
-- -- --     rAppendLem (suc n) a b (trunc e e₁ x y i i₁) p = {!!}
    
-- -- --     rAppend : ∀ n → GQ.Rrec {Rt = isTrans↔ₙ n} (Fin→//↔ (suc n))
-- -- --     Rrec.Bgpd (rAppend n) = squash//
-- -- --     (Rrec.fb (rAppend n)) f = [ cons→ x f ]//
-- -- --     (Rrec.feq (rAppend n)) {a = a} {b} (prm e p) = 
-- -- --       rAppendLem n a b e p
-- -- --       -- eq// (prm (sucPerm* e) ({!!} ∙ cong (cons→ x) p))
-- -- --     Rrec.fsq (rAppend zero) {a} {b} {c} (prm e p) (prm f q) =
-- -- --       compPath-filler _ _ ▷ {!!}
-- -- --     Rrec.fsq (rAppend (suc n)) {a} {b} {c} (prm e p) (prm f q) = 
-- -- --       compPath-filler _ _ ▷
-- -- --         λ i → rAppendLem (suc n) a (p (~ i)) e (λ j → p (~ i ∧ j))
-- -- --             ∙ rAppendLem (suc n) (p (~ i)) c f
-- -- --                (compPath-filler' (cong ((_∘' fst (to≃' (suc n) f))) p) q i)
...
-- -- -- -- -- toFM2≡UC (suc (suc n)) e a b p ∙
-- -- -- -- --       (λ i → toFM2≡UC (suc (suc n)) f b c q i)
-- -- -- -- --       ≡
-- -- -- -- --       toFM2≡UC (suc (suc n)) e a (λ x → a (fst (to≃' (suc (suc n)) e) x))
-- -- -- -- --       (λ _ → a ∘ fst (to≃' (suc (suc n)) e))
-- -- -- -- --       ∙
-- -- -- -- --       toFM2≡UC (suc (suc n)) f (λ x → a (fst (to≃' (suc (suc n)) e) x)) c
-- -- -- -- --       (l≡ (isTrans↔ₙ (suc (suc n)) a b c (prm e p) (prm f q)))
...
-- -- -- -- -- --              (e : fst (Perm n))
-- -- -- -- -- --              → (a b : Fin n → A)
-- -- -- -- -- --              → PathP (λ i → ua ((to≃' n) e) i → A) a b 
-- -- -- -- -- --            → tabulateFM2 a ≡ tabulateFM2 b
-- -- -- -- -- --   toFM2≡UC zero e a b _ = refl
-- -- -- -- -- --   toFM2≡UC one e a b p = {!!}
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (η zero) a b p i =
-- -- -- -- -- --    let z = p i ∘ glueSwap0and1 n i 
-- -- -- -- -- --    in comm (z one) (z zero)
-- -- -- -- -- --        (tabulateFM2 (z ∘' suc ∘' suc)) i
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (η (suc x)) a b p =
-- -- -- -- -- --    cong₂ _∷2_ (λ i → p i (glueAdjT (suc n) (suc x) i zero) )
-- -- -- -- -- --      (toFM2≡UC (suc n) (η x) (a ∘ suc) (b ∘ suc) {!!})
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (e · e₁) a b p =
-- -- -- -- -- --     let z = toFM2≡UC (suc (suc n)) e a (b ∘' fst ((to≃' _) e₁)) {!!}
-- -- -- -- -- --         z' = toFM2≡UC (suc (suc n)) e₁ (b ∘' fst ((to≃' _) e₁)) b
-- -- -- -- -- --           (congP (λ _ → b ∘'_) λ i x → ua-unglue ((to≃' (suc (suc n))) e₁) i x)
-- -- -- -- -- --     in z ∙ z'
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) ε a b p =
-- -- -- -- -- --     {!cong tabulateFM2 p!}
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (inv e) a b p =
-- -- -- -- -- --     sym (toFM2≡UC (suc (suc n)) (e) b a {!symP p!})
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (PresentedGroup.assoc e e₁ e₂ i) a b p = {!!}
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (idr e i) a b p = {!!}
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (idl e i) a b p = {!!}
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (invr e i) a b p = {!!}
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (invl e i) a b p = {!!}
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (rel ix i) a b p = {!!}
-- -- -- -- -- --   toFM2≡UC (suc (suc n)) (trunc e e₁ x y i i₁) a b p = {!!}

-- -- -- -- -- -- -- --   toFM2≡UC : ∀ n → 
-- -- -- -- -- -- -- --              (F≃ : fst (Perm n))
-- -- -- -- -- -- -- --              → (a : Fin n → A)
-- -- -- -- -- -- -- --              -- (l≡ : a ≡ a ∘ fst (fst (to≃ n) F≃))
-- -- -- -- -- -- -- --            → (tabulateFM2 (a ∘ fst (fst (to≃ n) F≃))) ≡ (tabulateFM2 a)
-- -- -- -- -- -- -- --   toFM2≡UC zero e a = {!!}
-- -- -- -- -- -- -- --   toFM2≡UC (suc .(suc _)) (η zero) a = comm _ _ _
-- -- -- -- -- -- -- --   toFM2≡UC (suc .(suc _)) (η (suc x)) a =
-- -- -- -- -- -- -- --     cong (a zero ∷2_) (toFM2≡UC _ (η (x)) (a ∘ suc))
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) (e PresentedGroup.· e₁) a = {!!}
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) ε a = refl
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) (inv e) a = {!!}
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) (PresentedGroup.assoc e e₁ e₂ i) a = {!!}
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) (idr e i) a = {!!}
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) (idl e i) a = {!!}
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) (invr e i) a = {!!}
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) (invl e i) a = {!!}
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) (rel ix i) a = {!!}
-- -- -- -- -- -- -- --   toFM2≡UC (suc n) (trunc e e₁ x y i i₁) a = {!!}
...
-- -- -- -- -- -- -- -- --    let (e' , p') = unwindPermHead e
-- -- -- -- -- -- -- -- --        e0 = fst e zero
-- -- -- -- -- -- -- -- --        pS = p ∙ cong (λ f → b ∘ (fst f)) p'
-- -- -- -- -- -- -- -- --          --subst (λ f → a ≡ b ∘ f) (cong fst p') p
-- -- -- -- -- -- -- -- --        q = toFM2≡ n {a = a ∘ suc} {b = b ∘ (fst (rot≃ e0)) ∘ suc}
-- -- -- -- -- -- -- -- --           (prm e' ((cong (_∘ suc) pS)) )
-- -- -- -- -- -- -- -- --    in cong₂ _∷2_ (funExt⁻ pS zero) q ∙ toFM2≡Punch e0 b


-- -- -- -- -- -- -- -- --   toFM2≡-L∙ : ∀ n → (a b c : Fin n → A) →
-- -- -- -- -- -- -- -- --               (p : a ≡ b) (r : b ↔ₙ c)
-- -- -- -- -- -- -- -- --             → cong (fst ∘ tabulateFM2OfLen) p ∙ toFM2≡ n r
-- -- -- -- -- -- -- -- --              ≡ toFM2≡ n (isTrans↔ₙ n a b c (prm (idEquiv _) p) r)
-- -- -- -- -- -- -- -- --   toFM2≡-L∙ n a b c p r = {!!}

-- -- -- -- -- -- -- -- --   toFM2≡Uid : ∀ {n} → (a b : Fin n → A) → ∀ p →
-- -- -- -- -- -- -- -- --     toFM2≡ n {a = a} {b = b} (prm (idEquiv _) p) ≡
-- -- -- -- -- -- -- -- --       cong (fst ∘ tabulateFM2OfLen) p
-- -- -- -- -- -- -- -- --   toFM2≡Uid {zero} a b p = refl
-- -- -- -- -- -- -- -- --   toFM2≡Uid {suc n} a b p =
-- -- -- -- -- -- -- -- --    let (e' , p') = unwindPermHead (idEquiv _)       
-- -- -- -- -- -- -- -- --        pS = p ∙ cong (λ f → b ∘ (fst f)) p'
-- -- -- -- -- -- -- -- --        z = toFM2≡Uid {n} (a ∘ suc) (b ∘ suc) (cong (_∘ suc) (p))
-- -- -- -- -- -- -- -- --    in sym (rUnit _) ∙
-- -- -- -- -- -- -- -- --         {!!} ∙ (λ i j →
-- -- -- -- -- -- -- -- --           (rUnit λ j → p j zero) (~ i) j ∷2
-- -- -- -- -- -- -- -- --             toFM2≡Uid {n} (a ∘ suc) (b ∘ suc) (cong (_∘ suc) (rUnit p (~ i))) i j) 

-- -- -- -- -- -- -- -- --   tab-coh : ∀ n → (a b c : Fin n → A)
-- -- -- -- -- -- -- -- --      → (r : a ↔ₙ b) (s : b ↔ₙ c) →
-- -- -- -- -- -- -- -- --               toFM2≡ n r ∙ (toFM2≡ n s) ≡
-- -- -- -- -- -- -- -- --               toFM2≡ n (isTrans↔ₙ n a b c r s)
-- -- -- -- -- -- -- -- --   tab-coh zero a b c r s = sym compPathRefl
-- -- -- -- -- -- -- -- --   tab-coh (suc n) a b c (prm e p) (prm f q) = {!!}
-- -- -- -- -- -- -- -- --     -- h n e f a b c p q

-- -- -- -- -- -- -- -- --     where
-- -- -- -- -- -- -- -- --       H : ∀ n → Fin (suc n) ≃ Fin (suc n) → Type ℓ
-- -- -- -- -- -- -- -- --       H n e = (f : Fin (suc n) ≃ Fin (suc n))
-- -- -- -- -- -- -- -- --            → (a b c : Fin (suc n) → A)
-- -- -- -- -- -- -- -- --            → (p : a ≡ b ∘ fst e) (q : b ≡ c ∘ fst f) → 
-- -- -- -- -- -- -- -- --                     toFM2≡ (suc n) {b = b}  (prm e p) ∙
-- -- -- -- -- -- -- -- --                       (toFM2≡ (suc n) {b = c}  (prm f q)) ≡
-- -- -- -- -- -- -- -- --                     toFM2≡ (suc n) (isTrans↔ₙ (suc n) a b c (prm e p) (prm f q))

-- -- -- -- -- -- -- -- --       h1 : ∀ n → H n (idEquiv _) 
-- -- -- -- -- -- -- -- --       h1 n f a b c p q =
-- -- -- -- -- -- -- -- --         cong (_∙ (toFM2≡ (suc n) {b = c}  (prm f q)))
-- -- -- -- -- -- -- -- --           (toFM2≡Uid a b p) ∙
-- -- -- -- -- -- -- -- --           toFM2≡-L∙ (suc n) a b c p (prm f q)

-- -- -- -- -- -- -- -- --       hInd : ∀ n → (∀ e → H (suc n) e)
-- -- -- -- -- -- -- -- --              → ∀ k e
-- -- -- -- -- -- -- -- --              → H (suc (suc n)) e
-- -- -- -- -- -- -- -- --              → H (suc (suc n)) (sucPerm (adjTransposition (suc k)) ∙ₑ e) 
-- -- -- -- -- -- -- -- --       hInd n X k e Y f a b c p q =
-- -- -- -- -- -- -- -- --         assoc _ _ _ ∙ {!!}
-- -- -- -- -- -- -- -- --        where
-- -- -- -- -- -- -- -- --          x : {!!}
-- -- -- -- -- -- -- -- --          x = X
...
-- -- -- -- -- -- -- -- --          y : _
-- -- -- -- -- -- -- -- --          y = Y f (a ∘ swap0and1) b c {!p!} q

    


-- -- -- -- -- -- -- -- --       hS : ∀ n → ∀ e → H (suc n) e → H (suc n) (swap0and1≃ ∙ₑ e) 
-- -- -- -- -- -- -- -- --       hS n = {!!}


-- -- -- -- -- -- -- -- --       -- h : ∀ n → ∀ e → H n e
-- -- -- -- -- -- -- -- --       -- h n = GeneratedElimConstr'
-- -- -- -- -- -- -- -- --       --          (SymData (suc n) )
-- -- -- -- -- -- -- -- --       --          (generatedSym n)
-- -- -- -- -- -- -- -- --       --          _
-- -- -- -- -- -- -- -- --       --          (h1 n)
...
-- -- -- -- -- -- -- -- -- --    -- (cong (λ e → fst (tabulateFM2OfLen (λ x → (b ∘ fst e) (x)))) p')
