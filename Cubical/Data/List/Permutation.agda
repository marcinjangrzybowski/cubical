{-# OPTIONS --safe #-}
module Cubical.Data.List.Permutation where
...
module _ {A : Type ℓ} where
  ...
  lookup (x ∷ _) nothing = x
  lookup (_ ∷ l) (just x) = lookup l x

  remove : ∀ (l : List A) → Fin (length l) → List A 
  remove (_ ∷ l) nothing = l
  remove (x ∷ l) (just k) = x ∷ remove l k

  isEmpty : List A → Type
  isEmpty [] = Unit
  isEmpty (x ∷ x₁) = ⊥

  _↔e_ : List A → List A → Type
  [] ↔e y = isEmpty y
  (_ ∷ xs) ↔e y =
    Σ (Fin (length y)) λ k → (xs ↔e (remove y k))


  ↔p : ∀ {x y : List A} → (x ↔e y) → Type ℓ
  ↔p {[]} {[]} k = Unit*
  ↔p {x ∷ x₁} {y} (k , ks) = (x ≡ lookup y k) × ↔p {x₁} {remove y k} ks

  _↔_ : List A → List A → Type ℓ
  x ↔ y = Σ (x ↔e y) (↔p {x} {y}) 
  ...
  isTrans↔e : BinaryRelation.isTrans (_↔e_)
  isTrans↔e [] [] [] _ _ = _
  isTrans↔e (x₂ ∷ a) (x₃ ∷ b) (x₄ ∷ c) (nothing , ks) (l , ls) =
   l , isTrans↔e a b _ ks ls
  isTrans↔e (x₂ ∷ a) (x₃ ∷ b) (x₄ ∷ c) (just x , ks) (l , ls) =
    {!!} , isTrans↔e {!!} {!!} {!!} ks {!!}


  tabulateFM2 : ∀ {n} → (Fin n → A) → FMSet2 A
  tabulateFM2 {zero} _ = []
  tabulateFM2 {suc n} f = (f nothing) ∷2 tabulateFM2 (f ∘ just)

  List→FMSet : List A → FMSet2 A
  List→FMSet = foldr {B = FMSet2 A} _∷2_ []

  punchFM2≡ : ∀ ys → ∀ k → lookup ys k ∷2 List→FMSet (remove ys k) ≡ List→FMSet ys
  punchFM2≡ (x ∷ ys) nothing = refl
  punchFM2≡ (x ∷ ys) (just x₁) = comm _ _ _ ∙ cong (x ∷2_) (punchFM2≡ ys x₁)

  FM2≡ : ∀ {xs ys : List A}
           → xs ↔ ys → List→FMSet xs ≡ List→FMSet  ys
  FM2≡ {[]} {[]} r = refl
  FM2≡ {x ∷ xs} {ys} ((k , ks) , p , ps ) =
    cong₂ _∷2_ p (FM2≡ (ks , ps)) ∙ punchFM2≡ ys k




-- -- --   isTrans↔e a b c nothing nothing = nothing
-- -- --   isTrans↔e [] [] c nothing (just x) = just x
-- -- --   isTrans↔e [] (x₁ ∷ b) (x₂ ∷ c) nothing (just x) = {!x!}
-- -- --   isTrans↔e (x₁ ∷ a) b c nothing (just x) = {!!}
-- -- --   isTrans↔e a b c (just x) nothing = {!!}
-- -- --   isTrans↔e a b c (just x) (just x₁) = {!!}

-- -- --   isTrans↔ : BinaryRelation.isTrans (_↔_)
-- -- --   isTrans↔ a b c x x₁ = {!x x₁!}
