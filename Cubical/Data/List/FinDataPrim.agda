{-# OPTIONS --safe #-}
module Cubical.Data.List.FinDataPrim where
...
module _ {A : Type ℓ} where
  ...
  record _[_↔_] (n : ℕ) (x y : List A) : Type ℓ where
    constructor prm
    field
      ...
      lX : length x ≡ n
      lY : length y ≡ n 
      l≡ : PathP (λ i → ua ((to≃' n) F≃) i → Maybe A)
             (lookupMb x ∘ toℕ) (lookupMb y ∘ toℕ)


  compP↔ : ∀ {ℓ} {A : Type ℓ} → ∀ n → {a b c : Fin n → A} → (e f : fst (Perm n)) →
               (p : PathP (λ i → ua (to≃' n e) i → A) a b)
             → (q : PathP (λ i → ua (to≃' n f) i → A) b c)
             → PathP (λ i → ua (to≃' n (e · f)) i → A) a c
  compP↔ zero e f p q = toPathP (funExt (⊥.rec ∘ ¬Fin0))
  compP↔ {A = A} (suc n) e f p q i =
    comp
  ...
  isTrans↔ :  ∀ n → BinaryRelation.isTrans (n [_↔_]) 
  isTrans↔ n a b c (prm e lA lB p) (prm f lB' lC q) =
    prm (e · f) lA lC (compP↔ n e f p q)


  lToFM2 : List A → FMSet2 A
  lToFM2 = foldr {B = FMSet2 A} _∷2_ []
...
-- -- tabulateMb : List A → (ℕ → Maybe A) → List A
-- -- tabulateMb [] x = []
-- -- tabulateMb (x₁ ∷ l) x = 
-- --  Mb.rec [] (λ a → a ∷ tabulateMb l (x ∘ suc)) (x zero)

-- -- permuteMb : ∀ {n} → Fin n ≃ Fin n → (l : List A) → List A
-- -- permuteMb {n = zero} e l = l
-- -- permuteMb {n = suc n} e l = tabulateMb l (lookupMb l ∘ toℕ ∘ fst e ∘ fromℕForce)

-- -- permuteL : ∀ {n} → Fin n ≃ Fin n → List A → List A
-- -- permuteL = {!!}

-- -- -- isEquivPermuteMb : ∀ {n} (l : List A) → n ≡ length l → (e : Fin n ≃ Fin n) →
-- -- --                       isEquiv {!permuteMb e!}
-- -- -- isEquivPermuteMb = {!!}
...
-- -- -- -- -- predFin' : {n : ℕ} → Fin n → Fin (suc n) → Fin n
-- -- -- -- -- predFin' x x₁ = {!x!}
...
-- -- -- -- -- --   lookup* [] _ = nothing
-- -- -- -- -- --   lookup* (x ∷ x₂) zero = just x
-- -- -- -- -- --   lookup* (x ∷ x₂) (suc x₁) = lookup* x₂ x₁

-- -- -- -- -- --   remove* : ∀ (xs : List A) → ℕ → List A
-- -- -- -- -- --   remove* [] _ = []
-- -- -- -- -- --   remove* (_ ∷ xs) zero    = xs
-- -- -- -- -- --   remove* (x ∷ xs) (suc i) = x ∷ remove* xs i

-- -- -- -- -- --   lookup' : ∀ {n} → List A → Fin n → Maybe A
-- -- -- -- -- --   lookup' l = lookup* l ∘ toℕ


-- -- -- -- -- --   ↔p' : ∀ {n} → ∀ (xs ys : List A) → ((Fin n ≃ Fin n)) → Type ℓ
-- -- -- -- -- --   ↔p' xs ys e =  ∀ k → lookup' xs k ≡ lookup' ys (fst e k)


-- -- -- -- -- --   ↔p* : List ℕ → (xs ys : List A) → Type ℓ
-- -- -- -- -- --   ↔p* l [] [] = Unit*
-- -- -- -- -- --   ↔p* l [] (x ∷ ys) = ⊥*
-- -- -- -- -- --   ↔p* l (x ∷ xs) [] = ⊥*
-- -- -- -- -- --   ↔p* [] (x ∷ xs) ys = ⊥*
-- -- -- -- -- --   ↔p* (k ∷ ks) (x ∷ xs) ys = 
-- -- -- -- -- --      (just x ≡ lookup* ys k ) × ↔p* ks xs (remove* ys k)

-- -- -- -- -- --   leh : ∀ {n} {m} → ((Fin n ≃ Fin m)) → List ℕ
-- -- -- -- -- --   leh {zero} {zero} _ = []
-- -- -- -- -- --   leh {zero} {suc m} _ = []
-- -- -- -- -- --   leh {suc n} {zero} _ = []
-- -- -- -- -- --   leh {suc n} {suc m} e =
-- -- -- -- -- --       toℕ (fst e zero) ∷ leh (fst (unwindPermHead e))

-- -- -- -- -- --   ↔p : ∀ {n} → ∀ (xs ys : List A) → ((Fin n ≃ Fin n)) → Type ℓ
-- -- -- -- -- --   ↔p xs ys e = ↔p* (leh e) xs ys

-- -- -- -- -- --   p'pIsoTo :  ∀ (xs ys : List A) → (e : (Fin (length xs) ≃ Fin (length xs))) →
-- -- -- -- -- --              (↔p xs ys e) → (↔p' xs ys e)
-- -- -- -- -- --   p'pIsoTo (x₁ ∷ xs) (x₂ ∷ ys) e x zero = fst x
-- -- -- -- -- --   p'pIsoTo (x₁ ∷ xs) (x₂ ∷ ys) e x (suc k) =
-- -- -- -- -- --    let z = p'pIsoTo xs ((remove* (x₂ ∷ ys) (toℕ (fst e zero)))) (fst (unwindPermHead e))
-- -- -- -- -- --               (snd x)
-- -- -- -- -- --    in {!z k ∙ ?!}

-- -- -- -- -- -- --   p'pIso : ∀ (xs ys : List A) → (e : (Fin (length xs) ≃ Fin (length ys))) →
-- -- -- -- -- -- --              Iso (↔p xs ys e) (↔p' xs ys e)
-- -- -- -- -- -- --   Iso.fun (p'pIso xs ys e) = {!!}
-- -- -- -- -- -- --   Iso.inv (p'pIso xs ys e) = {!!}
-- -- -- -- -- -- --   Iso.rightInv (p'pIso xs ys e) = {!!}
-- -- -- -- -- -- --   Iso.leftInv (p'pIso xs ys e) = {!!}



-- -- -- -- -- -- -- --     {!--     (just x ≡ lookup' ys (fst e zero)) × ↔p {xs = xs} {remove ys (fst e zero)}
-- -- -- -- -- -- -- -- --      (unwindPermHead' ys (fst e zero) e)!}
...
-- -- -- -- -- -- -- --   unwindPermHead' : ∀ {n} → ∀ (l : List A) k → (e : Fin (suc n) ≃ Fin (length l))
-- -- -- -- -- -- -- --                        → Fin n ≃ Fin (length (remove l k))
-- -- -- -- -- -- -- --   fst (unwindPermHead' (x ∷ l) zero e) = {!unwindPermHead e!}
-- -- -- -- -- -- -- --   fst (unwindPermHead' (x ∷ l) (suc k) e) = {!!}
-- -- -- -- -- -- -- --     -- λ x → predFin' {!x!}
-- -- -- -- -- -- -- --     --   ( {!fst (e ∙ₑ invEquiv (rot≃ (equivFun e zero))) (suc x)!} )
-- -- -- -- -- -- -- --     -- {!!} ∘ fst (e ∙ₑ invEquiv (rot≃ (equivFun e zero))) ∘ suc
-- -- -- -- -- -- -- --   snd (unwindPermHead' l k e) = {!!}


-- -- -- -- -- -- -- --   hlpE : ∀ a (y : List A) → ∀ k → Fin (length y) ≃ Fin (length (remove (a ∷ y) k))
-- -- -- -- -- -- -- --   hlpE _ _ zero = idEquiv _
-- -- -- -- -- -- -- --   hlpE _ (x ∷ y) (suc k) = sucPerm (hlpE x y k)


-- -- -- -- -- -- -- --   ↔p' : ∀ (xs ys : List A) → ((Fin (length xs) ≃ Fin (length ys))) → Type ℓ
-- -- -- -- -- -- -- --   ↔p' xs ys e =  ∀ k → lookup' xs k ≡ lookup' ys (fst e k)
...
-- -- -- -- -- -- -- -- --   lookupUnwind : ∀ {n} → ∀ {x} → (ys : List A) → ∀ k → (e : (Fin (suc n) ≃ Fin (suc (length ys))))
-- -- -- -- -- -- -- -- --                     → lookup (remove (x ∷ ys) (fst e zero))
-- -- -- -- -- -- -- -- --                           (fst (fst (unwindPermHead e) ∙ₑ hlpE x ys (fst e zero) ) k)
-- -- -- -- -- -- -- -- --                           ≡ lookup (x ∷ ys) (fst e (suc k))
-- -- -- -- -- -- -- -- --   lookupUnwind ys k e = {!!}

-- -- -- -- -- -- -- -- --   p'pIsoTo :  ∀ (xs ys : List A) → (e : (Fin (length xs) ≃ Fin (length ys))) →
-- -- -- -- -- -- -- -- --              (↔p {xs = xs} {ys = ys} e) → (↔p' xs ys e)
-- -- -- -- -- -- -- -- --   p'pIsoTo (x₁ ∷ xs) (x₂ ∷ ys) e (p , ps) zero = p
-- -- -- -- -- -- -- -- --   p'pIsoTo (x₁ ∷ xs) ((_ ∷ ys)) e (p , ps) (suc k) =
-- -- -- -- -- -- -- -- --     p'pIsoTo xs _ _ ps k ∙ lookupUnwind ys k e  

-- -- -- -- -- -- -- -- --   p'pIsoFrom :  ∀ (xs ys : List A) → (e : (Fin (length xs) ≃ Fin (length ys))) →
-- -- -- -- -- -- -- -- --               (↔p' xs ys e) → (↔p {xs = xs} {ys = ys} e)
-- -- -- -- -- -- -- -- --   p'pIsoFrom [] [] e x = _
-- -- -- -- -- -- -- -- --   p'pIsoFrom [] (x₁ ∷ ys) e x = lift (ℕznots (isInjectiveFin≃ e))
-- -- -- -- -- -- -- -- --   p'pIsoFrom (x₁ ∷ xs) [] e x = lift (ℕsnotz (isInjectiveFin≃ e))
-- -- -- -- -- -- -- -- --   p'pIsoFrom (x₁ ∷ xs) (x₂ ∷ ys) e x =
-- -- -- -- -- -- -- -- --     (x zero) , p'pIsoFrom xs (remove (x₂ ∷ ys) (fst e zero))
-- -- -- -- -- -- -- -- --       _ λ k → {!!}

-- -- -- -- -- -- -- -- --   p'pIso : ∀ (xs ys : List A) → (e : (Fin (length xs) ≃ Fin (length ys))) →
-- -- -- -- -- -- -- -- --              Iso (↔p {xs = xs} {ys = ys} e) (↔p' xs ys e)
-- -- -- -- -- -- -- -- --   Iso.fun (p'pIso xs ys e) = {!!}
-- -- -- -- -- -- -- -- --   Iso.inv (p'pIso xs ys e) = {!!}
-- -- -- -- -- -- -- -- --   Iso.rightInv (p'pIso xs ys e) = {!!}
-- -- -- -- -- -- -- -- --   Iso.leftInv (p'pIso xs ys e) = {!!}
...
-- -- -- -- -- -- -- -- -- l≡ (isTrans↔ [] [] [] (prm e p) (prm f q)) = tt*
-- -- -- -- -- -- -- -- -- l≡ (isTrans↔ (x ∷ xs) (y ∷ ys) (z ∷ zs) (prm e (p , ps)) (prm f (q , qs))) =
-- -- -- -- -- -- -- -- --    {!!}
-- -- -- -- -- -- -- -- --   -- let z = isTrans↔ xs (remove (y ∷ ys) (fst e zero))
-- -- -- -- -- -- -- -- --   --          ((remove (z ∷ zs) (fst f zero))) (prm _ ps)
-- -- -- -- -- -- -- -- --   --            (prm {!!} {!!})
-- -- -- -- -- -- -- -- --   -- in (p ∙ {! q!}) , {!l≡ (isTrans↔ _ _ _ (prm ? ps) ?)  !}
...
-- -- -- -- -- -- -- -- -- --   -- data ↔Cases {xs ys : List A} (r : xs ↔ ys) : Type ℓ where
-- -- -- -- -- -- -- -- -- --   --   c= : (p : xs ≡ ys) → fst (F≃ r) ≡ transport (cong (Fin ∘ length) p) → ↔Cases r 
-- -- -- -- -- -- -- -- -- --   --   c↔ : ∀ a l → safe-head a xs ≡ a → (p : safe-tail xs ↔ l)
-- -- -- -- -- -- -- -- -- --   --              → {!!}
-- -- -- -- -- -- -- -- -- --   --              → ↔Cases r

-- -- -- -- -- -- -- -- -- --   record Split↔ {x y : A } {xs ys : List A} (r : x ∷ xs ↔ y ∷ ys) : Type ℓ where
-- -- -- -- -- -- -- -- -- --     constructor s↔
-- -- -- -- -- -- -- -- -- --     field
-- -- -- -- -- -- -- -- -- --       xs' : List A
-- -- -- -- -- -- -- -- -- --       r' : xs ↔ {!!}
     
-- -- -- -- -- -- -- -- -- --     -- c= : x ≡ y → (p : xs ≡ ys) → fst (F≃ r) ≡ transport (cong (Fin ∘ suc ∘ length) p) → ↔Cases r 
-- -- -- -- -- -- -- -- -- --     -- c↔ : ∀ l → (p : xs ↔ l)
-- -- -- -- -- -- -- -- -- --     --            → {! !}
-- -- -- -- -- -- -- -- -- --     --            → ↔Cases r
