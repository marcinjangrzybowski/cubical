...
module _ (isSetA : isSet A) where
  ...
  --    in
  --       (cong (x ∷fm2_) (permute→FMSet2≡ xs (invEquiv e'))
  ...
  --          p↔ = permute→FMSet2≡ a (invEquiv (F≃ p))
  --          q' = factor↔Σ' _ _ q
  --          q= = permute→FMSet2≡EQ y ys (length zs) (invEquiv (F≃ q))
  --          q↔ = permute→FMSet2≡ b (invEquiv (F≃ q))
  --          pq' = factor↔Σ' _ _ (p ∙↔ q)
  --          (r* , p* , P) =
  --            ↔→FMSet2≡COMM (length c) (permute a (invEquiv (F≃ p))) b (fst p') (invEquiv (F≃ q))
  ...
  --          -- PP' = sym (assoc _ _ _) ∙ ?
  --     in  (_ ∙ cong List→FMSet2 (fst p'))
  --            ∙ ((_ ∙ cong List→FMSet2 q=) ∙ cong List→FMSet2 (fst q'))
  --           ≡⟨ cong₂ _∙_
  --                 (sym (assoc _ _ _) ∙ {!!})
  --                 (sym (assoc _ _ _) ∙ {!!}) ⟩ _
  --         _ ≡⟨ {!!} ⟩ _
