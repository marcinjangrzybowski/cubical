module Cubical.Tactics.CommRingSolverFast.EvalHomInt where
...
module HomomorphismProperties where
  ...
  EvalHom+0 {n = ℕ.zero} (const x) [] = +IdL x --cong (scalar R) (+Ridℤ x)
  ...
  -EvalDist (const x) []   = refl --{!!} ---DistScalar R x
  -EvalDist 0H  xs =
  ...
  ... | no p = compute+ₕEvalNotBothZero n P Q r s x xs (¬true→false _ p)
  ...
  +Homeval (P ·X+ Q) (S ·X+ T) (x ∷ xs) =
   ...
   ≡⟨ cong (λ u → u · x + (eval Q xs + eval T xs)) (+Homeval P S (x ∷ xs)) ⟩
    ...
    + (eval Q xs + eval T xs)
   ≡⟨ cong (λ u → u + (eval Q xs + eval T xs)) (·DistL+ (eval P (x ∷ xs)) (eval S (x ∷ xs)) _) ⟩
  ...
  ·0LeftAnnihilates :
    {n : ℕ} (P : IteratedHornerForms νR n) (xs : Vec ⟨ νR ⟩ n)
    → eval (0ₕ ·ₕ P) xs ≡ 0r
  ·0LeftAnnihilates (const x) xs = (cong (λ x → eval (const x)) (0LeftAnnihilates x) ≡$ xs) ∙ Eval0H xs
