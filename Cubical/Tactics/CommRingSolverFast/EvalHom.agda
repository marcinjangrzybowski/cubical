module Cubical.Tactics.CommRingSolverFast.EvalHom where
...
module HomomorphismProperties (R : CommRing ℓ) (homFromℤ@(scalar , _) : CommRingHom ℤCommRing R) where
  private
    νR = CommRing→RawℤAlgebra R homFromℤ
  open CommRingStr (snd R)
  open RingTheory (CommRing→Ring R)
  open IteratedHornerOperations νR
  open IsCommRingHom (snd homFromℤ)

  open HRE R homFromℤ

  EvalHom+0 : {n : ℕ} (P : IteratedHornerForms νR n) (xs : Vec ⟨ νR ⟩ n)
      → eval (0ₕ +ₕ P) xs ≡ eval P xs
  EvalHom+0 {n = ℕ.zero} (const x) [] = cong (scalar) (+Ridℤ x)
  ...
  +Homeval (const x) (const y) [] = pres+ x y
  +Homeval 0H Q xs =
    eval (0H +ₕ Q) xs            ≡⟨ refl ⟩
    eval Q xs                    ≡⟨ sym (+IdL _) ⟩
