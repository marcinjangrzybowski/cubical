module Cubical.Tactics.CommRingSolverFast.FastRationalsReflectionPre where
...
module Equations (A : Type) where
 data Equations : Type where
  eq[] : Equations
  _eq∷_ : ∀ {a₀ a₁ : A} → a₀ ≡ a₁ → Equations → Equations

module _ (dbg : Bool) (nmsℚ : List Name) where
 ...
 wrdℚ = withReduceDefs (false , nmsℚ)
 ...
 isRingOpName : Name → Bool
 isRingOpName (quote ℚ._+_) = true
 isRingOpName (quote ℚ._·_) = true
 isRingOpName (quote ℚ.-_) = true
 isRingOpName _ = false


 congWrapper : Term → Term → (Term → Term) × (Term × Term)
 congWrapper x@(def nm v[ t ]) x'@(def nm' v[ t' ]) =
  if (isRingOpName nm or not (primQNameEquality nm nm'))
   then idfun _ , (x , x')
   else ((λ t → def (quote congS) (def nm [] v∷ v[ t ])) , t , t')
 congWrapper x x' = idfun _ , (x , x')
 ...
 solve!-macro hole =
   do
     ...
     wrdℚ $ debugPrint' "ratSolver" 20 ("normalised goal: " ∷ₑ [ goal ]ₑ)
     just (lhs , rhs) ← wrdℚ $ get-boundary goal
       where
         nothing
           → typeError(strErr "The CommRingSolver failed to parse the goal "
                              ∷ termErr goal ∷ [])
     let cWrap , (lhsW , rhsW) = congWrapper lhs rhs
     (lhs' , rhs' , vars) ← wrdℚ $  toAlgebraExpression (lhsW , rhsW)

     -- if dbg then (typeError (map,ₑ vars)) else do
     ...
     unify hole (cWrap solution)

macro
  ℚ!! : Term → TC _
  ℚ!! = solve!-macro false ((quote ℚ._+_) ∷ (quote (ℚ.-_)) ∷ (quote ℚ._·_) ∷ doNotUnfoldsℚ)

  ℚ!!dbg : Term → TC _
  ℚ!!dbg = solve!-macro true ((quote ℚ._+_) ∷ (quote (ℚ.-_)) ∷ (quote ℚ._·_) ∷ doNotUnfoldsℚ)
