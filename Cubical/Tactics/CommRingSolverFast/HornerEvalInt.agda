module Cubical.Tactics.CommRingSolverFast.HornerEvalInt where

open import Cubical.Foundations.Prelude

open import Cubical.Data.Nat using (ℕ)
import Cubical.Data.Fast.Int as ℤ
open import Cubical.Data.Fast.Int using (ℤ;pos;negsuc)
...
open import Cubical.Algebra.CommRing.Instances.Fast.Int as ℤ
...
module _ where
  ...
  evalIsZero : {n : ℕ} (P : IteratedHornerForms ℤasRawℤAlgebra n)
             → (l : Vec (fst R) n)
             → isZero ℤasRawℤAlgebra P ≡ true
             → eval P l ≡ 0r
  ...
  evalIsZero {n = ℕ.suc n} (P ·X+ Q) (x ∷ xs) isZeroPandQ with isZero ℤasRawℤAlgebra P
  ...
  computeEvalSummandIsZero :
               {n : ℕ}
               (P : IteratedHornerForms ℤasRawℤAlgebra (ℕ.suc n))
               (Q : IteratedHornerForms ℤasRawℤAlgebra n)
             → (xs : Vec (fst R) n)
             → (x : (fst R))
             → isZero ℤasRawℤAlgebra P ≡ true
             → eval (P ·X+ Q) (x ∷ xs) ≡ eval Q xs
  computeEvalSummandIsZero P Q xs x isZeroP with isZero ℤasRawℤAlgebra P
  ... | true = refl
  ... | false = byBoolAbsurdity isZeroP
  ...
  computeEvalNotZero P Q xs x notZeroP with isZero ℤasRawℤAlgebra P
  ... | true = byBoolAbsurdity (sym (¬true→false true notZeroP))
  ... | false = refl

  combineCasesEval :
    {n : ℕ}  (P : IteratedHornerForms ℤasRawℤAlgebra (ℕ.suc n)) (Q : IteratedHornerForms ℤasRawℤAlgebra n)
    (x : (fst R)) (xs : Vec (fst R) n)
    →   eval (P ·X+ Q) (x ∷ xs)
      ≡ (eval P (x ∷ xs)) · x + eval Q xs
  combineCasesEval P Q x xs with isZero ℤasRawℤAlgebra P ≟ true
  ... | yes p =
  ...
  compute+ₕEvalBothZero :
    (n : ℕ) (P Q : IteratedHornerForms ℤasRawℤAlgebra (ℕ.suc n))
    (r s : IteratedHornerForms ℤasRawℤAlgebra n)
    (x : (fst R)) (xs : Vec (fst R) n)
    → (isZero ℤasRawℤAlgebra (P +ₕ Q) and isZero ℤasRawℤAlgebra (r +ₕ s)) ≡ true
    → eval ((P ·X+ r) +ₕ (Q ·X+ s)) (x ∷ xs) ≡ eval ((P +ₕ Q) ·X+ (r +ₕ s)) (x ∷ xs)
  compute+ₕEvalBothZero n P Q r s x xs bothZero with isZero ℤasRawℤAlgebra (P +ₕ Q) and isZero ℤasRawℤAlgebra (r +ₕ s) | bothZero
  ... | true | p =
               eval {A = ℤasRawℤAlgebra} 0H (x ∷ xs)                            ≡⟨ refl ⟩
               0r                                                   ≡⟨ someCalculation {x} ⟩
               0r · x + 0r                                          ≡⟨ step1  ⟩
  ...
  compute+ₕEvalNotBothZero :
    ...
    (r s : IteratedHornerForms ℤasRawℤAlgebra n)
    (x : (fst R)) (xs : Vec (fst R) n)
    → (isZero ℤasRawℤAlgebra (P +ₕ Q) and isZero ℤasRawℤAlgebra (r +ₕ s)) ≡ false
    → eval ((P ·X+ r) +ₕ (Q ·X+ s)) (x ∷ xs) ≡ eval ((P +ₕ Q) ·X+ (r +ₕ s)) (x ∷ xs)
  compute+ₕEvalNotBothZero n P Q r s _ _ notBothZero
    with isZero ℤasRawℤAlgebra (P +ₕ Q) and isZero ℤasRawℤAlgebra (r +ₕ s) | notBothZero
