module Cubical.Tactics.CommRingSolverFast.RawAlgebra where
...
open import Cubical.Data.Fast.Int
  ...
  hiding
  ( ·IdL
  ; ·IdR ; +IdR; +IdL ; sucℤ ; predℤ)

open import Cubical.Tactics.CommRingSolverFast.RawRing renaming (⟨_⟩ to ⟨_⟩ᵣ)
...
-}
module _ (R : CommRing ℓ) (homFromℤ : CommRingHom ℤCommRing R) where
...
--   lemmaPred (pos ℕ.zero) = sym (+IdR (- 1r))
--   lemmaPred (pos (ℕ.suc n)) =
--         (cong scalar (cong (ℕ-hlp n) (ℕ.zero∸ n)  ∙ ℕ-hlp-0 n)
--          ∙ sym (+IdL' _ _ (+InvL 1r)))
--      ∙∙ sym (+Assoc _ _ _)
--      ∙∙ cong (- 1r +_) (sym (lemmaSuc (pos n)))
--   lemmaPred (negsuc n) = {!-Dist 1r ?!}
...
--   +HomScalar (pos (ℕ.suc ℕ.zero)) l = lemmaSuc l


--   +HomScalar (pos (ℕ.suc (ℕ.suc n))) l =
--          cong scalar (sym (+ℤAssoc (pos 1) (pos (ℕ.suc n)) l))
--      ∙  lemmaSuc (pos (ℕ.suc n) +ℤ l)
--      ∙ cong (1r +_) (+HomScalar (pos (ℕ.suc n)) l)
--      ∙ +Assoc _ _ _

--   +HomScalar (negsuc ℕ.zero) l = lemmaPred l
--   +HomScalar (negsuc (ℕ.suc n)) l =
--     scalar (negsuc (ℕ.suc n) +ℤ l)               ≡[ i ]⟨ scalar ((negsuc (ℕ.suc (ℕ.+-zero n (~ i)))) +ℤ l) ⟩
...
--     (- 1r + - scalar (pos (ℕ.suc n))) + scalar l ≡[ i ]⟨ -Dist 1r (scalar (pos (ℕ.suc n))) i
--      + scalar l ⟩
--     (- (1r + scalar (pos (ℕ.suc n)))) + scalar l  ∎
...
--   ·HomScalar : (k l : ℤ)
--              → scalar (k ·ℤ l) ≡ scalar k · scalar l
--   -- ·HomScalar (pos ℕ.zero) (pos n₁) = sym (0LeftAnnihilates _)
--   -- ·HomScalar (pos (ℕ.suc n)) (pos n₁) = {!!}
--   -- ·HomScalar (pos n) (negsuc n₁) = {!!}
--   -- ·HomScalar (negsuc n) (pos n₁) = {!!}
--   -- ·HomScalar (negsuc n) (negsuc n₁) = {!!}
...
--     -- scalar (negsuc (ℕ.suc n)) · scalar l ∎
  ...
  CommRing→RawℤAlgebra : RawAlgebra ℤAsRawRing ℓ
  CommRing→RawℤAlgebra  =
    rawalgebra (fst R) (fst homFromℤ) 0r 1r _+_ _·_ (-_)
