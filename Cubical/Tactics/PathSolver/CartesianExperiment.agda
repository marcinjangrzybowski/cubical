...
module _ (dim : ℕ) where
 macro
  ...
  unConnTestCCTT : R.Term → R.Term → R.TC Unit
  unConnTestCCTT t h = do
   cu ← (extractCuTermNC nothing dim t)
   cu' ← unConn dim cu
   -- te0 ← ppCT dim 100 cu
   te0' ← codeGen true dim 100 cu'
   (R.typeError [ te0' ]ₑ)
   -- wrapError h $
   --        "input:" ∷nl (indentₑ 4 te0)
   --   ++nl "\n∨,∧,~ - removed :" ∷nl (indentₑ 4 te0')
...
module _ {A : Type ℓ} {x y z w : A} (p : x ≡ y)(q : y ≡ z)(r : z ≡ w) where
 ...
 _ : _
 _ = unConnTestCCTT (suc (suc zero)) λ (i j : I) → doubleCompPath-filler p q r i j
