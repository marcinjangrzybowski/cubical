...
module Cubical.DataDefinitions.Nat2 where
...
isNat≡₀ : ∀ {A} → (isn₁ : isNat A) → (isn₂ : isNat A)
         ...
         → (∀ B → ∀ zB₁ → ∀ zB₂ → ∀ sB₁ → ∀ sB₂  → (e1 : subst B z≡ zB₁ ≡ zB₂ ) → (e2 : ∀ n x → subst B ((cong _ s≡)) (sB₁ n x) ≡ sB₂ n x)
              ...
              isNat.ℕ-induction isn₁ B zB₁ sB₁ m
              ≡ isNat.ℕ-induction isn₂ B zB₂ sB₂ m) 
         → (w : isNat→isNat′ isn₁ ≡ isNat→isNat′ isn₂)
         → (e1 : transp (λ x₁ → isNat′.ℕ-ind-zero (w x₁)) i0
                   (isNat.ℕ-induction-zero isn₁)
                   ≡ isNat.ℕ-induction-zero isn₂)
         → (e2 : transp (λ x₁ → isNat′.ℕ-ind-suc (w x₁)) i0
                   (isNat.ℕ-induction-suc isn₁)
                   ≡ isNat.ℕ-induction-suc isn₂)
         →  isn₁ ≡ isn₂ 
isNat≡₀ {ℕ} isn₁ isn₂ z≡ s≡ x w e1 e2 i  =
  isNat′→isNat (w i )
  (toPathP {A = λ x₁ → isNat′.ℕ-ind-zero (w x₁)}
  {x = isNat.ℕ-induction-zero isn₁}
  {y = isNat.ℕ-induction-zero isn₂}
  e1 i)
  (toPathP {A = λ x₁ → isNat′.ℕ-ind-suc (w x₁)}
  {x = isNat.ℕ-induction-suc isn₁}
  {y = isNat.ℕ-induction-suc isn₂}
  e2 i)
...
isNat≡ : ∀ {A} → (isn₁ : isNat A) → (isn₂ : isNat A)
         → (z≡ : (isNat.zero isn₁ ≡ isNat.zero isn₂))
         → (s≡ : (isNat.suc isn₁ ≡ isNat.suc isn₂)) 
         → (w : ∀ B → ∀ zB₁ → ∀ zB₂ → ∀ sB₁ → ∀ sB₂  → (e1 : subst B z≡ zB₁ ≡ zB₂ ) → (e2 : ∀ n x → subst B ((cong _ s≡)) (sB₁ n x) ≡ sB₂ n x)
              → ∀ m → 
              isNat.ℕ-induction isn₁ B zB₁ sB₁ m
              ≡ isNat.ℕ-induction isn₂ B zB₂ sB₂ m)
         → (e1 : _) → (e2 : _)
         → isn₁ ≡ isn₂ 
isNat≡ {ℕ} isn₁ isn₂ z≡ s≡ w e1 e2 = isNat≡₀ isn₁ isn₂ z≡ s≡ w (isNat′≡ (isNat→isNat′ isn₁) (isNat→isNat′ isn₂) z≡ s≡ w) e1 e2
...
isDefinition-isNat = isDefinition₂ h-f h-p h-pp h-ppp
  where
    ...
    h-ppp A₁ A₂ x a = isNat≡
      ...
      (funExt (λ x₁ → funExt λ x₂ → funExt λ x₃ → {!!}))
      {!!}
