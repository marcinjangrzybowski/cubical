...
isDefinition-isNat : IsDefiningProperty IsNat
isDefinition-isNat = isDefiningProperty h-f h-p h-pp λ _ → IsNat.isSet-Nat {_}
  where
...
w : ∀ {B : binℕ.Binℕ → Type₀} → ∀ x → B x
w binℕ.binℕ0 = {!!}
w (binℕ.binℕpos binℕ.pos1) = {!!}
w (binℕ.binℕpos (binℕ.x0 x)) = {!!}
w (binℕ.binℕpos (binℕ.x1 x)) = {!!}
